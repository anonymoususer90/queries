...
typedef struct TableAmRoutine
{
	...
	/*
	 * Return next tuple from `ebi scan`, store in slot.
	 */
	bool 		(*scan_getnextslot_from_ebi) (TableScanDesc scan,
											ScanDirection direction,
											TupleTableSlot *slot);
	...
	/*
	 * Search tuple from `tid` of p-locator into `slot`, after doing a
	 * visibility test according to `snapshot`. If a tuple was found and passed
	 * the visibility test, returns true, false otherwise.
	 */
	bool		(*tuple_search_row_version) (Relation rel,
											 ItemPointer tid,
											 Snapshot snapshot,
											 TupleTableSlot *slot);
...
#ifdef PANDORA
	/* see table_tuple_update() for reference about parameters */
	TM_Result	(*tuple_update_for_pandora_partition) (Relation rel,
								 ItemPointer otid,
...
table_beginscan(Relation rel, Snapshot snapshot,
				int nkeys, struct ScanKeyData *key, List *hap_partid_list)
{
...
#ifdef PANDORA
static inline TableScanDesc
table_beginscan_tid(Relation rel, Snapshot snapshot, List *hap_partid_list)
{
...
#ifdef PANDORA
static inline TableScanDesc
table_beginscan_analyze(Relation rel, List *hap_partid_list)
{
 ...
 */
static inline bool
table_scan_getnextslot_from_ebi(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
	slot->tts_tableOid = RelationGetRelid(sscan->rs_rd);
	 ...
	 */
	if (unlikely(TransactionIdIsValid(CheckXidAlive) && !bsysscan))
		elog(ERROR, "unexpected table_scan_getnextslot_from_ebi call during logical decoding");

	return sscan->rs_rd->rd_tableam->scan_getnextslot_from_ebi(sscan, direction, slot);
}
...
/*
 * Search tuple at `tid` of p-locator into `slot`, after doing a visibility test
 * according to `snapshot`. If a tuple was found and passed the visibility test,
 * returns true, false otherwise.
 *
 * TODO: replace this function to table_index_fetch_tuple if available
 */
static inline bool
table_tuple_search_row_version(Relation rel,
							   ItemPointer tid,
							   Snapshot snapshot,
							   TupleTableSlot *slot)
{
	/*
	 * We don't expect direct calls to table_tuple_search_row_version with
	 * valid CheckXidAlive for catalog or regular tables.  See detailed
	 * comments in xact.c where these variables are declared.
	 */
	if (unlikely(TransactionIdIsValid(CheckXidAlive) && !bsysscan))
	...
	return rel->rd_tableam->tuple_search_row_version(rel, tid, snapshot, slot);
}
				   ...
				   int options, struct BulkInsertStateData *bistate)
{
	...
	struct timespec stat_tpcc_breakdown_insert_tuple_insert_start;
	struct timespec stat_tpcc_breakdown_insert_tuple_insert_end;

	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
				  	  &stat_tpcc_breakdown_insert_tuple_insert_start);
#endif /* STAT_TPCC */
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_tuple_insert_end);
		...
		stat_tpcc_breakdown_insert_tuple_insert += stat_tpcc_elapsed_time;
	}
#endif /* STAT_TPCC */
}
...
#ifdef PANDORA
static inline TM_Result
table_tuple_update_for_pandora_partition(Relation rel, ItemPointer otid, 
				   PandoraTuplePosition tuplePosition, TupleTableSlot *slot,
				   CommandId cid, Snapshot snapshot, Snapshot crosscheck,
				   bool wait, TM_FailureData *tmfd, LockTupleMode *lockmode,
				   bool *update_indexes)
{
	return rel->rd_tableam->tuple_update_for_pandora_partition(rel, otid, 
										 tuplePosition, slot,
										 cid, snapshot, crosscheck,
										 wait, tmfd,
										 lockmode, update_indexes);
...
#ifdef PANDORA
extern const TableAmRoutine *GetPandoraamTableAmRoutine(void);
#endif /* PANDORA */
