...
typedef struct RelationData
{
...
#ifdef DIVA
	bool		siro_checked; 	/* whether siro relation checking or not */
	bool		is_siro; 		/* is it siro relation? */
	...
	bool		is_systable;
#ifdef DIVA
	uint16		record_size;	/* size of one record including line pointer */
	uint16		records_per_block;
#endif /* DIVA */
#endif 
#ifdef HAP
	bool rd_is_hap;
	List *rd_hap_parent_desc_map;
	List *rd_hap_child_desc_map;
	List *rd_hap_parent_relid_list;

	bool rd_hap_partitioned;
	Oid rd_hap_partmap_id;
	Oid rd_hap_partmap_hash_idx_id;
	Oid rd_hap_partmap_gin_idx_id;
#endif /* HAP */
#ifdef PANDORA
	bool rd_pandora_partitioning;
	int32 rd_pandora_part_count;
	int16 rd_pandora_level_count;
	int16 rd_pandora_spread_factor;
#endif /* PANDORA */
} RelationData;
