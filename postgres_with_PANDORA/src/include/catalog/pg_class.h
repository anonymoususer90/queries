...
CATALOG(pg_class,1259,RelationRelationId) BKI_BOOTSTRAP BKI_ROWTYPE_OID(83,RelationRelation_Rowtype_Id) BKI_SCHEMA_MACRO
{
	...
	/* is this table uses HAP module? */
	bool		relishap	BKI_DEFAULT(f);

	/* is this table uses HAP's hidden partition? */
	bool		relhappartitioned	BKI_DEFAULT(f);

	/* is this table uses PANDORA"s online-partitioning? */
	bool		relpandorapartitioning BKI_DEFAULT(f);
