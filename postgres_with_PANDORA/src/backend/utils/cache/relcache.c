...
#ifdef DIVA
#include "access/heaptoast.h" /* for maxlen of tupleDesc */
#endif /* DIVA */
...
RelationBuildTupleDesc(Relation relation)
{
...
#ifdef DIVA
	uint16		maxlen = 0;
	uint16		aligned_maxlen;
	uint16		aligned_maxDataLen = 0;
	bool		siro;
#endif /* DIVA */
	...
	if (siro)
	{
		maxlen = offsetof(HeapTupleHeaderData, t_bits) + BITMAPLEN(need);
		aligned_maxDataLen = MAXALIGN(maxlen);
	}
#endif /* DIVA */
	...
	while (HeapTupleIsValid(pg_attribute_tuple = systable_getnext(pg_attribute_scan)))
	{
		...
		if (siro)
		{
			...
			if (attp->attlen > -1)
				tmplen = attp->attlen; /* add fixed length simply */
			else if (atttypmod > -1) /* add max length if typmod is exist */
				tmplen = type_maximum_heap_size(attp->atttypid, atttypmod);
			else
				tmplen = 32; /* wild guess */
	...
	if (siro)
	{
		...
		aligned_maxlen = MAXALIGN(maxlen);
		aligned_maxDataLen =
			MAXALIGN(RelationGetToastTupleTarget(relation, TOAST_TUPLE_TARGET) -
					 aligned_maxDataLen);

		maxlen = aligned_maxlen < aligned_maxDataLen ?
				 aligned_maxlen : aligned_maxDataLen;

		record_size = (3 * sizeof(ItemIdData)) +
					  (MAXALIGN(PITEM_SZ) + 2 * maxlen);

		relation->rd_att->maxlen = maxlen;
		relation->record_size = record_size;
		relation->records_per_block =
			((BLCKSZ - (RelationGetTargetPageFreeSpace(relation,
...
#ifdef HAP
	/*
	 * Set HAP's hidden attribute related members.
	 */
	relation->rd_is_hap = relp->relishap;
	relation->rd_hap_parent_desc_map = NIL;
	relation->rd_hap_child_desc_map = NIL;
	relation->rd_hap_parent_relid_list = NIL;

	/*
	 * Set HAP's hidden attribute partitioning related members.
	 */
	relation->rd_hap_partitioned = relp->relhappartitioned;
	relation->rd_hap_partmap_id = InvalidOid;
	relation->rd_hap_partmap_hash_idx_id = InvalidOid;
	relation->rd_hap_partmap_gin_idx_id = InvalidOid;
#endif /* HAP */
#ifdef PANDORA
	/*
	 * Set PANDORA's mercer tree related members.
	 */
	relation->rd_pandora_partitioning = relp->relpandorapartitioning;
	relation->rd_pandora_part_count = 0;
	relation->rd_pandora_level_count = 0;
	relation->rd_pandora_spread_factor = 0;
#endif /* PANDORA */
...
#ifdef DIVA
	/* Initialization */
	relation->siro_checked = false;
	relation->is_systable = false;
#endif /* DIVA */
#ifdef HAP
	/* Build HAP's hidden attribute descriptor map */
	if (relation->rd_is_hap)
		HapInitHiddenAttrDescMap(relation);

	/* Build HAP's partition map metedata */
	if (relation->rd_hap_partitioned)
	{
		HapGetPartMapInfo(relid,
						  &relation->rd_hap_partmap_id,
						  &relation->rd_hap_partmap_hash_idx_id,
						  &relation->rd_hap_partmap_gin_idx_id);
	}
#endif /* HAP */
#ifdef PANDORA
	/* Build PANDORA mercer tree's metadata */
	if (relation->rd_pandora_partitioning)
	{
		PandoraGetMercerTreeInfo(relid,
								 &relation->rd_pandora_part_count,
								 &relation->rd_pandora_level_count,
								 &relation->rd_pandora_spread_factor);
	}
#endif /* PANDORA */
...
#ifdef HAP
	if (relation->rd_hap_parent_relid_list)
		list_free(relation->rd_hap_parent_relid_list);
	if (relation->rd_hap_child_desc_map)
		HapFreeHiddenAttrDescMap(relation->rd_hap_child_desc_map);
	if (relation->rd_hap_parent_desc_map)
		HapFreeHiddenAttrDescMap(relation->rd_hap_parent_desc_map);
#endif /* HAP */
	pfree(relation);
}
 ...
 */
inline bool IsSiro(Relation relation)
{
	bool is_siro = true;
	...
	if (likely(relation->siro_checked == true))
		/* If we checked before, use it */
		return relation->is_siro;

	if (!(relkind == RELKIND_RELATION || relkind == RELKIND_MATVIEW))
		/* Skip un-normal relation */
		is_siro = false;
	...
	else if (relation->rd_node.dbNode == 1)
		/* Skip information_schema */
		is_siro = false;

	/* Save siro checking for using later */
	relation->siro_checked = true;
	relation->is_siro = is_siro;
 ...
 * IsPandora
 *
 * The return value is true if Pandora partitioning is being used in the 
 * relation.
 */
inline bool 
IsPandora(Relation relation)
{
	if (relation == NULL)
		return false;

	return relation->rd_pandora_partitioning;
}
#endif
