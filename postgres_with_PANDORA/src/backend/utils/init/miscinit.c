...
#ifdef DIVA
static void sleeping_segfault_handler(int sig);

static void
sleeping_segfault_handler(int sig)
{
...
#ifdef DIVA
	signal(SIGSEGV, sleeping_segfault_handler);
	signal(SIGABRT, sleeping_segfault_handler);
#endif /* DIVA */
...
#endif
#ifdef PANDORA
		case B_PANDORA_PARTITION_LAUNCHER:
			backendDesc = "pandora partition launcher";
			break;
		case B_PANDORA_PARTITION_WORKER:
			backendDesc = "pandora partition worker";
			break;
		case B_PANDORA_BG_WRITER:
			backendDesc = "pandora background writer";
			break;
#endif /* PANDORA */
		case B_ARCHIVER:
...
InitializeSessionUserIdStandalone(void)
{
	...
	AssertState(!IsUnderPostmaster || IsAutoVacuumWorkerProcess() || 
					IsPandoraPartitionManagerProcess() || IsBackgroundWorker);
#else
