		 ...
		 IndexInfo *indexInfo)
{
	...
	unsigned short tup_size;
	unsigned short size;

	if (heapRel->rd_pandora_partitioning)
	{
		/* Load the pandora info and lvl_cnt of new record */
		pandora_fingerprint = (PandoraFingerprint)(values[INDEX_MAX_KEYS]);
		lvl_cnt = (int32)(values[INDEX_MAX_KEYS + 1]);
	}
	else
	{
		pandora_fingerprint = NULL;
		lvl_cnt = -1;
	}
#endif /* PANDORA */
	...
	if (pandora_fingerprint != NULL)
	{
		...
		tup_size = IndexTupleSize(itup);
		size = SizeOfPandoraFingerprintData(lvl_cnt);
		
		if (tup_size + size > INDEX_SIZE_MASK)
			ereport(ERROR, errmsg("Pandora: index tuple is too big"));

		itup->t_fingerprint_offset = tup_size;

		/* Reset a size of tuple */
		itup->t_info &= ~INDEX_SIZE_MASK;
		itup->t_info |= MAXALIGN(tup_size + size);
	}
	// else
		itup->t_tid = *ht_ctid;

	result = _bt_doinsert(rel, itup, checkUnique, indexUnchanged, heapRel,
						  lvl_cnt, pandora_fingerprint);
#else
	itup->t_tid = *ht_ctid;
