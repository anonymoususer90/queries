		...
		bool token)
{
...
#ifdef PANDORA
	/* Pass readers count to block readers */
	offnum = PageAddItemWithDummy(pageHeader, (Item) tuple->t_data,
			tuple->t_len, InvalidOffsetNumber, GetBufferDualRef(buffer),
			blockNum, tupDesc->maxlen, false, true);
#else /* !PANDORA */
	offnum = PageAddItemWithDummy(pageHeader, (Item) tuple->t_data,
			tuple->t_len, InvalidOffsetNumber,
			blockNum, tupDesc->maxlen, false, true);
#endif /* PANDORA */
	...
	ItemPointerSet(&(tuple->t_self), blockNum, offnum - 1);
...
/*
 * PartitionPutHeapTupleWithDummy
 *
 * Place a tuple with a dummy tuple at specified partition page
 */
bool
PartitionPutHeapTupleWithDummy(Relation relation, Buffer buffer,
							   ...
							   BlockNumber blockNum, OffsetNumber offnum)
{
	...
	/* Returned offset number is for a left slot (not dummy) of siro */
	/* Pass readers count to block readers */
	offnum = PageAddItemPartition(pageHeader, (Item) tuple->t_data,
			tuple->t_len, offnum, blockNum, tupDesc->maxlen, false, true,
			tuple->t_pandora_fingerprint, &isInit);
