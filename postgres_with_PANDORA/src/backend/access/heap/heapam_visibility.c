...
#ifdef DIVA
/* For HeapTupleSatisfiesNonVacuumable, tricky */
TransactionId copied_latestRemovedXid = InvalidTransactionId;
#endif /* DIVA */
...
/*
 * HeapTupleSatisfiesVacuumWithoutWrap
 *
 ...
 */
HTSV_Result
HeapTupleSatisfiesVacuumWithoutWrap(HeapTuple htup, TransactionId OldestXmin,
						 			Buffer buffer)
{
	HeapTupleHeader tuple = htup->t_data;
...
#ifdef PANDORA
/*
 * HeapTupleSetHintBitsForPartition
 */
bool
HeapTupleSetHintBitsForPartition(HeapTuple htup, Buffer buffer)
{
	HeapTupleHeader tuple = htup->t_data;
...
#ifdef DIVA
	if (copied_latestRemovedXid < dead_after)
		copied_latestRemovedXid = dead_after;
#endif /* DIVA */
...
bool
IsVersionOld(HeapTuple htup)
{
	HeapTupleHeader tuple = htup->t_data;

	return ((HeapTupleHeaderXminCommitted(tuple) ||
			 TransactionIdDidCommit(HeapTupleHeaderGetRawXmin(tuple))) &&
