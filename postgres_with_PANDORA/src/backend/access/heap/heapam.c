...
/* Private function */
static bool is_prefetching_needed(Relation relation, HeapScanDesc scan);
#endif /* PANDORA */
...
#ifdef LOCKFREE_MEASURE
int lockfree_measure_count = 0;
#endif
...
initscan(HeapScanDesc scan, ScanKey key, bool keep_startblock)
{
	...
	if (scan->rs_nblocks > 10000)
		fprintf(stderr, "relation %s scan, nblocks: %d\n", 
			RelationGetRelationName(scan->rs_base.rs_rd), scan->rs_nblocks);
#endif
...
heapgetpage_DIVA(TableScanDesc sscan, BlockNumber page)
{
...
#if !defined(PANDORA) || defined(USING_LOCK)
		 ...
		 lineoff++, lpp++)
	{
		...
		for (uint8 test_var = CHECK_RIGHT; test_var > CHECK_NONE; test_var--)
		{
			...
			/* Get item pointer of the version */
			v_pointer = PGetPointer(p_locator, test_var);

			/* Set block number and offset */
			v_block = ItemPointerGetBlockNumber(v_pointer);
			v_offset = ItemPointerGetOffsetNumber(v_pointer);
...
#endif /* PANDORA */
{
...
#ifdef LOCKFREE_MEASURE
	/* Make directory for measure */
	if (access("./lockfree_measure", F_OK) == -1)
		mkdir("./lockfree_measure", 0755);

	lockfree_measure_count += 1;
	snprintf(filename, "./lockfree_measure/%d_%d_.dat", 
								MyProc->pid, lockfree_measure_count, 128);

	/* Open file to record stats. */
	scan->lockfree_measure_fd = open(filename, O_WRONLY);

	clock_gettime(CLOCK_MONOTONIC, &scan->seqscan_start_time);
#endif
	...
	if (enable_prefetch && is_prefetching_needed(relation, scan))
	{
		...
		PrefetchHeapDesc heap_desc = 
				(PrefetchHeapDesc) palloc0(sizeof(struct PrefetchHeapDescData));
		PrefetchDesc prefetch_desc = (PrefetchDesc)heap_desc;
		...
		/* Set prefetch state */
 		...
 		heap_desc->prefetch_size = INIT_HEAPSCAN_PREFETCH_SIZE;
		...
		/* TODO: We set io_uring's queue size as MAX_HEAPSCAN_PREFETCH_SIZE*2
		...
		prefetch_desc->ring = 
			(struct io_uring *) palloc0(sizeof(struct io_uring));

		/* Init io_uring queue */
		io_uring_queue_init(MAX_HEAPSCAN_PREFETCH_SIZE * 2, prefetch_desc->ring, 0);

		ResourceOwnerRememberIoUring(CurrentResourceOwner, prefetch_desc->ring);

		scan->rs_base.rs_prefetch = prefetch_desc;
...
is_prefetching_needed(Relation relation, HeapScanDesc scan)
{
	...
	if (scan->rs_nblocks < INIT_HEAPSCAN_PREFETCH_SIZE)
		return false;
...
#ifdef LOCKFREE_MEASURE
	/* Close file about recording stats. */
	if (scan->lockfree_measure_fd != 0)
		close(scan->lockfree_measure_fd);
#endif
...
#ifdef PANDORA
/*
 * PandoraFingerprintGetTuplePositionForIndexscan
 */
void
PandoraFingerprintGetTuplePositionForIndexscan(PandoraRelationMeta *meta, 
											   Relation relation, 
											   PandoraFingerprint fingerprint,
											   PandoraTuplePosition tuplePosition)
{
	int level;
	int levelNums = relation->rd_pandora_level_count;
	int partitionId = fingerprint->rec_key.partid;
	PandoraPartNumber partNum;
	PandoraPartNumber partitionNumbers[32];
	PandoraPartGenNo partitionGenerationNumbers[32];
	PandoraSeqNum seqNum;
	LocationInfo locationInfos[32];

	tuplePosition->rec_key = fingerprint->rec_key;

	seqNum = fingerprint->rec_key.seqnum;

	/* Find partition number level by level. */
	partitionNumbers[0] = fingerprint->rec_pos.first_lvl_partnum;
	for (level = levelNums - 1, partNum = partitionId;
		 level > 0;
		 --level, partNum /= relation->rd_pandora_spread_factor)
	{
		partitionNumbers[level] = partNum;
	}

	/* Increment all reference count. */
	for (level = 0; level < levelNums; ++level)
	{
		if (unlikely(level == 0))
		{
			/* We use only partition number zero in level zero. */
			IncrMetaDataCount(PandoraRelationMetaRefCounter(meta, level, 0));
		}
		else
			IncrMetaDataCount(PandoraRelationMetaRefCounter(meta, level, partitionNumbers[level]));
	}
	...
	for (level = 0; level < levelNums; ++level)
	{
		...
		partitionGenerationNumbers[level] = 
			PandoraGetPartGenerationNumber(meta, level, partNum);

		if (level == 0)
		{
			locationInfos[level].firstSequenceNumber = meta->recordCountsLevelZero[2];
		}
		...
		{
			locationInfos[level] = meta->locationInfo[level][partNum];
		}
	}

	/* Decrement all reference count. */
	for (level = 0; level < levelNums; ++level)
	{
		if (level == 0)
		{
			/* We use only partition number zero in level zero. */
			DecrMetaDataCount(PandoraRelationMetaRefCounter(meta, level, 0));
		}
		else
			DecrMetaDataCount(PandoraRelationMetaRefCounter(meta, level, partitionNumbers[level]));
	}

	/* 
	 * Fix location of the tuple, comparing the sequence number and in-partition
	 * cumulative count.
	 */
	for (level = 0; level < levelNums; ++level)
	{
		if (seqNum >= locationInfos[level].firstSequenceNumber)
		{
			...
			if (level == 0)
			{
				...
				tuplePosition->partitionTuplePosition =
					fingerprint->rec_pos.first_lvl_pos;
			}
			else if (level + 1 == levelNums)
			{
				/* Or the record is in last level, use last level offset. */
				tuplePosition->partitionTuplePosition =
					fingerprint->rec_pos.last_lvl_pos;
			}
			else
			{
				/*
				 ...
				 */
				tuplePosition->partitionTuplePosition =
					(fingerprint->rec_pos.in_partition_counts[level - 1].cumulative_count - 
...
/*
 * PandoraFingerprintGetTuplePositionForUpdate
 */
void
PandoraFingerprintGetTuplePositionForUpdate(PandoraRelationMeta *meta, 
											Relation relation, 
											PandoraFingerprint fingerprint,
											PandoraTuplePosition tuplePosition)
{
...
retry_retrieve_for_update:
	...
	for (level = startLevel; level < levelNums; ++level)
	{
		if (seqNum >= locationInfos[level].firstSequenceNumber)
		{
			PandoraPartLevel partitionLevel = level;
			PandoraPartNumber partitionNumber = partitionNumbers[level];
			bool needDecrRefCount = false;
			PandoraModificationMethod modificationMethod;

			if (level + 1 != levelNums)
			{
				pg_atomic_uint64 *targetPartitionRefCount = (level == 0) ?
				 		PandoraRelationMetaRefCounter(meta, 0, 0) :
						PandoraRelationMetaRefCounter(meta, partitionLevel, partitionNumber);
				PandoraSeqNum firstSeqNum;
				...
				partitionGenerationNumbers[level] = 
						PandoraGetPartGenerationNumber(meta, level, 
												partitionNumbers[level]);

				/* Get repartitioned record count and cumulative record count */
				if (level == 0)
				{
					firstSeqNum = meta->recordCountsLevelZero[2];
				}
				else
				{
					firstSeqNum = 
						meta->locationInfo[level][partitionNumber].firstSequenceNumber;
				}
				...
				if (seqNum >= firstSeqNum)
				{
					...
					modificationMethod =
						GetModificationMethod(meta, targetPartitionRefCount, 
					...
					else
					{
						...
						/* Move to next level */
						startLevel = level + 1;
						Assert(startLevel < levelNums);

						/* 
						 * Navigate to child of current partition, finding correct 
						 * partition number.
						 */
						goto retry_retrieve_for_update;
					}
			...
			else
			{
				...
				tuplePosition->partitionTuplePosition =
					(fingerprint->rec_pos.in_partition_counts[level - 1].cumulative_count - 
						locationInfos[level].reorganizedRecordsCount) & CUMULATIVECOUNT_MASK;
				Assert(fingerprint->rec_pos.in_partition_counts[level - 1].cumulative_count >=
			...
			if (needDecrRefCount)
			{
				...
				else
				{
					 ...
					 */
					tuplePosition->lowerPartitionTuplePosition =
						(fingerprint->rec_pos.in_partition_counts[lowerLevel - 1].cumulative_count - 
							locationInfos[lowerLevel].reorganizedRecordsCount) & CUMULATIVECOUNT_MASK;
					Assert(fingerprint->rec_pos.in_partition_counts[lowerLevel - 1].cumulative_count >=
...
/*
 * pandora_search_version - search version satisfying snapshot
 */
bool
pandora_search_version(PandoraTuplePosition tuplePosition, 
					   Relation relation, 
					   ...
					   IndexFetchHeapData *hscan)
{
	...
	// Assert(PGetPandoraRecordKey(p_locator)->seqnum ==
	// 	   tuplePosition->rec_key.seqnum);
	if (PGetPandoraRecordKey(p_locator)->seqnum !=
		tuplePosition->rec_key.seqnum)
	{
		fprintf(stderr, "[PANDORA] rel: %u, level: %u, fingerprint seqnum: %lu, record seqnum: %lu, pid: %d\n",
						relation->rd_id,
						tuplePosition->partitionLevel,
						tuplePosition->rec_key.seqnum,
						PGetPandoraRecordKey(p_locator)->seqnum,
						(int) getpid());
			...
			int options, BulkInsertState bistate)
{
...
#ifdef STAT_TPCC
	struct timespec stat_tpcc_breakdown_insert_before_get_buffer_start;
	struct timespec stat_tpcc_breakdown_insert_before_get_buffer_end;

	struct timespec stat_tpcc_breakdown_insert_get_buffer_start;
	struct timespec stat_tpcc_breakdown_insert_get_buffer_end;

	struct timespec stat_tpcc_breakdown_insert_after_get_buffer_start;
	struct timespec stat_tpcc_breakdown_insert_after_get_buffer_end;
#endif /* STAT_TPCC */
...
#ifdef STAT_TPCC
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_before_get_buffer_start);
#endif /* STAT_TPCC */
...
#ifdef PANDORA
	heaptup->t_pandora_fingerprint = NULL;
#endif /* PANDORA */
	...
	if (tupDesc->maxlen < MAXALIGN(heaptup->t_len))
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_before_get_buffer_end);
	...
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_get_buffer_start);
#endif /* STAT_TPCC */
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_get_buffer_end);
	...
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_after_get_buffer_start);
#endif /* STAT_TPCC */
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_after_get_buffer_end);
					...
					bool wait, TM_FailureData *tmfd, bool changingPart)
{
	...
	/* Set pointers */
	l_pointer = (ItemPointer)(p_locator + PITEM_PTR_OFF);
	r_pointer = (ItemPointer)(p_locator + PITEM_PTR_OFF +
...
#endif /* PANDORA */
{
	...
	bool isPandora = IsPandora(relation);
	PandoraRelationMeta *meta = NULL;
	int		mempart_id;
	...
	if (isPandora)
	{
		...
		meta = PandoraGetRelationMeta(relation->rd_node.relNode);
	}
#endif /* PANDORA */
...
#ifdef PANDORA
	if (isPandora && tuplePosition->partitionLevel == 0)
	{
		/* Get mempartition lock */
		mempart_id = PandoraGetMempartIdFromBufId(buffer - 1);
		mempartition_lock = PandoraGetMempartLock(mempart_id);
...
#ifdef PANDORA
	Assert(!isPandora || (ItemPointerGetOffsetNumber(l_pointer) == offnum + 1));
#endif /* PANDORA */
...
#ifdef PANDORA
	Assert(!isPandora || (ItemPointerGetOffsetNumber(r_pointer) == offnum + 2));
#endif /* PANDORA */
...
l2:
	...
	/*
	 * In PANDORA, Since the tid passed as a parameter is the tid of the
	...
	else if (result == TM_BeingModified && wait)
	{
		...
		else
		{
			...
			heap_acquire_tuplock(relation, otid, *lockmode,
								 LockWaitBlock, &have_tuple_lock);
			XactLockTableWait(xwait, relation, otid,
							  XLTW_Update);
			checked_lockers = true;
			LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
...
#endif
		if (have_tuple_lock)
			UnlockTupleTuplock(relation, otid, *lockmode);
		if (vmbuffer != InvalidBuffer)
			ReleaseBuffer(vmbuffer);
		bms_free(hot_attrs);
		bms_free(key_attrs);
...
#ifdef PANDORA
		if (isPandora && tuplePosition->needDecrRefCount)
		{
			PandoraPartLevel partLvl = tuplePosition->partitionLevel;
			PandoraPartNumber partNum = (partLvl == 0) ? 0 :
											tuplePosition->partitionNumber;

			/* Decrement modification count. */
			DecrModificationCount(
				PandoraRelationMetaRefCounter(meta, partLvl, partNum), 
	...
	{
		targettup = &l_loctup;
		target_maxlen = (uint16*)(p_locator + PITEM_HDR_SZ);
		target_lp = l_lp;
		targetbuf = l_buffer;
	}
	else
	{
		targettup = &r_loctup;
		target_maxlen = (uint16*)(p_locator + PITEM_HDR_SZ + sizeof(uint16));
		target_lp = r_lp;
		targetbuf = r_buffer;
	}
	...
	{
#if 1
		elog(ERROR, "TODO: TOAST have to be implemented");
#else
...
#ifndef USING_LOCK
	/*
	 * In PANDORA, the share lock of buffer isn't acquired during SeqScan, so
	...
	/* In-place update, or append update on the heap page */
	is_appended_update = false;
	...
	if (isPandora && tuplePosition->needDecrRefCount)
	{
		...
		if (tuplePosition->modificationMethod == MODIFICATION_WITH_DELTA &&
				!CheckNonPartitioningInLevelZero(meta, 
												tuplePosition->partitionLevel, 
												tuplePosition->partitionNumber))
		{
			bool isLevelZeroOne = (partLvl <= 1 ? true : false);

			/* Add delta to dsa space. */
			AppendPandoraDelta(pandoraPartitionDsaArea, meta, 
												tuplePosition, isLevelZeroOne);
		}
...
#if 0
/*
 * pandoraam implementation of tableam's index_delete_tuples interface.
 *
 * This helper function is called by index AMs during index tuple deletion.
 * See tableam header comments for an explanation of the interface implemented
 * here and a general theory of operation.  Note that each call here is either
 ...
 */
TransactionId
pandora_index_delete_tuples(Relation rel, TM_IndexDeleteOp *delstate)
{
	/* Initial assumption is that earlier pruning took care of conflict */
	TransactionId latestRemovedXid = InvalidTransactionId;
	BlockNumber blkno = InvalidBlockNumber;
...
#endif
/*
 *	pandora_insert		- insert tuple into a mempartition
 *
 * The new tuple is stamped with current transaction ID and the specified
 * command ID.
 *
 ...
 */
void
pandora_insert(Relation relation, HeapTuple tup, CommandId cid,
			   int options, BulkInsertState bistate)
{
	TransactionId xid = GetCurrentTransactionId();
	HeapTuple	heaptup;
	...
	uint64					recCnt;
	PandoraPartNumber		partNum;
	PandoraFingerprint		pandora_fingerprint;
	PandoraRelationMeta	   *pandora_meta;

	LWLock				   *meta_mempartition_lock;
	uint32					max_recCnt;
	...
	struct timespec stat_tpcc_breakdown_insert_read_buffer_before_start;
	struct timespec stat_tpcc_breakdown_insert_read_buffer_before_end;

	struct timespec stat_tpcc_breakdown_insert_read_buffer_start;
	struct timespec stat_tpcc_breakdown_insert_read_buffer_end;

	struct timespec stat_tpcc_breakdown_insert_read_buffer_after_start;
	struct timespec stat_tpcc_breakdown_insert_read_buffer_after_end;

	struct timespec stat_tpcc_breakdown_insert_buffer_lock_start;
	struct timespec stat_tpcc_breakdown_insert_buffer_lock_end;

	struct timespec stat_tpcc_breakdown_insert_record_write_start;
	struct timespec stat_tpcc_breakdown_insert_record_write_end;

	struct timespec stat_tpcc_breakdown_insert_LOG_start;
	struct timespec stat_tpcc_breakdown_insert_LOG_end;

	struct timespec stat_tpcc_breakdown_insert_pandora_meta_mem_lock_start;
	struct timespec stat_tpcc_breakdown_insert_pandora_meta_mem_lock_end;

	struct timespec stat_tpcc_breakdown_insert_pandora_mempart_lock_start;
	struct timespec stat_tpcc_breakdown_insert_pandora_mempart_lock_end;
#endif /* STAT_TPCC */

#ifdef STAT_TPCC
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_read_buffer_before_start);
#endif /* STAT_TPCC */

	/* Init values */
	curLvl = relation->rd_pandora_level_count - 2; /* last internal level */
	pandora_fingerprint = tup->t_pandora_fingerprint;
	partid = pandora_fingerprint->rec_key.partid;
	pandora_meta = PandoraGetRelationMeta(RelationGetRelid(relation));
	meta_mempartition_lock =
		PandoraRelationMetaMemPartitionLock(pandora_meta);
	max_recCnt = PandoraMempartPageCount * relation->records_per_block;
	 ...
	 */
	heaptup = heap_prepare_insert(relation, tup, xid, cid, options);
	heaptup->t_pandora_fingerprint = tup->t_pandora_fingerprint;

	Assert(IsSiro(relation));
	Assert(tupDesc->maxlen >= MAXALIGN(heaptup->t_len));
	...
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_pandora_meta_mem_lock_start);
#endif /* STAT_TPCC */

	/* Acquire the lock of level 0 to read and write meta data */
	LWLockAcquire(meta_mempartition_lock, LW_EXCLUSIVE);

#ifdef STAT_TPCC
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_pandora_meta_mem_lock_end);
		...
		stat_tpcc_breakdown_insert_pandora_meta_mem_lock
			+= stat_tpcc_elapsed_time;
	}
#endif /* STAT_TPCC */

	/* Issue the insertion sequence number */
	seqNum = IncrementSequenceNumber(pandora_meta);
	pandora_fingerprint->rec_key.seqnum = seqNum;
	// fprintf(stderr, "[PANDORA] pandora_insert, relid: %u, seqnum: %lu\n", relation->rd_id, seqNum);
	/* Get the position of new record in level 0 */
	partNum = pandora_meta->partitionNumberLevelZero[1];
	recCnt = pandora_meta->recordCountsLevelZero[0];
	offnum = 3 * (recCnt % relation->records_per_block) + 1;

	Assert(recCnt < max_recCnt);

	/* Get the block number and do cumulative counting of level 0 */
	if (unlikely(recCnt == 0)) /* This is the first record */
	{
		/* Create new mempartition file and set the first seqnum */
		targetBlock = P_NEW;
		pandora_meta->recordCountsLevelZero[0]++;
	}
	else if (unlikely(recCnt == (max_recCnt - 1))) /* This is the last record */
	{
		/* Init info for next insertion */
		targetBlock = (recCnt / relation->records_per_block);
		pandora_meta->partitionNumberLevelZero[1]++;
		pandora_meta->recordCountsLevelZero[1] +=
			(pandora_meta->recordCountsLevelZero[0] + 1);
		pandora_meta->recordCountsLevelZero[0] = 0;
	}
	else
	{
		/* Get targetBlock */
	...
	if ((recCnt % relation->records_per_block) == 0)
		IncrBlockCount(pandora_meta, 1);

	/* Set position info of level 0 */
	pandora_fingerprint->rec_pos.first_lvl_partnum = partNum;
	pandora_fingerprint->rec_pos.first_lvl_pos = recCnt;
	Assert(seqNum <= partNum * relation->records_per_block * PandoraMempartPageCount + recCnt);

	/* Cumulative counting of last level */
	pandora_fingerprint->rec_pos.last_lvl_pos = (pandora_meta->lastCumulativeCount[partid])++;
	
	/* Cumulative counting of internal levels */
	for (partid /= relation->rd_pandora_spread_factor;
		 curLvl > 0;
		 curLvl--, partid /= relation->rd_pandora_spread_factor)
	{
		pandora_fingerprint->rec_pos.in_partition_counts[curLvl - 1].cumulative_count =
			(pandora_meta->internalCumulativeCount[curLvl][partid].cumulative_count)++;
	}

	LWLockRelease(meta_mempartition_lock);

#ifdef STAT_TPCC
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_read_buffer_before_end);
		...
		stat_tpcc_breakdown_insert_read_buffer_before
			+= stat_tpcc_elapsed_time;

		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_read_buffer_start);
	}
#endif /* STAT_TPCC */

	/* Get the partition buffer */
	buffer = PandoraReadBufferForInsert(relation, partNum, targetBlock);

#ifdef STAT_TPCC
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_read_buffer_end);
		...
		stat_tpcc_breakdown_insert_read_buffer
			+= stat_tpcc_elapsed_time;

		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_read_buffer_after_start);
	}
#endif /* STAT_TPCC */
	...
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_pandora_mempart_lock_start);
#endif /* STAT_TPCC */
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_pandora_mempart_lock_end);
		...
		stat_tpcc_breakdown_insert_pandora_mempart_lock
			+= stat_tpcc_elapsed_time;
	}
#endif /* STAT_TPCC */
	...
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_record_write_start);
#endif /* STAT_TPCC */
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_record_write_end);
		...
		stat_tpcc_breakdown_insert_record_write
			+= stat_tpcc_elapsed_time;
	}
#endif /* STAT_TPCC */
	...
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_LOG_start);
#endif /* STAT_TPCC */
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_LOG_end);
		...
		stat_tpcc_breakdown_insert_LOG
			+= stat_tpcc_elapsed_time;
	}
#endif /* STAT_TPCC */
...
#ifdef PANDORA_DEBUG
	if (GetInsertedRecordsCount(buffer) == relation->records_per_block * PandoraMempartPageCount)
		fprintf(stderr, "[PANDORA] incr, rel: %u, cnt: %u, buf: %p, pid: %d\n",
						relation->rd_id,
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_read_buffer_after_end);
		...
		stat_tpcc_breakdown_insert_read_buffer_after
			+= stat_tpcc_elapsed_time;
	}
#endif /* STAT_TPCC */
}
...
/* ----------------
 *		initpandorascan - scan code common to pandora_beginscan and
 *						  pandora_rescan
 * ----------------
 */
static void
initpandorascan(PandoraScanDesc pscan)
{
	// ParallelBlockTableScanDesc bpscan = NULL;
	HeapScanDesc scan = (HeapScanDesc) pscan;
	...
	ListCell *lc;
	PandoraPartLevel last_level;
	PandoraPartNumber part_num;
	PandoraPartNumber part_num_prev;
	PandoraPartNumber part_num_curr;
	PandoraPartInfo part_infos;
	PandoraPartInfo part_info;
	uint64 rec_cnt;
	uint64 level_zero_rec_cnt;
	int level_zero_part_cnt;

#ifdef PANDORA_MEASURE
	fprintf(stderr, "[PANDORA] relation %s scan\n", 
		RelationGetRelationName(relation));
#endif
	...
	/* Set initial values */
	meta = PandoraGetRelationMeta(relation->rd_id);
	metaMempartitionLock = PandoraRelationMetaMemPartitionLock(meta);
	last_level = relation->rd_pandora_level_count - 1;
	
	oldcxt = MemoryContextSwitchTo(CurTransactionContext);

	/* Init level 0 */
	if (pscan->part_infos_to_scan[0] != NIL)
	{
		pfree(linitial(pscan->part_infos_to_scan[0]));
		list_free(pscan->part_infos_to_scan[0]);
		pscan->part_infos_to_scan[0] = NIL;
	}

	/* Acquire lock, and increment refcounts */
	LWLockAcquire(metaMempartitionLock, LW_SHARED);
	for (int i = 1; i < relation->rd_pandora_level_count; ++i)
	{
		foreach(lc, pscan->part_infos_to_scan[i])
		{
			part_num = ((PandoraPartInfo)lfirst(lc))->partNum;
			IncrMetaDataCount(PandoraRelationMetaRefCounter(meta, i, part_num));
		}
	}

	/* Get scan range of level 0 */
	part_num_prev = meta->partitionNumberLevelZero[0];
	part_num_curr = meta->partitionNumberLevelZero[1];
	level_zero_rec_cnt = meta->recordCountsLevelZero[0];
	...
	for (int i = 1; i < relation->rd_pandora_level_count; ++i)
	{
		foreach(lc, pscan->part_infos_to_scan[i])
		{
			...
			part_info = ((PandoraPartInfo)lfirst(lc));
			part_num = part_info->partNum;

			/* Last level only has generation number 0 */
			part_info->partGen = i == last_level ?
								 0 : meta->generationNums[i][part_num];

			/* Get whole count temporary */
			part_info->last_recCnt = meta->realRecordsCount[i][part_num];
	...
	for (int i = relation->rd_pandora_level_count - 1; i > 0; --i)
	{
		foreach(lc, pscan->part_infos_to_scan[i])
		{
			part_num = ((PandoraPartInfo)lfirst(lc))->partNum;
			DecrMetaDataCount(PandoraRelationMetaRefCounter(meta, i, part_num));
		}
	}
	LWLockRelease(metaMempartitionLock);

	/* Get the count of mempartition */
	level_zero_part_cnt = level_zero_rec_cnt == 0 ?
						  part_num_curr - part_num_prev :
						  part_num_curr - part_num_prev + 1;
	
	if (likely(level_zero_part_cnt != 0))
	{
		/* Init array of PandoraPartInfoData */
		part_infos = palloc0_array(PandoraPartInfoData, level_zero_part_cnt);
		part_info = part_infos;

		/* Set scan range of level 0 (in-memory) */
		if (level_zero_rec_cnt != 0)
		{
			/* Set partition number */
			part_info->partNum = part_num_curr;

			/* First level only has generation number 0 */
			part_info->partGen = 0;

			/* Get block count */
			part_info->block_cnt =
				NBlocksUsingRecordCount(level_zero_rec_cnt, relation->records_per_block);
			
			/*
			 * The most recent mempartition may have records stored in a
			 * different chronological order than the insert order.
			 * Therefore, mark it as 0 and then use the offset of the page
			 * header during pandoragetpage() to get the correct count.
			 */
			part_info->last_recCnt = 0;

#ifdef PANDORA_DEBUG
			part_info->recCnt = level_zero_rec_cnt;
#endif /* PANDORA_DEBUG */

			/* Append this newest in-memory partition into list */
			pscan->part_infos_to_scan[0] =
					lappend(pscan->part_infos_to_scan[0], part_info);

			/* Add block count */
			nblocks += (part_info++)->block_cnt;
		}

		/* Set scan range of level 0 (tiered) */
		for (part_num = part_num_curr - 1;
			 (part_num - part_num_prev) != InvalidPandoraPartNumber;
			 --part_num, ++part_info)
		{
			/* Set partition nubmer */
			part_info->partNum = part_num;

			/* First level only has generation number 0 */
			part_info->partGen = 0;

			/* Get block count by checking file size */
			part_info->block_cnt = PandoraMempartPageCount;
...
#if defined(PANDORA_DEBUG) || defined(PANDORA_MEASURE)
		fprintf(stderr, "level: 0 (nblocks: %u)\n", nblocks);
#endif

		/* Set starting block */
		pscan->c_lc = list_head(pscan->part_infos_to_scan[0]);
		pscan->c_part_info = part_infos;
		pscan->c_level = 0;
	}
	else
		pscan->c_lc = NULL;
	...
	for (int i = 1; i < relation->rd_pandora_level_count; ++i)
	{
		...
		foreach(lc, pscan->part_infos_to_scan[i])
		{
			...
			if ((rec_cnt = part_info->last_recCnt) != 0)
			{
				...
				part_info->block_cnt =
					NBlocksUsingRecordCount(rec_cnt, relation->records_per_block);
				...
				/* Set starting block */
				if (unlikely(pscan->c_lc == NULL))
				{
					pscan->c_lc = lc;
					pscan->c_part_info = part_info;
					pscan->c_level = i;
				}
			}
			else
...
#ifdef PANDORA_DEBUG
	pscan->c_scanned_records = 0;
	pscan->c_valid_versions = 0;
#endif /* PANDORA_DEBUG */
...
/* ----------------------------------------------------------------
 *					 pandora access method interface
 * ----------------------------------------------------------------
 */
TableScanDesc
pandora_beginscan(Relation relation, Snapshot snapshot,
				  int nkeys, ScanKey key,
				  ParallelTableScanDesc parallel_scan,
				  uint32 flags,
				  List *hap_partid_list)
{
	PandoraScanDesc pscan;
	HeapScanDesc scan;
	MemoryContext oldcxt;
	...
	int level_cnt;
	PandoraPartLevel max_level;
	PandoraPartInfo part_infos[32];
	PandoraPartInfo part_info[32];

	PandoraPartNumber prev_part_nums[32];
	PandoraPartNumber curr_part_num;
	int pow_len, max_len;
	int16 spread_factor = relation->rd_pandora_spread_factor;
	 ...
	 * allocate and initialize scan descriptor
	 */
	pscan = (PandoraScanDesc) palloc(sizeof(PandoraScanDescData));
	scan = (HeapScanDesc) pscan;
	...
	/*
	 * we do this here instead of in initpandorascan() because heap_rescan also
	 * calls initpandorascan() and we don't want to allocate memory again
	 */
	if (nkeys > 0)
		scan->rs_base.rs_key = (ScanKey) palloc(sizeof(ScanKeyData) * nkeys);
	else
	...
	/* Init variables */
	level_cnt = relation->rd_pandora_level_count;
	max_level = level_cnt - 1;
	part_infos_to_scan = palloc0_array(List*, level_cnt);
	MemSet(prev_part_nums, -1, sizeof(PandoraPartNumber) * level_cnt);

	/* Target of SeqScan is passed */
	if (hap_partid_list != NULL)
	{
		/* Level 0 will be set during initpandorascan() */
		max_len = list_length(hap_partid_list);
		for (int i = max_level; i > 0; --i)
		{
			/* Get smaller max length */
			if ((pow_len = (int)pow(spread_factor, i)) < max_len)
				max_len = pow_len;

			/* Alloc the space of PandoraPartInfoData and set curr_pointer */
			part_infos[i] = palloc0_array(PandoraPartInfoData, max_len);
	...
	{
		/* Level 0 will be set during initpandorascan() */
		max_len = relation->rd_pandora_part_count;
		for (int i = max_level; i > 0; --i)
		{
			/* Get smaller max length */
			if ((pow_len = (int)pow(spread_factor, i)) < max_len)
		...
		/* Set partition numbers of each level that should be scanned */
		for (PandoraPartNumber curr = 0;
			 curr < relation->rd_pandora_part_count;
			 ++curr)
		{
			/* Get partid */
			curr_part_num = curr;
	...
	pscan->part_infos_to_scan = part_infos_to_scan;
	pscan->c_part_info = NULL;

	MemoryContextSwitchTo(oldcxt);

	initpandorascan(pscan);
	...
	if (enable_prefetch && is_prefetching_needed(relation, scan))
	{
		...
		partition_desc = 
				(PrefetchPartitionDesc) palloc0(sizeof(struct PrefetchPartitionDescData));
		prefetch_desc = (PrefetchDesc)partition_desc;

		/* Copy pandora scan state */
		partition_desc->level_cnt = relation->rd_pandora_level_count;
 		partition_desc->part_infos_to_prefetch = pscan->part_infos_to_scan;
 		partition_desc->c_level = pscan->c_level;
 		partition_desc->c_lc = pscan->c_lc;
 		partition_desc->c_block_cnt = pscan->c_part_info->block_cnt;
				
		/* Get file name */
		part_info = pscan->c_part_info;
		...
		if ((partition_desc->c_fd = PandoraSimpleOpen(path, O_RDWR | O_DIRECT)) == -1)
		{
		...
		pfree(path);
 		...
 		prefetch_desc->next_reqblock = pscan->c_block;
 		prefetch_desc->prefetch_end = false;
 		prefetch_desc->requested_io_num = 0;
 		prefetch_desc->complete_io_num = 0;
...
void
pandora_rescan(TableScanDesc sscan, ScanKey key, bool set_params,
			   bool allow_strat, bool allow_sync, bool allow_pagemode)
{
	PandoraScanDesc pscan = (PandoraScanDesc) sscan;
	HeapScanDesc scan = (HeapScanDesc) sscan;
	PrefetchDesc prefetchDesc = sscan->rs_prefetch;
	PrefetchPartitionDesc partition_desc;
	char *path;
	...
	if (part_info != NULL)
	{
		fprintf(stderr, "[SeqScan] level: %u, partNum: %u, partGen: %u, recCnt: %u, scanned records: %u, valid versions: %u\n",
						pscan->c_level, part_info->partNum, part_info->partGen,
						part_info->recCnt, pscan->c_scanned_records, pscan->c_valid_versions);
	}
#endif /* PANDORA_DEBUG */
	...
	if (prefetchDesc)
	{
		...
		/* Close previous file */
		if (PandoraSimpleClose(partition_desc->c_fd) == -1)
		{
			/* For error log */
			part_info = pscan->c_part_info;
			path = psprintf("base/%u/%u.%u.%u.%u",
	...
	{ 
		/* Copy pandora scan state */
		partition_desc->level_cnt = sscan->rs_rd->rd_pandora_level_count;
 		partition_desc->part_infos_to_prefetch = pscan->part_infos_to_scan;
 		partition_desc->c_level = pscan->c_level;
 		partition_desc->c_lc = pscan->c_lc;
 		partition_desc->c_block_cnt = pscan->c_part_info->block_cnt;
...
void
pandora_endscan(TableScanDesc sscan)
{
	PandoraScanDesc pscan = (PandoraScanDesc) sscan;
	HeapScanDesc scan = (HeapScanDesc) sscan;
	MemoryContext oldcxt;
	...
	for (int i = 1; i < sscan->rs_rd->rd_pandora_level_count; i++)
	{
		pfree(linitial(pscan->part_infos_to_scan[i]));
		list_free(pscan->part_infos_to_scan[i]);
	}

	/* Free part_info list*/
	pfree(pscan->part_infos_to_scan);
...
/*
 * pandorasetpage - subroutine for pandoragettup()
 *
 * This routine checks and sets the page that should be scanned at this time,
 * out of the scope stored in PandoraScanDesc.
 */
static void
pandorasetpage(PandoraScanDesc pscan)
{
	List *c_list;
	ListCell *c_lc;
	PandoraPartInfo c_part_info;
	int c_level = pscan->c_level;

	/*
	 * If the current partition hasn't been entirely scanned, continue to
	 * scan the rest of the partition.
	 */
	if (likely((++(pscan->c_block)) < pscan->c_part_info->block_cnt))
		return;

#ifdef PANDORA_DEBUG
	c_part_info = pscan->c_part_info;
	fprintf(stderr, "[SeqScan] level: %u, partNum: %u, partGen: %u, recCnt: %u, scanned records: %u, valid versions: %u\n",
					c_level, c_part_info->partNum, c_part_info->partGen,
					c_part_info->recCnt, pscan->c_scanned_records, pscan->c_valid_versions);
	...
	c_list = pscan->part_infos_to_scan[c_level];

	if ((c_lc = lnext(c_list, pscan->c_lc)) == NULL)
	{
		c_list = pscan->part_infos_to_scan[++c_level];
	...
	/* Store the variables */
	pscan->c_lc = c_lc;
	pscan->c_part_info = c_part_info;
	pscan->c_level = c_level;
	pscan->c_block = 0;
}

/*
 * pandoragetpage - subroutine for pandoragettup()
 *
 * This routine reads and pins the specified page of the relation.
 * In page-at-a-time mode it performs additional work, namely determining
 * which tuples on the page are visible.
 ...
 */
void
pandoragetpage(TableScanDesc sscan)
{
	PandoraScanDesc pscan = (PandoraScanDesc) sscan;
	HeapScanDesc scan = (HeapScanDesc) sscan;

	PandoraPartInfo		part_info;
	PandoraPartLevel	part_level;
	PandoraPartNumber	part_number;
	PandoraPartGenNo	part_gen;
	uint32				rec_cnt;
	...
	c_block = pscan->c_block;
	part_info = pscan->c_part_info;
	part_level = pscan->c_level;
...
#ifdef PANDORA_DEBUG
	pscan->c_scanned_records += rec_cnt;
#endif /* PANDORA_DEBUG */
	 ...
	 * mempartition at level 0. (See initpandorascan())
...
failed:
	...
	if ((lines / 3) != valid_records)
		fprintf(stderr, "[pandoragetpage] level: %u, partNum: %u, partGen: %u, block: %u, recCnt: %u, valid versions: %u\n",
...
/* ----------------
 *		pandoragettup - fetch next heap tuple from PANDORA partition
 *
 *		Initialize the scan if not already done; then advance to the next
 *		tuple as indicated by "dir"; return the next tuple in scan->rs_ctup with
 * 		scan->rs_ctup_copied, or set scan->rs_ctup.t_data = NULL if no more
 ...
 */
static void
pandoragettup(PandoraScanDesc pscan,
			  ScanDirection dir,
			  int nkeys,
			  ScanKey key)
{
	...
	if (!scan->rs_inited)
	{
		...
		pandoragetpage((TableScanDesc) pscan);
		lineoff = FirstOffsetNumber;	/* first offnum */
		scan->rs_inited = true;
	}
	else
	...
	c_block = pscan->c_block; /* current page */
	part_info = pscan->c_part_info;
	part_level = pscan->c_level;
	part_number = part_info->partNum;
	part_gen = part_info->partGen;
	 ...
	 * So, use the max offset of the page to get record count.
	 */
	lines = pscan->c_rec_cnt == 0 ? PageGetMaxOffsetNumber(dp) :
									3 * pscan->c_rec_cnt;
	/* page and lineoff now reference the physically next tid */
...
failed:
		...
		{
			/* PANDORA uses rs_cblock only to determine if scan is finished */
		...
		pandorasetpage(pscan);
		pandoragetpage((TableScanDesc) scan);
...
/* ----------------
 *		pandoragettup_pagemode - fetch next heap tuple from pandora partition
 *								 in page-at-a-time mode
 *
 *		Same API as pandoragettup, but used in page-at-a-time mode
 *
 * The internal logic is much the same as pandoragettup's too, but there are
 * some differences: we do not take the buffer content lock (that only needs to
 * happen inside pandoragetpage), and we iterate through just the tuples listed
 * in rs_vistuples[] rather than all tuples on the page.  Notice that
 * lineindex is 0-based, where the corresponding loop variable lineoff in
 * heapgettup is 1-based.
 * ----------------
 */
static void
pandoragettup_pagemode(PandoraScanDesc pscan,
					   ScanDirection dir,
					   int nkeys,
					   ScanKey key)
{
...
HeapTuple
pandora_getnext(TableScanDesc sscan, ScanDirection direction)
{
	PandoraScanDesc pscan = (PandoraScanDesc) sscan;
	HeapScanDesc scan = (HeapScanDesc) sscan;
	bool pagemode = (scan->rs_base.rs_flags & SO_ALLOW_PAGEMODE);
	...
	/*
	 * We don't expect direct calls to pandora_getnext with valid CheckXidAlive
	 * for catalog or regular tables.  See detailed comments in xact.c where
	 * these variables are declared.  Normally we have such a check at tableam
	 * level API but this is called from many places so we need to ensure it
	 * here.
	...
	if (pagemode)
		pandoragettup_pagemode(pscan, direction,
				   			   scan->rs_base.rs_nkeys, scan->rs_base.rs_key);
	else
		pandoragettup(pscan, direction,
					  scan->rs_base.rs_nkeys, scan->rs_base.rs_key);
...
bool
pandora_getnextslot(TableScanDesc sscan, ScanDirection direction,
					TupleTableSlot *slot)
{
	PandoraScanDesc pscan = (PandoraScanDesc) sscan;
	HeapScanDesc scan = (HeapScanDesc) pscan;
