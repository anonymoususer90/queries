				...
				int nkeys, int norderbys)
{
	...
	if (scan->xs_heapfetch && IsPandora(scan->xs_heapfetch->rel))
		scan->lastPartitionLevel = 
			PandoraGetLevelCount(scan->xs_heapfetch->rel->rd_node.relNode) - 1;
	else
		scan->lastPartitionLevel = -1;
#endif
...
//#ifdef PANDORA
//	if (!(scan->heapRelation->rd_pandora_partitioning))
//#endif /* PANDORA */
...
#ifdef PANDORA
/* ----------------
 * index_getnext_fingerprint - get the next fingerprint from a scan
 *
 * The result is the next fingerprint satisfying the scan keys,
 * or NULL if no more matching tuples exist.
 * ----------------
 */
PandoraFingerprint
index_getnext_fingerprint(IndexScanDesc scan, ScanDirection direction)
{
	bool		found;
	...
	/* Return the fingerprint of the tuple we found. */
	return scan->xs_fingerprint;
}
#endif 
...
{
#ifdef DIVA
	bool isSiro = IsSiro(scan->heapRelation);
#endif
#ifdef PANDORA
	...
	for (;;)
	{
		...
		if (!scan->xs_heap_continue)
		{
			if (isPandora)
			{
				fingerprint = index_getnext_fingerprint(scan, direction);
				
				if (fingerprint == NULL)
					break;

				slot->tts_pandora_fingerprint = fingerprint;
			}
			else
			{
				ItemPointer tid;
