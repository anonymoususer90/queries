...
#ifdef PANDORA
	Assert(!(rel->rd_pandora_partitioning) || (fingerprint != NULL));
...
table_block_relation_size(Relation rel, ForkNumber forkNumber)
{
	uint64		nblocks = 0;
 	...
 	if (forkNumber == InvalidForkNumber)
 	{
 		...
 		if (rel->rd_pandora_partitioning)
 		{
 			i = 1;
 			nblocks = PandoraGetNBlocks(rel);
 		}
 		else
 			i = 0;
 	...
 	{
 		if (rel->rd_pandora_partitioning && forkNumber == MAIN_FORKNUM)
 			nblocks = PandoraGetNBlocks(rel);
 		else
