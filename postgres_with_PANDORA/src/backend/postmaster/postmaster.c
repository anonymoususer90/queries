...
char statPageReadIODir[64];
#endif /* STAT_PAGE_READ_IO */

#ifdef STAT_IO_URING
char statIOUringDir[64];
#endif /* STAT_IO_URING */
...
#ifdef PANDORA
#define BACKEND_TYPE_PANDORA	BACKEND_TYPE_AUTOVAC
#endif /* PANDORA */
...
#endif
#ifdef PANDORA
			PandoraPartitionLauncherPID = 0,
			PandoraBgWriterPID = 0,
#endif
			WalReceiverPID = 0,
			AutoVacPID = 0,
			PgArchPID = 0,
...
#ifdef PANDORA
static void StartPandoraPartitioningWorker(void);
#endif /* PANDORA */
...
#endif
#ifdef PANDORA
#define StartPandoraBackgroundWriter() StartChildProcess(PandoraBgWriterProcess)
#endif
...
#ifdef PANDORA
static void postmaster_segfault_handler(int sig);
static void postmaster_abort_handler(int sig);
static pqsigfunc default_SIGSEGV;
static pqsigfunc default_SIGABRT;

static void
postmaster_segfault_handler(int sig)
{
	ereport(LOG, (errmsg("@@@@@ %d segfault, postmaster", getpid())));
	sleep(60);
	default_SIGSEGV(sig);
}

static void
postmaster_abort_handler(int sig)
{
	ereport(LOG, (errmsg("@@@@@ %d abort, postmaster", getpid())));
	sleep(60);
	default_SIGABRT(sig);
}
#endif /* PANDORA */
...
PostmasterMain(int argc, char *argv[])
{
...
#ifdef PANDORA
	default_SIGSEGV = pqsignal_pm(SIGSEGV, postmaster_segfault_handler);
	default_SIGABRT = pqsignal_pm(SIGABRT, postmaster_abort_handler);
#endif /* PANDORA */
...
#ifdef PANDORA
	if (PandoraBgWriterPID == 0)
		PandoraBgWriterPID = StartPandoraBackgroundWriter();
#endif
...
#ifdef STAT_PAGE_READ_IO
	/* mkdir */
	sprintf(statPageReadIODir, "../stat");
	if (mkdir(statPageReadIODir, 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s", statPageReadIODir));
	}

#ifdef PANDORA
	strcat(statPageReadIODir, "/pandora");
#else /* !PANDORA */
	strcat(statPageReadIODir, "/vanilla");
#endif /* PANDORA */
	if (mkdir(statPageReadIODir, 0777) == -1)
	{
		if (errno != EEXIST)
	...
	strcat(statPageReadIODir, "/olap_read_io");
	if (mkdir(statPageReadIODir, 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s", statPageReadIODir));
	...
	if (mkdir("../stat", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat"));
	}
#ifdef PANDORA
	if (mkdir("../stat/pandora", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat/pandora"));
	...
	if (mkdir("../stat/pandora/olap_read_io_breakdown", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat/pandora/olap_read_io_breakdown"));
	}

	if (mkdir("../stat/pandora/olap_read_io_breakdown/heap", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat/pandora/olap_read_io_breakdown/heap"));
	}

	if (mkdir("../stat/pandora/olap_read_io_breakdown/index", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat/pandora/olap_read_io_breakdown/index"));
	}
#else /* !PANDORA */
	if (mkdir("../stat/vanilla", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat/vanilla"));
	...
	if (mkdir("../stat/vanilla/olap_read_io_breakdown", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat/vanilla/olap_read_io_breakdown"));
	}

	if (mkdir("../stat/vanilla/olap_read_io_breakdown/heap", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat/vanilla/olap_read_io_breakdown/heap"));
	}

	if (mkdir("../stat/vanilla/olap_read_io_breakdown/index", 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s",
					"../stat/vanilla/olap_read_io_breakdown/index"));
	}
#endif /* PANDORA */
...
#ifdef STAT_IO_URING
	/* mkdir */
	sprintf(statIOUringDir, "../stat");
	if (mkdir(statIOUringDir, 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s", statIOUringDir));
	}

#ifdef PANDORA
	strcat(statIOUringDir, "/pandora");
#else /* !PANDORA */
	strcat(statIOUringDir, "/vanilla");
#endif /* PANDORA */
	if (mkdir(statIOUringDir, 0777) == -1)
	{
		if (errno != EEXIST)
	...
	strcat(statIOUringDir, "/io_uring");
	if (mkdir(statIOUringDir, 0777) == -1)
	{
		if (errno != EEXIST)
			ereport(FATAL, errmsg("stat path creation failed, path: %s", statIOUringDir));
...
#ifdef PANDORA
		if (PandoraPartitionLauncherPID == 0 && pmState == PM_RUN)
			PandoraPartitionLauncherPID = StartPandoraPartitionLauncher();
#endif
...
#endif
#ifdef PANDORA
		if (PandoraPartitionLauncherPID != 0)
			signal_child(PandoraPartitionLauncherPID, SIGHUP);
		if (PandoraBgWriterPID != 0)
			signal_child(PandoraBgWriterPID, SIGHUP);
#endif
		if (WalReceiverPID != 0)
...
#ifdef PANDORA
		/*
		 * Was it the pandora bgwriter?  Normal exit can be ignored; 
		 * we'll start a new  one at the next iteration of the postmaster's main 
		 * loop, if necessary.  Any other exit condition is treated as a crash.
		 */
		if (pid == PandoraBgWriterPID)
		{
			PandoraBgWriterPID = 0;
			if (!EXIT_STATUS_0(exitstatus))
				HandleChildCrash(pid, exitstatus,
								 _("pandora background writer process"));
			continue;
		}
#endif
...
#ifdef PANDORA
		/*
		 * Was it the pandora?  Normal exit can be ignored; we'll start a
		 * new one at the next iteration of the postmaster's main loop, if
		 * necessary.  Any other exit condition is treated as a crash.
		 */
		if (pid == PandoraPartitionLauncherPID)
		{
			PandoraPartitionLauncherPID = 0;
			if (!EXIT_STATUS_0(exitstatus))
				HandleChildCrash(pid, exitstatus,
								 _("Pandora partition launcher process"));
			continue;
		}
#endif
...
HandleChildCrash(int pid, int exitstatus, const char *procname)
{
	...
	if (pid == PandoraBgWriterPID)
		PandoraBgWriterPID = 0;
	else if (PandoraBgWriterPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process %d",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) PandoraBgWriterPID)));
		signal_child(PandoraBgWriterPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
#endif
...
#ifdef PANDORA
	/* Take care of the pandora too */
	if (pid == PandoraPartitionLauncherPID)
		PandoraPartitionLauncherPID = 0;
	else if (PandoraPartitionLauncherPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process %d",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) PandoraPartitionLauncherPID)));
		signal_child(PandoraPartitionLauncherPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
#endif
...
PostmasterStateMachine(void)
{
	...
	if (pmState == PM_STOP_BACKENDS)
	{
		...
		/* and the pandora too */
		if (PandoraPartitionLauncherPID != 0)
			signal_child(PandoraPartitionLauncherPID, SIGTERM);
		/* and the pandora bgwriter too */
		if (PandoraBgWriterPID != 0)
			signal_child(PandoraBgWriterPID, SIGTERM);
#endif
...
#endif
#ifdef PANDORA
			Assert(PandoraPartitionLauncherPID == 0);
			Assert(PandoraBgWriterPID == 0);
#endif
			Assert(AutoVacPID == 0);
...
#ifdef PANDORA
	if (PandoraPartitionLauncherPID != 0)
		signal_child(PandoraPartitionLauncherPID, signal);
	if (PandoraBgWriterPID != 0)
		signal_child(PandoraBgWriterPID, signal);
#endif
	if (WalReceiverPID != 0)
...
#ifdef PANDORA
	if (CheckPostmasterSignal(PMSIGNAL_PANDORA_PARTITIONING_WORKER) &&
		Shutdown <= SmartShutdown && pmState < PM_STOP_BACKENDS)
	{
		/*
		 * The pandora partitioning launcher wants us to start a worker process
		 */
		StartPandoraPartitioningWorker();
	}
#endif /* PANDORA */
...
#ifdef PANDORA
/*
 * Fork PANDORA partitioning worker.
 */
static void
StartPandoraPartitioningWorker(void)
{
	Backend    *bn;

	if (canAcceptConnections(BACKEND_TYPE_PANDORA) == CAC_OK)
	{
		/*
		 * Compute the cancel key that will be assigned to this session. We
		 * probably don't need cancel keys for autovac workers, but we'd
		...
		if (bn)
		{
			...
			bn->pid = StartPandoraPartitionWorker();
			if (bn->pid > 0)
			{
				bn->bkend_type = BACKEND_TYPE_PANDORA;
				dlist_push_head(&BackendList, &bn->elem);
#ifdef EXEC_BACKEND
				ShmemBackendArrayAdd(bn);
#endif
