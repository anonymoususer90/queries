...
{
	Datum		values[Natts_pg_hap_partition_map];
	bool		nulls[Natts_pg_hap_partition_map];
	HeapTuple	htup;
	...
	/* Make a pg_hap entry */
	values[Anum_pg_hap_partition_map_haprelid - 1]
		= ObjectIdGetDatum(relid);
	values[Anum_pg_hap_partition_map_happartmapid - 1]
		= ObjectIdGetDatum(partmapid);
	values[Anum_pg_hap_partition_map_happartmaphashid - 1]
		= ObjectIdGetDatum(hashid);
	values[Anum_pg_hap_partition_map_happartmapginid - 1]
