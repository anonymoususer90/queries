...
#endif
#ifdef PANDORA
	size = add_size(size, PandoraPartitioningShmemSize());
	size = add_size(size, PandoraRelationMetaShmemSize());
	size = add_size(size, NPandoraMempartCount * sizeof(LWLock));
#endif
#ifdef HAP
	size = add_size(size, HapSharedPartMapCacheSize());
	size = add_size(size, NUM_HAP_PARTMAP_CACHE_PARTITIONS * sizeof(LWLockPadded));
#endif /* HAP */
#ifdef EXEC_BACKEND
...
#ifdef HAP
	HapSharedPartMapCacheInit();
#endif /* HAP */

#ifdef PANDORA
	PandoraRelationMetaInit();
	PandoraPartitioningShmemInit();
#endif
