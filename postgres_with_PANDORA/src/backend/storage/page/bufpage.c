 ...
 *     Read pd_lower safely first, then calculate the max offset number
 */
							 ...
							 int flags)
{
...
#ifdef PANDORA
	/*
	 * In PANDORA, heap_insert() may not block SeqScan.
	 * If the new tuple is appended to the end of the correspoing heap page so
	 * that the previous tuples can be safely read by SeqScan, heap_insert will
	 * not block SeqScan.
	 * On the other hand, a tuple is being inserted in the middle, SeqScan must
	 * first be blocked to prevent it from reading the tuple being inserted, and
	 * the inserted.
	 */
	bool		need_block_reader;
	bool		insert_in_middle;
	pg_atomic_uint64 *dual_ref = (pg_atomic_uint64*)dref;
	ItemIdData	tmp_itemIdData = {0, 0, 0, 0, 0};
#endif /* PANDORA */
	...
	upper = (int) phdr->pd_upper - (int) maxlen * 2 - 
									(int) p_alignedSize;
...
#ifdef PANDORA
	/* If insertion point is not the end of tuples, block readers */
	if (need_block_reader)
	{
		/* Wait for all readers to pass by */
		if (DRefSetINSERT(dual_ref) != 0)
			DRefWaitForReaders(dual_ref, DRefPageRefCnt);
	}
#endif /* PANDORA */
	...
	if (needshuffle)
		memmove(itemId + 3, itemId,
				(limit - offsetNumber - 2) * sizeof(ItemIdData));

#ifdef PANDORA
	if (!insert_in_middle)
#endif /* PANDORA */
	{
		/* Set the line pointer for p-leaf meta tuple (p-locator) */
		ItemIdSetNormal(itemId, upper, PITEM_SZ);
		...
		ItemIdSetNormal(itemId, upper + (int)p_alignedSize, size);
		LP_OVR_SET_USING(itemId);

		/* Set the line pointer of right version */
		itemId = PageGetItemId(phdr, offsetNumber + 2);
		ItemIdSetNormal(itemId, upper + ((int)p_alignedSize + (int)maxlen), size);
		LP_OVR_SET_UNUSED(itemId);
	}
	...
	memset(p_pointer, 0x00, (Size) PITEM_SZ);

	/* Copy the item's data onto the page (not onto dummy) */
	p_pointer += (int) p_alignedSize;
	memcpy(p_pointer, item, size);

	/* Zero the right version */
	p_pointer += (int) maxlen;
	memset(p_pointer, 0x00, maxlen);

	/* Set info of p-locator (see architecture in bufpage.h) */
#ifdef PANDORA
	p_pointer = (char *) page + upper + PITEM_GEN_OFF;
	*((uint32*)p_pointer) = phdr->pd_gen;
	p_pointer += sizeof(uint32);
#else /* !PANDORA */
	p_pointer = (char *) page + upper + PITEM_HDR_SZ;
#endif /* PANDORA */
	*((uint16*)p_pointer) = maxlen;
	p_pointer += sizeof(uint16);
	*((uint16*)p_pointer) = maxlen;
	p_pointer += sizeof(uint16);
	ItemPointerSet((ItemPointer)p_pointer, blockNum, offsetNumber + 1);
	p_pointer += sizeof(ItemPointerData);
	ItemPointerSet((ItemPointer)p_pointer, blockNum, offsetNumber + 2);
 /* !PANDORA */
	/* adjust page header */
#ifdef PANDORA
	if (insert_in_middle)
	{
		...
		/* Set the line pointer for p-leaf meta tuple (p-locator) */
		itemId = PageGetItemId(phdr, offsetNumber);
		ItemIdSetNormal(&tmp_itemIdData, upper, PITEM_SZ);
		LP_OVR_SET_LEFT(&tmp_itemIdData);
		LP_OVR_SET_UNUSED(&tmp_itemIdData);
		LP_SET_PLEAF_FLAG(&tmp_itemIdData);
		LP_SET_ATOMIC(itemId, tmp_itemIdData);
	}

	/* Use atomic instruction to write pd_lower safely */
	__sync_lock_test_and_set(&(phdr->pd_lower), (LocationIndex)lower);
#else /* !PANDORA */
...
#ifdef PANDORA
OffsetNumber
PageAddItemPartitionExtended(Page page,
							 Item item,
							 Size size,
							 OffsetNumber offsetNumber,
							 BlockNumber blockNum,
							 ...
							 int flags)
{
	...
	/*
	 * In PANDORA, pandora_insert() may not block SeqScan.
	 * Because itemId is updated atomically in PANDORA when a new tuple is
	 * inserted into the mempartition, SeqScan can safely pass over new tuples
	 * that are being inserted concurrently.
	 */
	ItemIdData	tmp_itemIdData = {0, 0, 0, 0, 0};

	Assert(offsetNumber != InvalidOffsetNumber);
	Assert(isInit != NULL);
	...
	{
		elog(WARNING, "can't put more than MaxHeapTuplesPerPage items in a heap page");
		ereport(LOG, (errmsg("@@ corrupted page pointers: lower = %u, upper = %u, special = %u, offsetNumber = %u, MaxHeapTuplesPerPage = %u, size = %lu", phdr->pd_lower, phdr->pd_upper, phdr->pd_special, offsetNumber, MaxHeapTuplesPerPage, size)));
	 ...
	 * alignedSize > pd_upper.
	 */
	if ((offsetNumber + 2) > PageGetMaxOffsetNumber(page))
		lower = SizeOfPageHeaderData + sizeof(ItemIdData) * (offsetNumber + 2);
	else
		lower = phdr->pd_lower;
	...
	if (phdr->pd_upper == phdr->pd_special)
		*isInit = true;
	...
	*((uint16*)p_pointer) = maxlen;
	...
	p_pointer += sizeof(ItemPointerData);
	ItemPointerSet((ItemPointer)p_pointer, blockNum, offsetNumber + 2);
	if (pandora_fingerprint != NULL)
	{
		p_pointer += sizeof(ItemPointerData);
		*((PandoraRecordKey *)p_pointer) = pandora_fingerprint->rec_key;
	}
...
#ifdef PANDORA
OffsetNumber
PageAddItemWithFingerprintExtended(Page page,
								   Item item,
								   Size size,
								   OffsetNumber offsetNumber,
								   int flags,
								   ...
								   PandoraFingerprint pandora_fingerprint)
{
	...
	/* copy the item's data onto the page */
	memcpy((char *) page + upper, item, tup_size);
	memcpy((char *) page + upper + tup_size, pandora_fingerprint,
												(size - tup_size));

	/*
	fprintf(stderr, "[INSERT] partid %lu, seqnum %lu, first_lvl_partnum %lu, first_lvl_pos %lu, cumulative_counts[0] %lu, cumulative_counts[1] %lu, cumulative_counts[2] %lu\n",
				((PandoraFingerprint) ((char *) page + upper + tup_size))->rec_key.partid,
				((PandoraFingerprint) ((char *) page + upper + tup_size))->rec_key.seqnum,
				((PandoraFingerprint) ((char *) page + upper + tup_size))->rec_pos.first_lvl_partnum,
				((PandoraFingerprint) ((char *) page + upper + tup_size))->rec_pos.first_lvl_pos,
				((PandoraFingerprint) ((char *) page + upper + tup_size))->rec_pos.cumulative_counts[0],
				((PandoraFingerprint) ((char *) page + upper + tup_size))->rec_pos.cumulative_counts[1],
				((PandoraFingerprint) ((char *) page + upper + tup_size))->rec_pos.cumulative_counts[2]);
	*/
...
#ifdef PANDORA
		__sync_fetch_and_sub(&(((PageHeader) page)->pd_lower),
...
{
	return PageGetHeapFreeSpaceWithLP(page, FirstOffsetNumber);
}
/*
 * PageGetHeapFreeSpace
 *    Returns the size of the free (allocatable) space on a page,
