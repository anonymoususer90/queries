...
static int EbiWarmupRel(EbiNode node, Oid rel_id);
...
#ifdef PANDORA
		for (int j = 0;
			 j < pg_atomic_read_u32(&(subnode->old_partitions_counter));
			 ++j)
...
EbiWarmUp(HeapScanDesc scan)
{
	...
	/* If root's left boundary doesn't set yet, return immediately */
	if (!DsaPointerIsValid(curr->left_boundary)) return -1;
	...
	while ((curr != NULL))
	{
		...
		pages_read += EbiWarmupRel(curr, rel_id);
		++nodes_visited;
	...
	elog(LOG, "pages_read(EbiWarmUp): %d", pages_read);

	return pages_read;
}

static int 
EbiWarmupRel(EbiNode node, Oid rel_id)
{
	int page_id;
	EbiSubNode subnode;
	...
	for (page_id = 0; page_id < subnode->max_page_id; page_id++)
	{
		/* Just warm up */
		int buf_id = EbiSubBufGetBufRef(subnode->seg_id, subnode->rel_id,
					  ...
					  LWLock* rwlock)
{
	...
	/* 
	 * Write version to EbiSubNode. We sholud get a block to read a page.
...
EbiSiftAndBindPartitionOldFile(Oid db_id,
							   ...
							   TransactionId xmin,
							   TransactionId xmax,
							   PandoraPartLevel part_lvl,
							   PandoraPartNumber min_part_num,
							   PandoraPartNumber max_part_num,
							   PandoraPartGenNo part_gen)
{
	...
	Assert(index < PANDORA_MAX_OLD_FILES);

	/* Set old partitions */
	subnode->old_partitions[index].db_id = db_id;
	subnode->old_partitions[index].part_lvl = part_lvl;
	subnode->old_partitions[index].min_part_num = min_part_num;
	subnode->old_partitions[index].max_part_num = max_part_num;
	subnode->old_partitions[index].part_gen = part_gen;

#ifdef PANDORA_DEBUG
	fprintf(stderr, "[PANDORA] link old file for GC, rel: %u level: %u, minpart: %u, maxpart: %u, gen: %u\n",
					rel_id, part_lvl, min_part_num, max_part_num, part_gen);
#endif /* PANDORA_DEBUG */
