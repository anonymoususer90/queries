...
#ifdef PANDORA
	void		(*smgr_request) (SMgrRelation reln, ForkNumber forknum,
									BlockNumber blocknum, struct iovec *iovs,
									struct io_uring* ring, IoInfo info);
#endif
} f_smgr;
...
#ifdef PANDORA
/*
 *	smgrrequest() -- request range blocks from a relation
 */
void
smgrrequest(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
		 		 struct iovec *iovs, struct io_uring* ring, void* info)
{
	smgrsw[reln->smgr_which].smgr_request(reln, forknum, blocknum, 
														iovs, ring, info);
}
#endif
