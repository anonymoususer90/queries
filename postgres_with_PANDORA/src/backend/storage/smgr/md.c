...
{
#if defined(PANDORA) && defined(PANDORA_MD_ASSERT)
	Assert(forkNum != PANDORA_PARTITION_FORKNUM);
#endif /* PANDORA */
...
#ifdef PANDORA
/*
 *	mdrequest() -- Request the specified block from a relation using async i/o.
 */
void
mdrequest(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
	   			struct iovec *iovs, struct io_uring* ring, IoInfo io_info)
{
	off_t		seekpos;
	MdfdVec    *v;
	...
	FileRequest(ring, v->mdfd_vfd, iovs, seekpos, 
									io_info->page_io_num, io_info);
}
#endif
