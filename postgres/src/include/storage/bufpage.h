...
typedef struct PageHeaderData
{
	 ...
	 * In this case, when SeqScan checks how many tuples are on the page, there
	 * is a contention between SeqScan and heap_insert in pd_lower.
	 * So, pd_lower should be read and written using atomic instructions, and
	 * should have the volatile attribute.
	 */
	volatile LocationIndex pd_lower;		/* offset to start of free space */
#else
...
#ifdef PANDORA
	uint32		pd_gen;			/* to protect p-leaf array referenced through
								 * read buffer */
#endif /* PANDORA */
...
#ifdef PANDORA
#define PageGetItemNoCheck(page, itemId) \
( \
	(Item)(((char *)(page)) + ItemIdGetOffset(itemId)) \
)
#endif
...
/**
 * ----------------------------------------------------------------------------------------------------------------------------------------------
 *   8-byte   |    8-byte   |   4-byte  | 4-byte |   2-byte    |    2-byte    |    6-byte    |     6-byte    |     5-byte      |     3-byte     |
 * left-pleaf | right-pleaf | xid-bound | pd_gen | maxlen_left | maxlen_right | left_pointer | right_pointer | pandora_seqnum  | pandora_partid |
 * ----------------------------------------------------------------------------------------------------------------------------------------------
 * |------------------- header ------------------|                                                           |----------- record key -----------|
 * |----------------------------- pointer offset -----------------------------|
 */
...
#define PITEM_SZ (48)
#define PITEM_PANDORA_RECORDKEY_OFF (40)

#define PGetPandoraRecordKey(plocator) \
	((PandoraRecordKey *) (((char *) plocator) + PITEM_PANDORA_RECORDKEY_OFF))

#define PITEM_GEN_OFF (20)
#else /* !PANDORA */
/**
 * -----------------------------------------------------------------------------------------------------------
 *   8-byte   |    8-byte   |   4-byte  | 4-byte |   2-byte    |    2-byte    |    6-byte    |     6-byte    |
 * left-pleaf | right-pleaf | xid-bound | padded | maxlen_left | maxlen_right | left_pointer | right_pointer |
 * -----------------------------------------------------------------------------------------------------------
 * |------------------- header ------------------|
 * |----------------------------- pointer offset -----------------------------|
 */
...
#define PITEM_HDR_SZ (24)
#define PITEM_PTR_OFF (28)

#define PGetPointer(p_loc, test_var) \
	((ItemPointer) ((char *) p_loc + PITEM_PTR_OFF + (test_var - 1) * sizeof(ItemPointerData)))

#ifdef PANDORA
#define PageAddItemWithDummy(page, item, size, offsetNumber, \
							 reader_cnt, blockNum, maxlen, overwrite, is_heap) \
		PageAddItemExtendedWithDummy(page, item, size, offsetNumber, reader_cnt, \
									 blockNum, maxlen,\
									 ((overwrite) ? PAI_OVERWRITE : 0) | \
									 ((is_heap) ? PAI_IS_HEAP : 0))
#define PageAddItemPartition(page, item, size, offsetNumber, \
							 blockNum, maxlen, overwrite, is_heap, \
							 pandora_fingerprint, isInit) \
		PageAddItemPartitionExtended(page, item, size, offsetNumber, \
									 blockNum, maxlen, pandora_fingerprint, isInit, \
									 ((overwrite) ? PAI_OVERWRITE : 0) | \
									 ((is_heap) ? PAI_IS_HEAP : 0))
#else /* !PANDORA*/
#define PageAddItemWithDummy(page, item, size, offsetNumber, \
...
#ifdef PANDORA
#define PageAddItemWithFingerprint(page, item, size, offsetNumber, overwrite, is_heap, tup_size, pandora_fingerprint) \
	PageAddItemWithFingerprintExtended(page, item, size, offsetNumber, \
								   ((overwrite) ? PAI_OVERWRITE : 0) | \
								   ((is_heap) ? PAI_IS_HEAP : 0), tup_size, pandora_fingerprint)
#endif /* PANDORA */
...
extern OffsetNumber PageAddItemExtendedWithDummy(Page page, Item item,
												 ...
												 int flags);
#ifdef PANDORA
extern OffsetNumber PageAddItemPartitionExtended(Page page, Item item,
												 Size size,
												 OffsetNumber offsetNumber,
							 					 BlockNumber blockNum,
												 Size maxlen,
...
#ifdef PANDORA
extern OffsetNumber PageAddItemWithFingerprintExtended(Page page, Item item,
													   Size size,
													   OffsetNumber offsetNumber,
													   int flags,
													   Size tup_size,
