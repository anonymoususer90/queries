...
/* NEWORD */
static volatile uint64_t stat_tpcc_neword_cnt = 0;
static volatile uint64_t stat_tpcc_neword_total_time = 0;
static volatile uint64_t stat_tpcc_neword_max_time = 0;
static volatile uint64_t stat_tpcc_neword_min_time = UINT_MAX;

/* DELIVERY */
static volatile uint64_t stat_tpcc_delivery_cnt = 0;
static volatile uint64_t stat_tpcc_delivery_total_time = 0;
static volatile uint64_t stat_tpcc_delivery_max_time = 0;
static volatile uint64_t stat_tpcc_delivery_min_time = UINT_MAX;

/* PAYMENT */
static volatile uint64_t stat_tpcc_payment_cnt = 0;
static volatile uint64_t stat_tpcc_payment_total_time = 0;
static volatile uint64_t stat_tpcc_payment_max_time = 0;
static volatile uint64_t stat_tpcc_payment_min_time = UINT_MAX;

/* OSTAT */
static volatile uint64_t stat_tpcc_ostat_cnt = 0;
static volatile uint64_t stat_tpcc_ostat_total_time = 0;
static volatile uint64_t stat_tpcc_ostat_max_time = 0;
static volatile uint64_t stat_tpcc_ostat_min_time = UINT_MAX;

/* SLEV */
static volatile uint64_t stat_tpcc_slev_cnt = 0;
static volatile uint64_t stat_tpcc_slev_total_time = 0;
static volatile uint64_t stat_tpcc_slev_max_time = 0;
static volatile uint64_t stat_tpcc_slev_min_time = UINT_MAX;

/* INSERT (non-static variables), check "stat/stat_tpcc.h"  */
volatile uint64_t stat_tpcc_insert_cnt = 0;
volatile uint64_t stat_tpcc_insert_total_time = 0;
volatile uint64_t stat_tpcc_insert_max_time = 0;
volatile uint64_t stat_tpcc_insert_min_time = UINT_MAX;

/* UPDATE (non-static vairable), check "stat/stat_tpcc.h" */
volatile uint64_t stat_tpcc_update_cnt = 0;
volatile uint64_t stat_tpcc_update_total_time = 0;
volatile uint64_t stat_tpcc_update_max_time = 0;
volatile uint64_t stat_tpcc_update_min_time = UINT_MAX;

/* BREAKDOWN UPDATE */
volatile bool stat_tpcc_breakdown_update_exec_update_flag = false;
volatile uint64_t stat_tpcc_breakdown_update_cnt = 0;
volatile uint64_t stat_tpcc_breakdown_update_total = 0;
volatile uint64_t stat_tpcc_breakdown_update_ExecUpdateAct = 0;

/* BREAKDOWN INSERT */
volatile bool stat_tpcc_breakdown_insert_exec_insert_flag = false;
volatile uint64_t stat_tpcc_breakdown_insert_cnt = 0;
volatile uint64_t stat_tpcc_breakdown_insert_total = 0;
volatile uint64_t stat_tpcc_breakdown_insert_tuple_insert = 0;
volatile uint64_t stat_tpcc_breakdown_insert_read_buffer_before = 0;
volatile uint64_t stat_tpcc_breakdown_insert_read_buffer = 0;
volatile uint64_t stat_tpcc_breakdown_insert_read_buffer_after = 0;
volatile uint64_t stat_tpcc_breakdown_insert_buffer_lock = 0;
volatile uint64_t stat_tpcc_breakdown_insert_record_write = 0;
volatile uint64_t stat_tpcc_breakdown_insert_LOG = 0;
volatile uint64_t stat_tpcc_breakdown_insert_pandora_meta_mem_lock = 0;
volatile uint64_t stat_tpcc_breakdown_insert_pandora_mempart_lock = 0;
volatile uint64_t stat_tpcc_breakdown_insert_ExecInsertIndexTuples = 0;
volatile uint64_t stat_tpcc_breakdown_insert_HapGetPartIdHash = 0;
volatile uint64_t stat_tpcc_breakdown_insert_fkey_check = 0;
volatile uint64_t stat_tpcc_breakdown_insert_build_hidden = 0;
volatile uint64_t stat_tpcc_breakdown_insert_deconstruct_hidden = 0;
volatile uint64_t stat_tpcc_breakdown_insert_index_insert = 0;
volatile uint64_t stat_tpcc_breakdown_insert_heap_insert = 0;
volatile uint64_t stat_tpcc_breakdown_insert_heap_insert_before_get_buffer = 0;
volatile uint64_t stat_tpcc_breakdown_insert_heap_insert_get_buffer = 0;
volatile uint64_t stat_tpcc_breakdown_insert_heap_insert_after_get_buffer = 0;

/* Save results */
static uint64_t stat_tpcc_print_sec = 0;
static bool stat_tpcc_top_dir_exist = false;
static bool stat_tpcc_neword_dir_exist = false;
static bool stat_tpcc_delivery_dir_exist = false;
static bool stat_tpcc_payment_dir_exist = false;
static bool stat_tpcc_ostat_dir_exist = false;
static bool stat_tpcc_slev_dir_exist = false;
static bool stat_tpcc_insert_dir_exist = false;
static bool stat_tpcc_update_dir_exist = false;
static bool stat_tpcc_breakdown_update_dir_exist = false;
static bool stat_tpcc_breakdown_insert_dir_exist = false;

/* Make result directories */
static void
stat_tpcc_make_result_dir(void)
{
	if (!stat_tpcc_top_dir_exist)
	{
		mkdir("../stat", 0777);
#ifdef PANDORA
		mkdir("../stat/pandora", 0777);
		mkdir("../stat/pandora/tpcc", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla", 0777);
		mkdir("../stat/vanilla/tpcc", 0777);
#endif /* PANDORA */

		stat_tpcc_top_dir_exist = true;
	}

	if (!stat_tpcc_neword_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/neword", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/neword", 0777);
#endif /* PANDORA */

		stat_tpcc_neword_dir_exist = true;
	}

	if (!stat_tpcc_delivery_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/delivery", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/delivery", 0777);
#endif /* PANDORA */

		stat_tpcc_delivery_dir_exist = true;
	}

	if (!stat_tpcc_payment_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/payment", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/payment", 0777);
#endif /* PANDORA */

		stat_tpcc_payment_dir_exist = true;
	}

	if (!stat_tpcc_ostat_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/ostat", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/ostat", 0777);
#endif /* PANDORA */

		stat_tpcc_ostat_dir_exist = true;
	}

	if (!stat_tpcc_slev_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/slev", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/slev", 0777);
#endif /* PANDORA */

		stat_tpcc_slev_dir_exist = true;
	}

	if (!stat_tpcc_insert_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/insert", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/insert", 0777);
#endif /* PANDORA */

		stat_tpcc_insert_dir_exist = true;
	}

	if (!stat_tpcc_update_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/update", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/update", 0777);
#endif /* PANDORA */

		stat_tpcc_update_dir_exist = true;
	}

	if (!stat_tpcc_breakdown_update_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/breakdown_update", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/breakdown_update", 0777);
#endif /* PANDORA */

		stat_tpcc_breakdown_update_dir_exist = true;
	}

	if (!stat_tpcc_breakdown_insert_dir_exist)
	{
#ifdef PANDORA
		mkdir("../stat/pandora/tpcc/breakdown_insert", 0777);
#else /* !PANDORA */
		mkdir("../stat/vanilla/tpcc/breakdown_insert", 0777);
#endif /* PANDORA */

		stat_tpcc_breakdown_insert_dir_exist = true;
	}
}

/* Print neword transaction results */
static void
stat_tpcc_print_neword(time_t sec, int pid)
{
	char filename[256];
	FILE *fp;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/neword/neword.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/neword/neword.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp, "%lu %lu %lu %lu %lu\n",
			sec,
			stat_tpcc_neword_cnt,
			stat_tpcc_neword_total_time,
			stat_tpcc_neword_max_time,
			stat_tpcc_neword_min_time);

	fclose(fp);

	/* Initialize */
	stat_tpcc_neword_cnt = 0;
	stat_tpcc_neword_total_time = 0;
	stat_tpcc_neword_max_time = 0;
	stat_tpcc_neword_min_time = UINT_MAX;
}

/* Print delivery transaction results */
static void
stat_tpcc_print_delivery(time_t sec, int pid)
{
	char filename[256];
	FILE *fp;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/delivery/delivery.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/delivery/delivery.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp, "%lu %lu %lu %lu %lu\n",
			sec,
			stat_tpcc_delivery_cnt,
			stat_tpcc_delivery_total_time,
			stat_tpcc_delivery_max_time,
			stat_tpcc_delivery_min_time);

	fclose(fp);

	/* Initialize */
	stat_tpcc_delivery_cnt = 0;
	stat_tpcc_delivery_total_time = 0;
	stat_tpcc_delivery_max_time = 0;
	stat_tpcc_delivery_min_time = UINT_MAX;
}

/* Print payment transaction results */
static void
stat_tpcc_print_payment(time_t sec, int pid)
{
	char filename[256];
	FILE *fp;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/payment/payment.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/payment/payment.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp, "%lu %lu %lu %lu %lu\n",
			sec,
			stat_tpcc_payment_cnt,
			stat_tpcc_payment_total_time,
			stat_tpcc_payment_max_time,
			stat_tpcc_payment_min_time);

	fclose(fp);

	/* Initialize */
	stat_tpcc_payment_cnt = 0;
	stat_tpcc_payment_total_time = 0;
	stat_tpcc_payment_max_time = 0;
	stat_tpcc_payment_min_time = UINT_MAX;
}

/* Print ostat transaction results */
static void
stat_tpcc_print_ostat(time_t sec, int pid)
{
	char filename[256];
	FILE *fp;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/ostat/ostat.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/ostat/ostat.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp, "%lu %lu %lu %lu %lu\n",
			sec,
			stat_tpcc_ostat_cnt,
			stat_tpcc_ostat_total_time,
			stat_tpcc_ostat_max_time,
			stat_tpcc_ostat_min_time);

	fclose(fp);

	/* Initialize */
	stat_tpcc_ostat_cnt = 0;
	stat_tpcc_ostat_total_time = 0;
	stat_tpcc_ostat_max_time = 0;
	stat_tpcc_ostat_min_time = UINT_MAX;
}

/* Print slev transaction results */
static void
stat_tpcc_print_slev(time_t sec, int pid)
{
	char filename[256];
	FILE *fp;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/slev/slev.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/slev/slev.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp, "%lu %lu %lu %lu %lu\n",
			sec,
			stat_tpcc_slev_cnt,
			stat_tpcc_slev_total_time,
			stat_tpcc_slev_max_time,
			stat_tpcc_slev_min_time);

	fclose(fp);

	/* Initialize */
	stat_tpcc_slev_cnt = 0;
	stat_tpcc_slev_total_time = 0;
	stat_tpcc_slev_max_time = 0;
	stat_tpcc_slev_min_time = UINT_MAX;
}

/* Print insert operation results */
static void
stat_tpcc_print_insert(time_t sec, int pid)
{
	char filename[256];
	FILE *fp;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/insert/insert.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/insert/insert.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp, "%lu %lu %lu %lu %lu\n",
			sec,
			stat_tpcc_insert_cnt,
			stat_tpcc_insert_total_time,
			stat_tpcc_insert_max_time,
			stat_tpcc_insert_min_time);

	fclose(fp);

	/* Initialize */
	stat_tpcc_insert_cnt = 0;
	stat_tpcc_insert_total_time = 0;
	stat_tpcc_insert_max_time = 0;
	stat_tpcc_insert_min_time = UINT_MAX;
}

/* Print update operation results */
static void
stat_tpcc_print_update(time_t sec, int pid)
{
	char filename[256];
	FILE *fp;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/update/update.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/update/update.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp, "%lu %lu %lu %lu %lu\n",
			sec,
			stat_tpcc_update_cnt,
			stat_tpcc_update_total_time,
			stat_tpcc_update_max_time,
			stat_tpcc_update_min_time);

	fclose(fp);

	/* Initialize */
	stat_tpcc_update_cnt = 0;
	stat_tpcc_update_total_time = 0;
	stat_tpcc_update_max_time = 0;
	stat_tpcc_update_min_time = UINT_MAX;
}

/* Print update operation's breakdown results */
static void
stat_tpcc_print_breakdown_update(int pid)
{
	char filename[256];
	FILE *fp;

	/* quick exit */
	if (stat_tpcc_breakdown_update_cnt == 0)
		return;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/breakdown_update/breakdown_update.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/breakdown_update/breakdown_update.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp,
			...
			stat_tpcc_breakdown_update_total
				/ stat_tpcc_breakdown_update_cnt,

			stat_tpcc_breakdown_update_ExecUpdateAct
				/ stat_tpcc_breakdown_update_cnt);

	fclose(fp);

	/* Initialize */
	stat_tpcc_breakdown_update_exec_update_flag = false;
	stat_tpcc_breakdown_update_cnt = 0;
	stat_tpcc_breakdown_update_total = 0;
	stat_tpcc_breakdown_update_ExecUpdateAct = 0;
}

/* Print insert operation's breakdown results */
static void
stat_tpcc_print_breakdown_insert(int pid)
{
	char filename[256];
	FILE *fp;

	/* quick exit */
	if (stat_tpcc_breakdown_insert_cnt == 0)
		return;

#ifdef PANDORA
	sprintf(filename, "../stat/pandora/tpcc/breakdown_insert/breakdown_insert.%d", pid);
#else /* !PANDORA */
	sprintf(filename, "../stat/vanilla/tpcc/breakdown_insert/breakdown_insert.%d", pid);
#endif /* PANDORA */
	...
	fprintf(fp,
			...
			"\"total\" : %lu, "
			"\"tuple_insert\" : %lu, "
			"\"read_buffer_before\" : %lu, "
			"\"read_buffer\" : %lu, "
			"\"read_buffer_after\" : %lu, "
			"\"buffer_lock\" : %lu, "
			"\"record_write\" : %lu, "
			"\"LOG\" : %lu, "
			"\"pandora_meta_mem_lock\" : %lu, "
			"\"pandora_mempart_lock\" : %lu, "
			"\"ExecInsertIndexTuples\" : %lu, "
			"\"HapGetPartIdHash\" : %lu, "
			"\"FKeyCheck\" : %lu, "
			"\"build_hidden\" : %lu, "
			"\"deconstruct_hidden\" : %lu, "
			"\"index_insert\" : %lu, "
			"\"heap_insert\" : %lu, "
			"\"heap_insert_before_get_buffer\" : %lu, "
			"\"heap_insert_get_buffer\" : %lu, "
			"\"heap_insert_after_get_buffer\" : %lu "
			"}\n",

			stat_tpcc_breakdown_insert_total
				/ stat_tpcc_breakdown_insert_cnt,

			stat_tpcc_breakdown_insert_tuple_insert
				/ stat_tpcc_breakdown_insert_cnt,
			...
			stat_tpcc_breakdown_insert_ExecInsertIndexTuples
				/ stat_tpcc_breakdown_insert_cnt,

			stat_tpcc_breakdown_insert_HapGetPartIdHash
				/ stat_tpcc_breakdown_insert_cnt,

			stat_tpcc_breakdown_insert_fkey_check
				/ stat_tpcc_breakdown_insert_cnt,

			stat_tpcc_breakdown_insert_build_hidden
				/ stat_tpcc_breakdown_insert_cnt,

			stat_tpcc_breakdown_insert_deconstruct_hidden
				/ stat_tpcc_breakdown_insert_cnt,

			stat_tpcc_breakdown_insert_index_insert
				/ stat_tpcc_breakdown_insert_cnt,

			stat_tpcc_breakdown_insert_heap_insert
				/ stat_tpcc_breakdown_insert_cnt,
	...
	/* Initialize */
	stat_tpcc_breakdown_insert_exec_insert_flag = false;
	stat_tpcc_breakdown_insert_cnt = 0;
	stat_tpcc_breakdown_insert_total = 0;
	stat_tpcc_breakdown_insert_tuple_insert = 0;
	stat_tpcc_breakdown_insert_read_buffer_before = 0;
	stat_tpcc_breakdown_insert_read_buffer = 0;
	stat_tpcc_breakdown_insert_read_buffer_after = 0;
	stat_tpcc_breakdown_insert_buffer_lock = 0;
	stat_tpcc_breakdown_insert_record_write = 0;
	stat_tpcc_breakdown_insert_LOG = 0;
	stat_tpcc_breakdown_insert_pandora_meta_mem_lock = 0;
	stat_tpcc_breakdown_insert_pandora_mempart_lock = 0;
	stat_tpcc_breakdown_insert_ExecInsertIndexTuples = 0;
	stat_tpcc_breakdown_insert_HapGetPartIdHash = 0;
	stat_tpcc_breakdown_insert_fkey_check = 0;
	stat_tpcc_breakdown_insert_build_hidden = 0;
	stat_tpcc_breakdown_insert_deconstruct_hidden = 0;
	stat_tpcc_breakdown_insert_index_insert = 0;
	stat_tpcc_breakdown_insert_heap_insert = 0;
	stat_tpcc_breakdown_insert_heap_insert_before_get_buffer = 0;
	stat_tpcc_breakdown_insert_heap_insert_get_buffer = 0;
	stat_tpcc_breakdown_insert_heap_insert_after_get_buffer = 0;
}

/* Write results */
static void
stat_tpcc_print_result(void)
{
	...
	/* Make directories */
	stat_tpcc_make_result_dir();

	/* Print results per second */
	if (stat_tpcc_print_sec >= curr.tv_sec)
		return;

	pg_memory_barrier();

	/* pid is used as a file identifier */
	pid = getpid();

	stat_tpcc_print_neword(curr.tv_sec, pid);

	stat_tpcc_print_delivery(curr.tv_sec, pid);

	stat_tpcc_print_payment(curr.tv_sec, pid);

	stat_tpcc_print_ostat(curr.tv_sec, pid);

	stat_tpcc_print_slev(curr.tv_sec, pid);

	stat_tpcc_print_insert(curr.tv_sec, pid);

	stat_tpcc_print_update(curr.tv_sec, pid);

	stat_tpcc_print_breakdown_update(pid);

	stat_tpcc_print_breakdown_insert(pid);

	/* Reset print second */
	stat_tpcc_print_sec = curr.tv_sec;
}
#endif /* STAT_TPCC */
						...
						QueryCompletion *qc)
{
	...
	/* NEWORD */
	struct timespec stat_tpcc_neword_start;
	struct timespec stat_tpcc_neword_end;

	/* DELIVERY */
	struct timespec stat_tpcc_delivery_start;
	struct timespec stat_tpcc_delivery_end;

	/* PAYMENT */
	static struct timespec stat_tpcc_payment_start;
	static struct timespec stat_tpcc_payment_end;

	/* OSTAT */
	static struct timespec stat_tpcc_ostat_start;
	static struct timespec stat_tpcc_ostat_end;

	/* SLEV */
	static struct timespec stat_tpcc_slev_start;
	static struct timespec stat_tpcc_slev_end;

	/* Measure the number of NEW ORDER transactions per second */
	if (strncmp(queryString, "call neword", 11) == 0)
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_neword_start);
	else if (strncmp(queryString, "call delivery", 13) == 0)
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_delivery_start);
	else if (strncmp(queryString, "call payment", 12) == 0)
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_payment_start);
	else if (strncmp(queryString, "call ostat", 10) == 0)
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_ostat_start);
	else if (strncmp(queryString, "call slev", 9) == 0)
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_slev_start);
#endif /* STAT_TPCC */
	...
	if (strncmp(queryString, "call neword", 11) == 0)
	{
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_neword_end);

		stat_tpcc_elapsed_time
			= stat_tpcc_get_time_diff(stat_tpcc_neword_start,
									  stat_tpcc_neword_end);

		stat_tpcc_neword_cnt++;

		stat_tpcc_neword_total_time += stat_tpcc_elapsed_time;

		stat_tpcc_neword_max_time
			= stat_tpcc_neword_max_time < stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_neword_max_time;

		stat_tpcc_neword_min_time
			= stat_tpcc_neword_min_time > stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_neword_min_time;
	}
	/* DELIVERY */
	else if (strncmp(queryString, "call delivery", 13) == 0)
	{
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_delivery_end);

		stat_tpcc_elapsed_time
			= stat_tpcc_get_time_diff(stat_tpcc_delivery_start,
									  stat_tpcc_delivery_end);

		stat_tpcc_delivery_cnt++;

		stat_tpcc_delivery_total_time += stat_tpcc_elapsed_time;

		stat_tpcc_delivery_max_time
			= stat_tpcc_delivery_max_time < stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_delivery_max_time;

		stat_tpcc_delivery_min_time
			= stat_tpcc_delivery_min_time > stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_delivery_min_time;
	}
	/* PAYMENT */
	else if (strncmp(queryString, "call payment", 12) == 0)
	{
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_payment_end);

		stat_tpcc_elapsed_time
			= stat_tpcc_get_time_diff(stat_tpcc_payment_start,
									  stat_tpcc_payment_end);

		stat_tpcc_payment_cnt++;

		stat_tpcc_payment_total_time += stat_tpcc_elapsed_time;

		stat_tpcc_payment_max_time
			= stat_tpcc_payment_max_time < stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_payment_max_time;

		stat_tpcc_payment_min_time
			= stat_tpcc_payment_min_time > stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_payment_min_time;
	}
	/* OSTAT */
	else if (strncmp(queryString, "call ostat", 10) == 0)
	{
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_ostat_end);

		stat_tpcc_elapsed_time
			= stat_tpcc_get_time_diff(stat_tpcc_ostat_start,
									  stat_tpcc_ostat_end);

		stat_tpcc_ostat_cnt++;

		stat_tpcc_ostat_total_time += stat_tpcc_elapsed_time;

		stat_tpcc_ostat_max_time
			= stat_tpcc_ostat_max_time < stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_ostat_max_time;

		stat_tpcc_ostat_min_time
			= stat_tpcc_ostat_min_time > stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_ostat_min_time;
	}
	/* SLEV */
	else if (strncmp(queryString, "call slev", 9) == 0)
	{
		clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_slev_end);

		stat_tpcc_elapsed_time
			= stat_tpcc_get_time_diff(stat_tpcc_slev_start,
									  stat_tpcc_slev_end);

		stat_tpcc_slev_cnt++;

		stat_tpcc_slev_total_time += stat_tpcc_elapsed_time;

		stat_tpcc_slev_max_time
			= stat_tpcc_slev_max_time < stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_slev_max_time;

		stat_tpcc_slev_min_time
			= stat_tpcc_slev_min_time > stat_tpcc_elapsed_time ?
					stat_tpcc_elapsed_time : stat_tpcc_slev_min_time;
	}

	pg_memory_barrier();

	/* Print results */
	stat_tpcc_print_result();
#endif /* STAT_TPCC */
}
