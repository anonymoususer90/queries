...
extern char statPageReadIODir[64];
extern int pageReadCnt;
extern uint64 elapsedTime;
#endif /* STAT_PAGE_READ_IO */

#ifdef STAT_IO_URING
extern char statIOUringDir[64];
char io_uring_query_no[4];
uint64 io_uring_submitTime;
uint64 io_uring_waitTime;
#endif /* STAT_IO_URING */
				...
				DestReceiver *dest)
{
	...
	if (isOLAP)
	{
		...
		MemSet(io_uring_query_no, 0, 4);

		if (strncmp(queryDesc->sourceText+1, "-", 1))
			query_no_pos = 32;

		strncpy(io_uring_query_no, queryDesc->sourceText+query_no_pos, 2);

		io_uring_submitTime = 0;
		io_uring_waitTime = 0;
#endif /* STAT_IO_URING */
	}
#endif /* STAT_PAGE_READ_IO || STAT_IO_URING */
...
#ifdef STAT_PAGE_READ_IO_BREAKDOWN
	for (int i = 0; i < PANDORA_MAX_LEVEL; i++)
	{
		stat_is_valid_level[i] = false;
		stat_heap_io_uring[i] = 0;
		stat_heap_shared_buffer_io[i] = 0;
		stat_heap_shared_buffer_hit[i] = 0;
	}

	stat_idx_shared_buffer_io = 0;
	stat_idx_shared_buffer_hit = 0;
	stat_ebi_io = 0;
	stat_ebi_hit = 0;
#endif /* STAT_PAGE_READ_IO_BREAKDOWN */
	...
	if (isOLAP)
	{
#ifdef STAT_PAGE_READ_IO
		{
			...
			int query_no_pos = 5;
			FILE *result;
			float elapsedTimeFloat = ((float)elapsedTime) / 1000000000;

			if (strncmp(queryDesc->sourceText+1, "-", 1))
				query_no_pos = 32;

			strncpy(query_no, queryDesc->sourceText+query_no_pos, 2);
			sprintf(file_name, "%s/%s_%d.txt", statPageReadIODir, query_no, MyProcPid);
			fprintf(stderr, "file name: %s, query #: %s, elapsed time: %.3f, page count: %d\n", file_name, query_no, elapsedTimeFloat, pageReadCnt);

			result = fopen(file_name, "a");
			fprintf(result, "%s %.3f %d\n", query_no, elapsedTimeFloat, pageReadCnt);
			fclose(result);
		}
#endif /* STAT_PAGE_READ_IO */
#ifdef STAT_IO_URING
		if (io_uring_submitTime != 0)
		{
			char file_name[64] = { 0 };
			FILE *result;
			float submitTimeFloat = ((float)io_uring_submitTime) / 1000000000;
			float waitTimeFloat = ((float)io_uring_waitTime) / 1000000000;
			
			sprintf(file_name, "%s/%s_%d.txt", statIOUringDir, io_uring_query_no, MyProcPid);
			fprintf(stderr, "file name: %s, query #: %s, submit time: %.3f, wait time: %.3f\n", file_name, io_uring_query_no, submitTimeFloat, waitTimeFloat);

			result = fopen(file_name, "a");
			fprintf(result, "%s %.3f %.3f\n", io_uring_query_no, submitTimeFloat, waitTimeFloat);
			fclose(result);
		}
#endif /* STAT_IO_URING */
	}
	...
	if (isOLAP && queryDesc)
	{
...
#endif /* PANDORA */
		{
			...
			if (!stat_is_valid_level[i])
				continue;

#ifdef PANDORA
			sprintf(filename,
					"../stat/pandora/olap_read_io_breakdown/heap/%s_%d.%d",
					query_no, i, getpid());
#else /* !PANDORA */
			sprintf(filename,
					"../stat/vanilla/olap_read_io_breakdown/heap/%s_%d.%d",
					query_no, i, getpid());
#endif /* PANDORA */
			...
			fprintf(fp,
					"%lu %lu %lu %lu\n",
					curr.tv_sec,
					stat_heap_io_uring[i],
					stat_heap_shared_buffer_io[i],
					stat_heap_shared_buffer_hit[i]);
...
#ifdef PANDORA
			sprintf(filename,
					"../stat/pandora/olap_read_io_breakdown/index/%s.%d",
					query_no, getpid());
#else /* !PANDORA */
			sprintf(filename,
					"../stat/vanilla/olap_read_io_breakdown/index/%s.%d",
					query_no, getpid());
#endif /* PANDORA */
