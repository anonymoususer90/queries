					...
					LWLock *rwlock)
{
	...
	Assert(page_offset + aligned_tuple_size <= EBI_SUB_SEG_PAGESZ);
				   ...
				   bool fast_memory_lookup)
{
	...
	/* 
	 * If this is fast_memory_lookup and the ebi page is not in memroy, don't do
	 * I/O, just return -2.
	 */
...
EbiSubCreateSegmentFile(EbiSubSegmentId seg_id, Oid rel_id)
{
	...
	fd = open(filename, O_RDWR | O_CREAT | O_DIRECT | O_SYNC, (mode_t)0600);

	if (fd < 0)
	{
		fprintf(stderr, "ebi sub segment file create error: %s", strerror(errno));
		Assert(fd >= 0);
	}
...
EbiSubRemoveOldPartitionFile(Oid rel_id, OldPartitions old_partitions)
{
	...
	Assert((old_partitions.part_lvl == 0 && old_partitions.part_gen == 0) ||
		   (old_partitions.part_lvl != 0 &&
		    old_partitions.min_part_num == old_partitions.max_part_num));

	for (int i = old_partitions.min_part_num;
		 i <= old_partitions.max_part_num; ++i)
	{
		snprintf(filename, 32, "base/%u/%u.%u.%u.%u",
				 old_partitions.db_id, rel_id, old_partitions.part_lvl, i, old_partitions.part_gen);
		result = unlink(filename);
		Assert(result == 0);
#ifdef PANDORA_DEBUG
		fprintf(stderr, "[PANDORA] GC old partition file, name: %s\n", filename);
#endif /* PANDORA_DEBUG */
	}
}
#endif /* PANDORA */
...
EbiSubOpenSegmentFile(EbiSubSegmentId seg_id, Oid rel_id)
{
	...
	fd = open(filename, O_RDWR | O_DIRECT | O_DSYNC, (mode_t) 0600);
