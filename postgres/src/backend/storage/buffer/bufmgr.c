...
#ifdef STAT_TPCC
#include "stat/stat_tpcc.h"
#endif /* STAT_TPCC */

#define MEMPARTITION_FLUSH_RETRY_MAX 16
#endif /* PANDORA */

#ifdef STAT_PAGE_READ_IO_BREAKDOWN
#include "stat/stat_page_read_io_breakdown.h"

bool stat_is_valid_level[PANDORA_MAX_LEVEL] = { 0, };
uint64_t stat_heap_io_uring[PANDORA_MAX_LEVEL] = { 0, };
uint64_t stat_heap_shared_buffer_io[PANDORA_MAX_LEVEL] = { 0, };
uint64_t stat_heap_shared_buffer_hit[PANDORA_MAX_LEVEL] = { 0, };
uint64_t stat_idx_shared_buffer_io = 0;
uint64_t stat_idx_shared_buffer_hit = 0;
uint64_t stat_ebi_io = 0;
uint64_t stat_ebi_hit = 0;
#endif /* STAT_PAGE_READ_IO_BREAKDOWN */

#ifdef PANDORA
extern ReadBufDescPadded *ReadBufDescriptors;
extern char *ReadBufBlocks;
...
#define PrefetchDescGetNeedPrefetch(prefetchDesc) \
			(((PrefetchDesc) prefetchDesc)->prefetch_end == false && \
			((PrefetchDesc) prefetchDesc)->io_req_free != NULL)
#define PartitionPrefetchDescGetNeedPrefetch(prefetchDesc) \
			(((PartitionPrefetchDesc) prefetchDesc)->prefetch_end == false && \
			((PartitionPrefetchDesc) prefetchDesc)->io_req_free != NULL)

static void PopulateBuffersFromReadBuffer(SMgrRelation smgr, 
						PrefetchDesc prefetchDesc, BufferAccessStrategy strategy);
static bool GetBufferFromIoCompletion(PrefetchDesc prefetchDesc, 
						BlockNumber blockNum, char **readBufferPage, 
						Buffer *traditionalBuffer);
static bool PandoraFlushMempartBuffer(BufferDesc *buffer, int nblock);
static void PopulatePartitionBuffersFromReadBuffer(RelFileNode node,
...
#ifdef PANDORA
typedef struct BufferIOInfoData
{
	BufferDesc 	*buf_hdr;				/* buffer desc */
	struct BufferIOInfoData* next;	/* next element */
	bool 				isForInput;			/* is input or output */
} BufferIOInfoData;

typedef struct BufferIOInfoData *BufferIOInfo;

#define INPROGRESS_BUF_HASH_SIZE (256)

/* we use hashtable instead of InProgressBuf */
static BufferIOInfo InProgressBufHash[INPROGRESS_BUF_HASH_SIZE] = { NULL };

/* we recycle BufferIoInfo object using freelist */
static BufferIOInfo BufferIOFreeList = NULL;
#else /* !PANDORA */
...
#if defined(STAT_PAGE_READ_IO) || defined(STAT_IO_URING) || \
	defined(STAT_PAGE_READ_IO_BREAKDOWN)
bool isOLAP = false;
#endif /* STAT_PAGE_READ_IO || STAT_IO_URING */

#ifdef STAT_PAGE_READ_IO
int pageReadCnt = 0;
uint64 elapsedTime = 0;
uint64 diff;
struct timespec beginStamp;
struct timespec endStamp;
#endif /* STAT_PAGE_READ_IO */

#ifdef STAT_IO_URING
extern uint64 io_uring_waitTime;
#endif /* STAT_IO_URING */
...
#ifdef PANDORA
	/* Change the given buffer to the mempart buffer if it needed */
	if (PandoraCheckMempartBuf(buffer - 1))
		buffer = PandoraGetMempartBufId(buffer - 1) + 1;
#endif /* PANDORA */
...
static Buffer ReadBuffer_common(SMgrRelation reln, char relpersistence,
								ForkNumber forkNum, PandoraPartLevel partLvl,
								PandoraPartNumber partNum,
								PandoraPartGenNo partGen, BlockNumber blockNum,
...
static BufferDesc *BufferAlloc(SMgrRelation smgr,
							   char relpersistence,
							   ForkNumber forkNum,
							   PandoraPartLevel partLvl,
							   PandoraPartNumber partNum,
							   PandoraPartGenNo partGen,
							   BlockNumber blockNum,
							   BufferAccessStrategy strategy,
							   bool *foundPtr);
static bool FlushBuffer(BufferDesc *buf, SMgrRelation reln,
						bool isPandoraPartition);
static BufferDesc *
PandoraBufferAllocLevel0(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
			PandoraPartLevel partLvl, PandoraPartNumber partNum,
			PandoraPartGenNo partGen, BlockNumber blockNum,
			BufferAccessStrategy strategy,
			bool *foundPtr, bool is_pandora_mempart_insert,
			bool is_pandora_new_mempart);
#else
static BufferDesc *BufferAlloc(SMgrRelation smgr,
							   char relpersistence,
							   ForkNumber forkNum,
...
#ifdef PANDORA
static void InsertInProgressBuffer(BufferDesc *buf, bool forInput);
static void DeleteInProgressBuffer(BufferDesc *buf);
static void ClearInProgressBufferHash(void);
#endif /* PANDORA */
 ...
 */
#ifdef PANDORA
/* TODO: Apply pandora partitioning when implementing logging & recovery */
#endif /* PANDORA */
bool
...
#ifdef PANDORA
Buffer
ReadBufferExtended_internal(Relation reln, ForkNumber forkNum,
							PandoraPartLevel partLvl, PandoraPartNumber partNum,
							PandoraPartGenNo partGen, BlockNumber blockNum,
							ReadBufferMode mode, BufferAccessStrategy strategy)
#else
...
#endif /* PANDORA */
{
	...
	buf = ReadBuffer_common(RelationGetSmgr(reln), reln->rd_rel->relpersistence,
							forkNum, partLvl, partNum, partGen,	blockNum, mode,
							strategy, &hit);

#ifdef STAT_PAGE_READ_IO_BREAKDOWN
	if (reln->rd_rel->relkind == RELKIND_RELATION &&
		partLvl != InvalidPandoraPartLevel)
	{
		stat_is_valid_level[partLvl] = true;

		if (hit)
			stat_heap_shared_buffer_hit[partLvl]++;
		else
			stat_heap_shared_buffer_io[partLvl]++;
	}

	if (reln->rd_rel->relkind == RELKIND_INDEX)
	{
		if (hit)
			stat_idx_shared_buffer_hit++;
		else
			stat_idx_shared_buffer_io++;
	}
#endif /* STAT_PAGE_READ_IO_BREAKDOWN */
...
#ifdef STAT_PAGE_READ_IO_BREAKDOWN
	stat_is_valid_level[0] = true;

	if (hit)
	{
		if (reln->rd_rel->relkind == RELKIND_RELATION)
			stat_heap_shared_buffer_hit[0]++;
...
#ifdef PANDORA
/*
 * ReadPartitionBufferForRepartitioning -- returns a buffer containing the
 * 		requested partition block of the requested
 * 		relation by the repartitioning process.
 * 		Logic is same as ReadBufferExtended.
 *
 * Returns: the buffer number for the buffer containing
 *		the block read.  The returned buffer has been pinned.
 *		Does not return on error --- elog's instead.
 */
Buffer
ReadPartitionBufferForRepartitioning(Relation reln,
									 PandoraPartLevel partLvl, PandoraPartNumber partNum,
									 PandoraPartGenNo partGen, BlockNumber blockNum,
									 ReadBufferMode mode, BufferAccessStrategy strategy)
{
	...
	buf = ReadBuffer_common(RelationGetSmgr(reln), 'p', PANDORA_PARTITION_FORKNUM,
							partLvl, partNum, partGen, blockNum, mode,
							strategy, &hit);
...
/*
 * TODO
 * This function is called during "CREATE DATABASE" and redo(recovery).
 * 
 * CREATE DATABASE: copy the template database to make new database
 * redo: replay the WAL log using buffer pool
 * 
 * So, if the template database will have partition data or or logging &
 * recovery will be implemented, a new function will need to be added
 * accordingly by applying this function.
 */
#endif /* PANDORA */
/*
						  ...
						  BufferAccessStrategy strategy, bool permanent)
{
	...
	return ReadBuffer_common(smgr, permanent ? RELPERSISTENCE_PERMANENT :
							 RELPERSISTENCE_UNLOGGED, forkNum,
							 InvalidPandoraPartLevel, InvalidPandoraPartNumber,
							 InvalidPandoraPartGenNo, blockNum,
							 mode, strategy, &hit);
#else
	return ReadBuffer_common(smgr, permanent ? RELPERSISTENCE_PERMANENT :
...
#endif /* PANDORA */
{
...
#ifdef PANDORA
	bool		isPandoraMempartInsert = false;
	bool		isPandoraPartition = (forkNum == PANDORA_PARTITION_FORKNUM);
	bool		isPandoraNewMempart = false;
	bool		onPandoraMempartBuf = false;
#ifdef STAT_TPCC
	uint64_t stat_tpcc_elapsed_time = 0;

	struct timespec stat_tpcc_breakdown_insert_buffer_lock_start;
	struct timespec stat_tpcc_breakdown_insert_buffer_lock_end;
#endif /* STAT_TPCC */

	Assert(!isPandoraPartition || (
		   partLvl != InvalidPandoraPartLevel &&
		   partNum != InvalidPandoraPartNumber &&
		   partGen != InvalidPandoraPartGenNo &&
		   !isLocalBuf
	));

	/* When insert new tuple for PANDORA, we use mempart buffer */
	if (isPandoraPartition && mode == RBM_PANDORA_MEMPART_INSERT)
	{
		isPandoraMempartInsert = true;
		...
		else
		{
			...
			isPandoraNewMempart = true;
		}
	}

	Assert(mode != RBM_PANDORA_MEMPART_INSERT);
#endif /* PANDORA */
	...
	*hit = false;
	...
	{
#ifdef PANDORA
		if (isPandoraPartition)
		{
			if (isPandoraMempartInsert)
				blockNum = 0;
			else
				blockNum = PandoraNblocks(smgr->smgr_rnode.node, partLvl, partNum,
										  partGen);
		}
		else
	...
	else
	{
		...
		if (partLvl == 0)
		{
			bufHdr = PandoraBufferAllocLevel0(smgr, relpersistence, forkNum,
							partLvl, partNum, partGen, blockNum, strategy,
							&found, isPandoraMempartInsert,
							isPandoraNewMempart);

			if (bufHdr == NULL)
			{				
				/* If this is not an insert, buffer allocation must success */
				Assert(isPandoraMempartInsert);
				return InvalidBuffer;
			}

			onPandoraMempartBuf = PandoraCheckMempartBuf(bufHdr->buf_id);
		}
		else
			bufHdr = BufferAlloc(smgr, relpersistence, forkNum, partLvl, partNum,
							 partGen, blockNum, strategy, &found);
#else
...
#ifdef PANDORA
			return onPandoraMempartBuf ?
				BufferDescriptorGetBuffer(bufHdr) + blockNum :
	...
	if (isExtend)
	{
		...
		if (onPandoraMempartBuf)
		{
			Assert(isPandoraPartition && isPandoraMempartInsert);
			MemSet((char *) bufBlock, 0, BLCKSZ * PandoraMempartPageCount);
		}
		else
		...
		{
			if (onPandoraMempartBuf)
				PandoraMempartExtend(smgr->smgr_rnode.node, partLvl, partNum,
									 partGen, blockNum, (char *) bufBlock);
			else
				PandoraExtend(smgr->smgr_rnode.node, partLvl, partNum, partGen,
	...
	else
	{
		...
		else
		{
...
#ifdef STAT_PAGE_READ_IO
			if (isOLAP)
				clock_gettime(CLOCK_MONOTONIC, &beginStamp);
#endif /* STAT_PAGE_READ_IO */
#ifdef PANDORA
			if (isPandoraPartition)
			{
				PandoraRead(smgr->smgr_rnode.node, partLvl, partNum, partGen,
							blockNum, (char *) bufBlock);
			}
			else
			...
			if (isOLAP)
			{
				clock_gettime(CLOCK_MONOTONIC, &endStamp);
				diff = (endStamp.tv_sec * 1000000000 + endStamp.tv_nsec) -
					 (beginStamp.tv_sec * 1000000000 + beginStamp.tv_nsec);
				elapsedTime += diff;
				++pageReadCnt;
			}
#endif /* STAT_PAGE_READ_IO */
...
#ifdef PANDORA
			/*
			 * Increment pd_gen to determine that the old array can be GC'd if
			 * the p-leaf array pointer of p-locator is modified.
			 */
			(((PageHeader)bufBlock)->pd_gen)++;
#endif /* PANDORA */
		}
	}
	...
	{
#ifdef STAT_TPCC
	if (onPandoraMempartBuf && stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_buffer_lock_start);
#endif /* STAT_TPCC */
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_buffer_lock_end);

		stat_tpcc_elapsed_time
			= stat_tpcc_get_time_diff(
				stat_tpcc_breakdown_insert_buffer_lock_start,
				stat_tpcc_breakdown_insert_buffer_lock_end);

		stat_tpcc_breakdown_insert_buffer_lock
			+= stat_tpcc_elapsed_time;
	}
#endif /* STAT_TPCC */
	}
...
#endif /* PANDORA */
{
	...
	/* create a tag so we can lookup the buffer */
#ifdef PANDORA
	INIT_BUFFERTAG_EXTENDED(newTag, smgr->smgr_rnode.node, forkNum, partLvl,
							partNum, partGen, blockNum);
#else
...
#ifdef PANDORA
				isWritten = FlushBuffer(buf, NULL,
										(buf->tag.forkNum == PANDORA_PARTITION_FORKNUM));
#else
...
MarkBufferDirty(Buffer buffer)
{
	...
	/*
	 * If this buffer is located in mempart buffer, change it to the start
	 * buffer of the mempartition.
	 */
	if (PandoraCheckMempartBuf(buffer - 1))
	{
		old_buffer = buffer;
	...
	/*
	 * When we use mempartition buffer, we increase refcount of the first page
	 * in mempartition and get lock of individual pages.
	 */
	if (old_buffer != InvalidBuffer)
	{
		Assert(BufferIsPinned(buffer));
		Assert(LWLockHeldByMeInMode(BufferDescriptorGetContentLock(GetBufferDescriptor(old_buffer - 1)),
									LW_EXCLUSIVE));
	}
	else
	{
...
#ifdef PANDORA
/* 
 * PandoraReleaseAndReadBuffer
 */
Buffer
PandoraReleaseAndReadBuffer(Buffer buffer, Relation relation,
						PandoraPartLevel partLvl, PandoraPartNumber partNum,
						PandoraPartGenNo partGen, BlockNumber blockNum)
{
	ForkNumber forkNum = PANDORA_PARTITION_FORKNUM;
	BufferDesc *bufHdr;
	...
	if (BufferIsValid(buffer))
	{
		...
		if (bufHdr->tag.blockNum == blockNum &&
			RelFileNodeEquals(bufHdr->tag.rnode, relation->rd_node) &&
			bufHdr->tag.partNum == partNum &&
			bufHdr->tag.partGen == partGen &&
			bufHdr->tag.partLvl == partLvl &&
			bufHdr->tag.forkNum == forkNum)
			return buffer;
		UnpinBuffer(bufHdr, true);
	}

	return ReadPartitionBufferExtended(relation, partLvl, partNum, partGen,
													blockNum, RBM_NORMAL, NULL);
}
#endif
...
#ifdef PANDORA
	/* Change the given buffer to the mempart buffer if it needed */
	if (PandoraCheckMempartBuf(b - 1))
	{
		b = PandoraGetMempartBufId(b - 1) + 1;
		buf = GetBufferDescriptor(b - 1);
	}
#endif /* PANDORA */
...
#ifdef PANDORA
			item->partLvl = bufHdr->tag.partLvl;
			item->partNum = bufHdr->tag.partNum;
			item->partGen = bufHdr->tag.partGen;
#endif /* PANDORA */
...
#ifdef PANDORA
/*
 * PandoraMempartSync -- Write out all dirty mempartitions in the pool.
 *
 * This is called at checkpoint time to write out all dirty mempartitions.
 * The checkpoint request flags should be passed in.  If CHECKPOINT_IMMEDIATE
 * is set, we disable delays between writes; if CHECKPOINT_IS_SHUTDOWN,
 * CHECKPOINT_END_OF_RECOVERY or CHECKPOINT_FLUSH_ALL is set, we write even
 * unlogged buffers, which are otherwise skipped.  The remaining flags
...
PandoraMempartSync(int flags)
{
	...
	/*
	 * Unless this is a shutdown checkpoint or we have been explicitly told,
	 * we write only permanent, dirty mempartitions.  But at shutdown or end of
	 * recovery, we write all dirty mempartitions.
	 */
	if (!((flags & (CHECKPOINT_IS_SHUTDOWN | CHECKPOINT_END_OF_RECOVERY |
					CHECKPOINT_FLUSH_ALL))))
		mask |= BM_PERMANENT;
	...
	/*
	 * Loop over all mempartition buffers, and flush the dirty mempartition.
	 */
	for (buf_id = PandoraMempartStart;
		 buf_id < NBuffers + NPandoraMempartBuffers;
		 buf_id += PandoraMempartPageCount)
	{
		BufferDesc *bufHdr = GetBufferDescriptor(buf_id);
		...
		if ((buf_state & mask) == mask)
		{
			...
			mempart_id = PandoraGetMempartIdFromBufId(bufHdr->buf_id);
			Assert(mempart_id >= 0);

			/*
			 * Befor flush, get the real number of blocks in dirty mempart.
			 * Here we do not modify meta's information. Just read the record
			 * count and record counts per block to calculate partition size.
			 */
			meta = PandoraGetRelationMeta(bufHdr->tag.rnode.relNode);
			metaMemPartitionLock = PandoraRelationMetaMemPartitionLock(meta);
			LWLockAcquire(metaMemPartitionLock, LW_SHARED);

			/*
			 * If this mempartition is not fully filled(includes the most
			 * recent mempartition) and the current checkpointing is not for
			 * shutdown, don't flush, just skip.
			 * Repartitioning worker determines the count of target record using
			 * lseek() first.
			 * So if the checkpointing flushes the most mempartition during in
			 * progress, repartitioning worker may get wrong count.
			 */
			if (!is_shutdown &&
				pg_atomic_read_u32(&(bufHdr->inserted_cnt)) <
				meta->recordNumsPerBlock * PandoraMempartPageCount)
			{
			...
			LWLockAcquire(PandoraGetMempartLock(mempart_id), LW_EXCLUSIVE);

			/* Calculate mempartition's real size */
			if (unlikely(meta->partitionNumberLevelZero[1] == bufHdr->tag.partNum))
			{
				partition_nblock =
				...
				partition_nblock = PandoraMempartPageCount;

			LWLockRelease(metaMemPartitionLock);

			/* Pin it, share-lock it, write it. */
			LWLockAcquire(BufferDescriptorGetContentLock(bufHdr), LW_SHARED);
			PandoraFlushMempartBuffer(bufHdr, partition_nblock);
			LWLockRelease(BufferDescriptorGetContentLock(bufHdr));

			/* Release mempart lock */
			LWLockRelease(PandoraGetMempartLock(mempart_id));
		...
		/* Check for barrier events in case PANDORA_MEMPART_COUNT is large. */
		if (ProcSignalBarrierPending)
			ProcessProcSignalBarrier();
	}
}
...
#ifdef PANDORA
	isWritten = FlushBuffer(bufHdr, NULL,
							(bufHdr->tag.forkNum == PANDORA_PARTITION_FORKNUM));
#else
...
BufferGetBlockNumber(Buffer buffer)
{
	...
	if (PandoraCheckMempartBuf(buffer - 1))
	{
		...
		Assert(!BufferIsLocal(old_buffer));

		bufHdr = GetBufferDescriptor(buffer - 1);

		Assert(bufHdr->tag.blockNum == 0);

		Assert(old_buffer >= buffer);
...
#ifdef PANDORA
static bool
FlushBuffer(BufferDesc *buf, SMgrRelation reln, bool isPandoraPartition)
#else
static void
...
#endif /* PANDORA */
{
	...
	/* PANDORA's mempartition buffer has its own flush function */
	Assert(!PandoraCheckMempartBuf(buf->buf_id));
#endif /* PANDORA */
	...
	if (isPandoraPartition)
	{
		isWritten = PandoraWrite(reln->smgr_rnode.node,
								 buf->tag.partLvl,
								 buf->tag.partNum,
								 buf->tag.partGen,
								 buf->tag.blockNum,
 ...
 * originate from the traditional buffer pool. If they are sourced from the 
 * traditional buffer pool and have not been released, we proceed to release 
 * all of them.
 */
void
ReleasePrefetchedBuffers(PrefetchDesc prefetchDesc)
{
	...
	while (readBufDesc != NULL)
	{
		...
		while (infoIdx < readBufDesc->page_num)
		{
			...
			if (ioInfo->buf_id != InvalidTraditionalBufferID)
			{
				/* 
				 * It is from traditional buffer pool. 
				 * It is necessary to convert the buffer id into a buffer.
				 */
				ReleaseBuffer(ioInfo->buf_id + 1);
				infoIdx += 1;
			}
			else
			{
...
#ifdef PANDORA
void*
GetBufferDualRef(Buffer buffer)
{
	BufferDesc *buf;
	...
	buf = GetBufferDescriptor(buffer - 1);
	return BufferDescriptorGetDualRef(buf);
}
#endif /* PANDORA */
...
#ifdef PANDORA
	InsertInProgressBuffer(buf, forInput);
#else /* !PANDORA */
	InProgressBuf = buf;
	IsForInput = forInput;
#endif /* PANDORA */
...
#ifdef PANDORA
	if (ba->partLvl < bb->partLvl)
		return -1;
	if (ba->partLvl > bb->partLvl)
		return 1;
	
	if (ba->partNum < bb->partNum)
		return -1;
	if (ba->partNum > bb->partNum)
		return 1;
	
	/* TODO: Does generation number have to be compared? */
	if (ba->partNum < bb->partNum)
		return -1;
	if (ba->partNum > bb->partNum)
		return 1;
...
#ifdef PANDORA
	if (a->partLvl < b->partLvl)
		return -1;
	else if (a->partLvl > b->partLvl)
		return 1;
	
	if (a->partNum < b->partNum)
		return -1;
	else if (a->partNum > b->partNum)
		return 1;
...
IssuePendingWritebacks(WritebackContext *context)
{
	...
	for (i = 0; i < context->nr_pending; i++)
	{
		 ...
		 */
#ifdef PANDORA
		if (tag.forkNum == PANDORA_PARTITION_FORKNUM)
		{
			for (ahead = 0; i + ahead + 1 < context->nr_pending; ahead++)
			{
				...
				if (!RelFileNodeEquals(cur->tag.rnode, next->tag.rnode) ||
					...
					cur->tag.partLvl != next->tag.partLvl ||
					cur->tag.partNum != next->tag.partNum ||
					cur->tag.partGen != next->tag.partGen)
					break;
			...
			PandoraWriteback(tag.rnode, tag.partLvl, tag.partNum,
							 tag.partGen, tag.blockNum, nblocks);
		}
		else
		{
			for (ahead = 0; i + ahead + 1 < context->nr_pending; ahead++)
...
/* TODO: */
static BufferDesc *
BufferLookup(RelFileNode node, ForkNumber forkNum, PandoraPartLevel partLvl,
			 ...
			 bool *foundPtr)
{
	...
	bool		valid;
	BufferDesc *buf = NULL;
	BlockNumber originalBlockNum = blockNum;
	bool		find_mempart = (partLvl == 0);
...
find_hash_table:
	/* create a tag so we can lookup the buffer */
	INIT_BUFFERTAG_EXTENDED(newTag, node, forkNum, partLvl, partNum, partGen,
							blockNum);
...
restart_buffer_lookup:
	/* see if the block is in the buffer pool already */
	LWLockAcquire(newPartitionLock, LW_SHARED);
	buf_id = BufTableLookup(&newTag, newHash);

	/* If there is no mempart buf, do the original thing */
	if (find_mempart && buf_id < PandoraMempartStart)
	{
		LWLockRelease(newPartitionLock);

		blockNum = originalBlockNum;

		find_mempart = false;

		goto find_hash_table;
	}
...
/*
 * ReadBufferPrefetch -- read the page and prefetch others
 * 
 * We read buffer page from traditional buffer or read buffer. We prefetch 
 * required pages on future. We support prefetch feature on sequential scan yet. 
 */
Buffer
ReadBufferPrefetch(Relation reln, BlockNumber blockNum, PrefetchDesc prefetchDesc, 
						BufferAccessStrategy strategy, char** readBufferPage)
{
	Buffer traditionalBuffer;
	int	completionNum = 0;
	bool wait = false;
...
recheck_io_request:
	 ...
	 */
	PopulateBuffersFromReadBuffer(smgr, prefetchDesc, strategy);
	...
	if (!GetBufferFromIoCompletion(prefetchDesc, blockNum, readBufferPage, 
													&traditionalBuffer))
	{
		/*
	...
	return traditionalBuffer;
}

/*
 * PopulateBuffersFromReadBuffer
 ...
 */
static void
PopulateBuffersFromReadBuffer(SMgrRelation smgr, PrefetchDesc prefetchDesc, 
												BufferAccessStrategy strategy)
{
	bool				found;
	PrefetchHeapDesc	hdesc = (PrefetchHeapDesc)prefetchDesc;

	/* If we have element on io_req_free, we need to do prefetch */
	while (PrefetchDescGetNeedPrefetch(prefetchDesc))
	{
		BlockNumber		reqPageId;
		BufferDesc		*bufHdr;
		ReadBufDesc     readBufDesc;
		IoInfo			ioInfo;
		IoInfo			reqIoInfo;
		int 			idx;
		int 			reqIdx = 0;
		uint32			page_io_num = 0; 

		/* Get read buffer desc from free list */
		...
		for (idx = 0; idx < MAX_PAGE_IN_CHUNK; idx++)
		{
			...
			reqPageId = prefetchDesc->next_reqblock;

			Assert(reqPageId <= hdesc->rs_nblocks);

			if (reqPageId == hdesc->rs_nblocks)
			{
				/* Completed when the start block was second encountered */
				if (page_io_num > 0)
				{
					reqIdx = idx - page_io_num;
					...
					reqIoInfo->page_io_num = page_io_num;
					reqIoInfo->req_page_id = reqPageId - page_io_num;

					/* If we can't find on buffer pool, do prefetch */
					smgrrequest(smgr, MAIN_FORKNUM, reqIoInfo->req_page_id,
									&readBufDesc->iovs[reqIdx], 
									prefetchDesc->ring, reqIoInfo);

 					prefetchDesc->requested_io_num += 1;
					page_io_num = 0;
				}

				/* if we meet start block twice, stop prefetching */
				prefetchDesc->prefetch_end = true;
				break;
			}
			...
			/*
		 	 ...
		 	 * not currently in memory.
		 	 */
			bufHdr = BufferLookup(smgr->smgr_rnode.node, MAIN_FORKNUM,
								  InvalidPandoraPartLevel,
								  InvalidPandoraPartNumber,
								  InvalidPandoraPartGenNo,
								  reqPageId, strategy, &found);
			...
			if (found)
			{
				...
				/* Set I/O request information */
				ioInfo->buf_id = bufHdr->buf_id;
				ioInfo->req_page_id = reqPageId;
			}
			else
			{
				/* Set io request information */
				ioInfo->buf_id = InvalidTraditionalBufferID;
			 ...
			 */
			if ((found || idx + 1 == MAX_PAGE_IN_CHUNK) && page_io_num > 0)
			{
				if (found == false && idx + 1 == MAX_PAGE_IN_CHUNK)
					reqIdx = idx - page_io_num + 1;
				else
					reqIdx = idx - page_io_num;
				...
				if (found == false && idx + 1 == MAX_PAGE_IN_CHUNK)
					reqIoInfo->req_page_id = reqPageId - page_io_num + 1;
				else
				  	reqIoInfo->req_page_id = reqPageId - page_io_num;
			...
			/* Get next request page id. */
			prefetchDesc->next_reqblock = reqPageId + 1; 
		}

		/* Init read buffer description. */
		...
		/* Enqueue read buffer desc to complete list. */
		EnqueueReadBufDescToCompleteList(prefetchDesc, readBufDesc);
	}
}

/*
 * RetrieveIoCompletionFromIoUring
 *		We retrieve I/O completion from io_uring's completion queue. If 'wait' 
 *		is true, we call 'io_uring_wait_cqe' to avoid inefficient spin for
 *		checking I/O completion. If not, we call 'io_uring_peek_cqe' to check 
 *		I/O completion promptly.
 */
int
RetrieveIoCompletionFromIoUring(PrefetchDesc prefetchDesc, bool wait)
{
#if defined(STAT_PAGE_READ_IO) || defined(STAT_IO_URING)
	...
	while (prefetchDesc->complete_io_num < prefetchDesc->requested_io_num)
	{
		IoInfo ioInfo;
		...
		if (likely(!wait))
			ret = io_uring_peek_cqe(prefetchDesc->ring, &cqe);
		else
#if defined(STAT_PAGE_READ_IO) || defined(STAT_IO_URING)
		{
			if (isOLAP)
			...
			ret = io_uring_wait_cqe(prefetchDesc->ring, &cqe);
...
#ifdef STAT_PAGE_READ_IO
				elapsedTime += io_uring_diff;
#endif /* STAT_PAGE_READ_IO */

#ifdef STAT_IO_URING
				io_uring_waitTime += io_uring_diff;
#endif /* STAT_IO_URING */
			}
		}
#else /* !(STAT_PAGE_READ_IO || STAT_IO_URING) */
			ret = io_uring_wait_cqe(prefetchDesc->ring, &cqe);
#endif /* STAT_PAGE_READ_IO || STAT_IO_URING */
		...
		if (ret)
		{
			if (ret == -EAGAIN)
			{
				/* There is no completion from io_uring_peek_cqe. */
				break;
			}
			else if (ret == -EINTR)
			{
			...
			{
				elog(ERROR, "%s: get invalid return from io_uring, %d", 
					 prefetchDesc->prefetch_end == false ? "heap" : "ebi", ret);
			}
		}
		...
		/* Ok, there is a completion that we should process. */
		if (cqe->res <= 0)
		{
			elog(LOG, "%s: get invalid size from io_uring, %d", 
				 prefetchDesc->prefetch_end == false ? "heap" : "ebi", cqe->res);
			while(true) {sleep(1000);}
		}

		/* Get user data */
		ioInfo = io_uring_cqe_get_data(cqe);

		/* I/O is not complete yet. */
		Assert(ioInfo->is_complete == false);

		/* I/O is complete */
		ioInfo->is_complete = true;
 		prefetchDesc->complete_io_num += 1;
		completionNum += 1;

		/* Resource cleanup */
		io_uring_cqe_seen(prefetchDesc->ring, cqe);
	}
...
/*
 * ReadPartitionBufferPrefetch -- read the partition page and prefetch others
 * 
 * We read partition buffer page from traditional buffer or read buffer. We
 * prefetch required pages on future. We support prefetch feature on sequential
 * scan yet. 
 */
Buffer
ReadPartitionBufferPrefetch(Relation reln, BlockNumber blockNum,
							PrefetchDesc prefetchDesc,
							BufferAccessStrategy strategy, char** readBufferPage)
{
	Buffer traditionalBuffer;
...
recheck_io_request:
	 ...
	 * We do I/O request using io_uring.
	 */
	PopulatePartitionBuffersFromReadBuffer(reln->rd_node, prefetchDesc, strategy);
...
/*
 * PopulatePartitionBuffersFromReadBuffer
 *		We load partition pages into a read buffer chunk utilizing io_uring.
 *		If a buffer is found in memory, it is accessed directly. However, if it
 * 		isn't available, we initiate an I/O request using io_uring. 		 
 */
static void
PopulatePartitionBuffersFromReadBuffer(RelFileNode node,
									   PrefetchDesc prefetchDesc, 
									   BufferAccessStrategy strategy)
{
	bool					found;
	PrefetchPartitionDesc	pdesc = (PrefetchPartitionDesc)prefetchDesc;
	...
	while (PrefetchDescGetNeedPrefetch(prefetchDesc))
	{
		...
		int 			reqIdx = 0;
		uint32			page_io_num = 0;
		bool			onPandoraMempartBuf;

		List			   *c_list;
		ListCell		   *c_lc;

		PandoraPartInfo		c_part_info = (PandoraPartInfo)lfirst(pdesc->c_lc);
		PandoraPartLevel	partLvl = pdesc->c_level;
		PandoraPartNumber	partNum = c_part_info->partNum;
		PandoraPartGenNo	partGen = c_part_info->partGen;

		/* Get read buffer desc from free list */
		readBufDesc = PopReadBufDescFromFreeList(prefetchDesc);

		onPandoraMempartBuf = (partLvl == 0);
		...
		for (idx = 0; idx < MAX_PAGE_IN_CHUNK; idx++)
		{
			...
			if (unlikely(reqPageId == pdesc->c_block_cnt))
			{
				if (page_io_num > 0)
				{
					reqIdx = idx - page_io_num;
...
#ifdef STAT_PAGE_READ_IO_BREAKDOWN
					stat_is_valid_level[partLvl] = true;
					stat_heap_io_uring[partLvl] += reqIoInfo->page_io_num;
#endif /* STAT_PAGE_READ_IO_BREAKDOWN */

					/* If we can't find on buffer pool, do prefetch */
					PandoraRequest(prefetchDesc->ring, pdesc->c_fd,
								   &readBufDesc->iovs[reqIdx],
								   reqIoInfo->req_page_id, reqIoInfo->page_io_num,
								   reqIoInfo);
				...
				/* Set next ListCell and PandoraPartInfo */
				c_list = pdesc->part_infos_to_prefetch[partLvl];

				if ((c_lc = lnext(c_list, pdesc->c_lc)) == NULL)
				{
					if ((++partLvl) == pdesc->level_cnt)
					{
						prefetchDesc->prefetch_end = true;
						goto finish;
					}

					pdesc->c_level = partLvl;
					c_list = pdesc->part_infos_to_prefetch[partLvl];
					c_lc = list_head(c_list);
				}

				c_part_info = (PandoraPartInfo)lfirst(c_lc);
				...
				/* Close previous file */
				if (PandoraSimpleClose(pdesc->c_fd) == -1)
				{
					ereport(ERROR, errmsg("Failed to close pandora partition file during prefetching, name: %s",
										  path));
				}
				pfree(path);

				/* Set current state of pdesc */
				pdesc->c_lc = c_lc;
				pdesc->c_block_cnt = c_part_info->block_cnt;

				/* Get new partition number and generation number */
				partNum = c_part_info->partNum;
				partGen = c_part_info->partGen;
				
				/* Get file name */
				path = psprintf("base/%u/%u.%u.%u.%u",
								node.dbNode, node.relNode, partLvl, partNum, partGen);

				if ((pdesc->c_fd = PandoraSimpleOpen(path, O_RDWR | O_DIRECT)) == -1)
				{
					ereport(ERROR, errmsg("Failed to open pandora partition file during prefetching, name: %s",
										  path));
				}
				pfree(path);

				/* Set new range */
				reqPageId = prefetchDesc->next_reqblock =0;
				pdesc->c_block_cnt = c_part_info->block_cnt;
			}
			...
			/*
		 	 ...
		 	 * not currently in memory.
		 	 */
			bufHdr = BufferLookup(node, PANDORA_PARTITION_FORKNUM, partLvl,
								  partNum, partGen, reqPageId, strategy, &found);
			...
			if (found)
			{
				...
				/* Set I/O request information */
				if (onPandoraMempartBuf && PandoraCheckMempartBuf(bufHdr->buf_id))
					ioInfo->buf_id = bufHdr->buf_id + reqPageId;
				else
					ioInfo->buf_id = bufHdr->buf_id;
				ioInfo->req_page_id = reqPageId;
#ifdef STAT_PAGE_READ_IO_BREAKDOWN
...
/*
 * GetBufferFromIoCompletion
 * 		We take buffer from traditional buffer or read buffer. We distinguish 
 *		it from mark of 'IoInfo'.
 */
static bool
GetBufferFromIoCompletion(PrefetchDesc prefetchDesc, BlockNumber blockNum, 
						  char **readBufferPage, Buffer *traditionalBuffer)
{
	ReadBufDesc	readBufDesc;
	IoInfo 		ioInfo;
	...
	if (ioInfo->buf_id != InvalidTraditionalBufferID)
	{
		...
		/*
		 * We get next page from traditional buffer pool. 
		 */

		Assert(blockNum == ioInfo->req_page_id);
		...
		bufHdr = GetBufferDescriptor(ioInfo->buf_id);

		*traditionalBuffer = BufferDescriptorGetBuffer(bufHdr);
	}
	else
	{
		/*
		...
		Assert(blockNum == (ioInfo->req_page_id + readBufDesc->cur_page_idx));
		...
		*traditionalBuffer = InvalidBuffer;
	}
...
/*
 * InsertInProgressBuffer: insert buffer io info into hashtable
 *
 * We request async i/o on multiple block at the same time. For saving all
 * informations, we use hashtable structure. In this function, we get one
 * element from freelist and insert into hashtable.
 */
static void
InsertInProgressBuffer(BufferDesc *buf, bool forInput)
{
	uint32				idx;
	BufferIOInfo 	bufio_info;
	MemoryContext oldcontext;

	/* calculate hash code using buffer id */
	idx = DatumGetUInt32(hash_any((void *) &(buf->buf_id), sizeof(int))) %
						INPROGRESS_BUF_HASH_SIZE;

	if (BufferIOFreeList)
	{
		/* we can reuse from freelist */

		bufio_info = BufferIOFreeList;

		/* remove from freelist */
		BufferIOFreeList = bufio_info->next;
		bufio_info->next = NULL;
	}
	else
	{
		/* we should set CurrentMemoryContext */
		oldcontext = MemoryContextSwitchTo(TopMemoryContext);

		/* we can't reuse, do allocating */
		bufio_info = (BufferIOInfo) palloc(sizeof(struct BufferIOInfoData));

		MemoryContextSwitchTo(oldcontext);
	}

	/* init buffer io info */
	bufio_info->buf_hdr = buf;	
	bufio_info->isForInput = forInput;
	bufio_info->next = NULL;

	/* append to chain */
	bufio_info->next = InProgressBufHash[idx];
	InProgressBufHash[idx] = bufio_info;
}

/*
 * DeleteInProgressBuffer: delete buffer io info from hashtable
 *
 * We request async i/o on multiple block at the same time. When block's io is
 * done, we should signal to others to wake up waiting processes.
 */
static void
DeleteInProgressBuffer(BufferDesc *buf)
{
	uint32				idx;
	BufferIOInfo  iter;
	BufferIOInfo  prev_iter;
	...
	prev_iter = NULL; 
	iter = InProgressBufHash[idx];

	Assert(iter != NULL);

	/* search and delete buffer io info from hashtable */
	for (;;)
	{
		if (iter->buf_hdr == buf)
		{
			/* we find it, just delete it */
			...
			{
				/* first element */
				InProgressBufHash[idx] = iter->next;
			}
		...
		/* we should find element which is valid */
		if (iter->next == NULL)
			elog(ERROR, "terminate buffer io info is not invalid");
	...
	/* add to freelist */
	iter->next = BufferIOFreeList;
	BufferIOFreeList = iter;
}

/*
 * ClearInProgressBufferHash: clear all buffer io info for abort process
 *
 * We insert and delete elements with hashtable when using async i/o. 
 * When abort occur, we should remove all buffer io info to yield our buffer
 * blocks to others. In this, we iterate all element and do termination one by
 * one.
 */
static void
ClearInProgressBufferHash(void)
{
	BufferIOInfo	next_bufio_info;
	BufferIOInfo	bufio_info;
	BufferDesc 	*buf;
	uint32 			idx;
	uint32		buf_state;

	/* find all buffer io info and abort */

	/* 
	 * search all hash bucket from 0 to (INPROGRESS_BUF_HASH_SIZE - 1)
	 * incremently.
	 */
	for (idx = 0; idx < INPROGRESS_BUF_HASH_SIZE; idx++)
	{
		bufio_info = InProgressBufHash[idx];

		/* 
		 * search all hash element on same bucket.
		 */
		while (bufio_info != NULL)
		{
			bool is_for_input;

			/* save next element in advance */
			next_bufio_info = bufio_info->next;

			buf = bufio_info->buf_hdr;
			is_for_input = bufio_info->isForInput;
			...
			/* move to next */
			bufio_info = next_bufio_info;
		}
	}
}

/*
 * BuferAlloc() for PANDORA's level 0 inserting.
 */
static BufferDesc *
PandoraMempartBufferAllocForInsert(SMgrRelation smgr,
			char relpersistence, ForkNumber forkNum,
			PandoraPartLevel partLvl, PandoraPartNumber partNum,
			PandoraPartGenNo partGen, BlockNumber blockNum,
			BufferAccessStrategy strategy,
			bool *foundPtr, bool is_pandora_new_mempart)
{
	BufferTag	newTag;			/* identity of requested block */
	uint32		newHash;		/* hash value for newTag */
	LWLock	   *newPartitionLock;	/* buffer partition lock for it */
	...
	bool		valid;
	uint32		buf_state;
	PandoraRelationMeta *victim_meta = NULL;
	int			tried = 0;

	Assert(partLvl == 0);

	/* Set the block number to the start of mempartition */
	blockNum = 0;
...
retry:
	...
	/*
	 * Mempartition is found.
	 */
	if (buf_id >= PandoraMempartStart)
	{
		/* It must be located in mempart buffer */
		Assert(buf_id >= PandoraMempartStart);

		/* The given buffer id must be a start of mempartition */
		Assert((buf_id - PandoraMempartStart) % PandoraMempartPageCount == 0);

		/* We are not creator for this mempart */
		Assert(!is_pandora_new_mempart);
	...
	/*
	 * (1) Inserts must use mempartition buffer. So if the buffer is located in
	 * traditional buffer, caller must retry insert by changing partNum and
	 * blockNum. To notify it, return NULL.
	 *
	 * (2) Hash table has no entry about this mempartition. But this try is not
	 * about creating new partition which means that this mempartition is
	 * populated already. In this case, we also have to notify it to the caller.
	 */
	else if (!is_pandora_new_mempart)
	{
		LWLockRelease(newPartitionLock);
		...
		/*
		 * Sleep for a while instead of increasing the lock contention of the
		 * hash table.
		 */
		pg_usleep(50);
		goto retry;
	...
	/* Only the creator for the mempart can reach here */
	Assert(is_pandora_new_mempart);
	...
	for (;;)
	{
		...
		/*
		 * Select a victim buffer using PANDORA's rule.
		 */
		buf = PandoraStrategyGetBuffer(&buf_state);

		/* It must be located in mempart buffer */
		Assert(buf->buf_id >= PandoraMempartStart);

		/* The given buffer id must be a start of mempartition */
		Assert((buf->buf_id - PandoraMempartStart) % PandoraMempartPageCount == 0);
		...
		/*
		 * Befor flush, get the real number of blocks in victim mempart.
		 * Here we do not modify meta's information. Just read the record count
		 * and record counts per block to calculate partition size.
		 */
		victim_meta = PandoraGetRelationMeta(buf->tag.rnode.relNode);

		if (victim_meta != NULL)
		{
#ifdef PANDORA_DEBUG
			fprintf(stderr, "[PANDORA] evict, rel: %u, cnt: %u, buf: %p, pid: %d\n",
							buf->tag.rnode.relNode,
							pg_atomic_read_u32(&(buf->inserted_cnt)),
							buf,
							(int)getpid());
#endif /* PANDORA_DEBUG */
			/* If this mempartition is not fully filled, skip it */
			if (pg_atomic_read_u32(&(buf->inserted_cnt)) <
				victim_meta->recordNumsPerBlock * PandoraMempartPageCount)
			{
				UnpinBuffer(buf, true);
				victim_meta = NULL;
			...
			Assert(pg_atomic_read_u32(&(buf->inserted_cnt)) ==
				   victim_meta->recordNumsPerBlock * PandoraMempartPageCount);
		}
		...
		if (oldFlags & BM_DIRTY)
		{
			int mempart_id = PandoraGetMempartIdFromBufId(buf->buf_id);
			Assert(mempart_id >= 0);
			Assert(victim_meta != NULL);

			/* Avoid inserts and updates */
			if (!LWLockConditionalAcquire(PandoraGetMempartLock(mempart_id),
										  LW_EXCLUSIVE))
			{
				UnpinBuffer(buf, true);
				victim_meta = NULL;
										 ...
										 LW_SHARED))
			{
				...
				fprintf(stderr, "[PANDORA] WARNING!!!: INSERT worker flushes dirty mempartition!!\n");
				...
				PandoraFlushMempartBuffer(buf, PandoraMempartPageCount);
				LWLockRelease(BufferDescriptorGetContentLock(buf));
		...
		{
			/*
			 * Allocated mempartitions are always dirty.
			 * So victim_meta cannot NULL.
			 */
			Assert(victim_meta != NULL);
		...
		{
			/*
			 * If this mempartition buffer not allocated for any file yet,
			 * there is no evicition.
			 */
			Assert(victim_meta == NULL);
		...
		if (buf_id >= 0)
		{
			...
			*foundPtr = true;
			...
			/*
			 * If this buffer is located in mempartition buffer, the given buffer id
			 * must be a start of mempartition.
			 */
			if (buf_id >= PandoraMempartStart)
				Assert((buf_id - PandoraMempartStart) % PandoraMempartPageCount == 0);
		...
		/* We do not evict, so this lock is meaningless */
		if (victim_meta)
			victim_meta = NULL;
	...
	/* Init the count of inserted records */
	pg_atomic_init_u32(&(buf->inserted_cnt), 0);
#ifdef PANDORA_DEBUG
	fprintf(stderr, "[PANDORA] init, rel: %u, cnt: %u, buf: %p, pid: %d\n",
...
/*
 * BufferAlloc() for PANDORA's level0 buffers (traditional + mempart).
 */
static BufferDesc *
PandoraBufferAllocLevel0(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
			PandoraPartLevel partLvl, PandoraPartNumber partNum,
			...
			bool is_pandora_new_mempart)
{
	...
	bool		isWritten;
	BlockNumber originalBlockNum = blockNum;
	bool		find_mempart = true;

	Assert(partLvl == 0);

	/* ----------- Divide insert operations to reduce complexity ----------- */
	if (is_pandora_mempart_insert)
		return PandoraMempartBufferAllocForInsert(smgr, relpersistence, forkNum,
										 partLvl, partNum, partGen, blockNum,
										 strategy, foundPtr,
										 is_pandora_new_mempart);
	/* --------------------------------------------------------------------- */

	/* At first, find the buffer in mempartition buffer */
	pg_memory_barrier();
	blockNum = 0;
...
find_hash_table:
	...
	if (buf_id >= 0)
	{
		...
		*foundPtr = true;
		...
		if (!valid)
		{
			...
			if (StartBufferIO(buf, true))
			{
				Assert(!PandoraCheckMempartBuf(buf_id));
	...
	/*
	 * Below codes must be executed only for traditional buffer pool which means
	 * that updators and scanners who want to use mempart can consider only the
	 * above codes.
	 */
	Assert(find_mempart == false);

	/* ------- Below code is executed only for traditional buffer pool -------- */
...
/*
 * FlushBuffer() for PANDORA's mempart
 */
bool
PandoraFlushMempartBuffer(BufferDesc *buf, int nblock)
{
	bool		isWritten = true;
	XLogRecPtr	recptr;
	ErrorContextCallback errcallback;
	 ...
	 * bufToWrite is either the shared buffer or a copy, as appropriate.
	 */
	isWritten = PandoraMempartWrite(reln->smgr_rnode.node,
							buf->tag.partLvl,
							buf->tag.partNum,
							buf->tag.partGen,
							bufBlock, nblock);
...
void
IncrInsertedRecordsCount(Buffer buffer)
{
	BufferDesc *buf = GetBufferDescriptor(PandoraGetMempartBufId(buffer - 1));
	pg_atomic_fetch_add_u32(&(buf->inserted_cnt), 1);
}

uint32
GetInsertedRecordsCount(Buffer buffer)
{
	BufferDesc *buf = GetBufferDescriptor(PandoraGetMempartBufId(buffer - 1));
	return pg_atomic_read_u32(&(buf->inserted_cnt));
}

/*
 * PandoraMempartionFlush -- Write out all dirty mempartitions in the pool.
 */
void
PandoraMempartionFlush()
{
	int			trycount = 0;
	uint32		bufState;
	int			mempartId;
	PandoraRelationMeta *meta;
	LWLock		*metaMemPartitionLock;
	LWLock		*mempartitionLock;
	int			nblock;
	BufferDesc *bufHdr;
	uint32		oldFlags;
	uint32		victimHash;		/* hash value for newTag */
	LWLock		*victimPartitionLock;
	uint32		mask;
	PandoraPartNumber reorganizedPartNumber, currentPartNumber;
	...
	mask = (BM_DIRTY | BM_PERMANENT);

retry_mempartition_flush:
	if (trycount++ > MEMPARTITION_FLUSH_RETRY_MAX)
		return;

	/* Get victim mempartition buffer with unlocked buffer. */
	bufHdr = PandoraStrategyGetVictimMempartitionBuffer(&bufState);

	/* Skip invalid buffer */
	if (bufHdr->tag.rnode.relNode == InvalidOid)
	{
		UnlockBufHdr(bufHdr, bufState);
		goto retry_mempartition_flush;
	}

	/* Unlocked */
	PinBuffer_Locked(bufHdr);

	/* Get meta mempartition lock */
	meta = PandoraGetRelationMeta(bufHdr->tag.rnode.relNode);
	metaMemPartitionLock = PandoraRelationMetaMemPartitionLock(meta);

	/*
	 * Before flush, get the real number of blocks in dirty mempart.
	 * Here we do not modify meta's information. Just read the record
	 * count and record counts per block to calculate partition size.
	 */
	LWLockAcquire(metaMemPartitionLock, LW_SHARED);
	reorganizedPartNumber = meta->partitionNumberLevelZero[0];
	currentPartNumber = meta->partitionNumberLevelZero[1];
	LWLockRelease(metaMemPartitionLock);

	/*
	 * If this mempartition is the most recent and the current
	 * checkpointing is not for shutdown, don't flush, just skip.
	 * Repartitioning worker determines the count of target record using
	 * lseek() first.
	 * So if the checkpointing flushes the most mempartition during in
	 * progress, repartitioning worker may get wrong count.
	 */
	if (pg_atomic_read_u32(&(bufHdr->inserted_cnt)) < 
			meta->recordNumsPerBlock * PandoraMempartPageCount)
	{
		UnpinBuffer(bufHdr, true);
		goto retry_mempartition_flush;
	...
	mempartId = PandoraGetMempartIdFromBufId(bufHdr->buf_id);
	mempartitionLock = PandoraGetMempartLock(mempartId);
	
	/* Calculate the # blocks of target mempartition. */
	nblock = PandoraMempartPageCount;

	/* Avoid inserts and updates */
	LWLockAcquire(mempartitionLock, LW_EXCLUSIVE);

	if ((bufState & mask) == mask)
	{
		/* Pin it, share-lock it, write it. */
		LWLockAcquire(BufferDescriptorGetContentLock(bufHdr), LW_SHARED);
		PandoraFlushMempartBuffer(bufHdr, nblock);
		LWLockRelease(BufferDescriptorGetContentLock(bufHdr));
	}

	/* Release mempart lock */
	LWLockRelease(mempartitionLock);

	/* 
	 * If the partition was already reorganized by worker, we put this buffer to 
	 * free buffer list. 
	 */
	if (bufHdr->tag.partNum <= reorganizedPartNumber)
	{
		/* determine its hash code and partition lock ID */
		victimHash = BufTableHashCode(&bufHdr->tag);
		victimPartitionLock = BufMappingPartitionLock(victimHash);
		LWLockAcquire(victimPartitionLock, LW_EXCLUSIVE);
		 ...
		 * over with a new victim buffer.
		 */
		oldFlags = bufState & BUF_FLAG_MASK;
		if (BUF_STATE_GET_REFCOUNT(bufState) != 1 || (oldFlags & BM_DIRTY))
		{
			LWLockRelease(mempartitionLock);
			UnlockBufHdr(bufHdr, bufState);
			UnpinBuffer(bufHdr, true);
		...
		BufTableDelete(&bufHdr->tag, victimHash);

		/* Clear buffer tag. */
		CLEAR_BUFFERTAG(bufHdr->tag);

		bufState &= ~(BM_VALID | BM_TAG_VALID | BM_DIRTY | BM_JUST_DIRTIED |
				   BM_CHECKPOINT_NEEDED | BM_IO_ERROR | BM_PERMANENT |
				   BUF_USAGECOUNT_MASK);

		LWLockRelease(mempartitionLock);
		LWLockRelease(victimPartitionLock);
		UnlockBufHdr(bufHdr, bufState);
		UnpinBuffer(bufHdr, true);

		/* Return to free buffer. */
		PandoraStrategyFreeBuffer(bufHdr);
	}
	else
	{
		/* Just unpin buffer. */
