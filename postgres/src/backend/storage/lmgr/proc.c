...
#ifdef PANDORA
static void pandora_segfault_handler(int sig);
static void pandora_abort_handler(int sig);
static pqsigfunc default_SIGSEGV;
static pqsigfunc default_SIGABRT;

static void
pandora_segfault_handler(int sig)
{
	ereport(LOG, (errmsg("@@@@@ %d segfault, process", getpid())));
	sleep(60);
	default_SIGSEGV(sig);
}

static void
pandora_abort_handler(int sig)
{
	ereport(LOG, (errmsg("@@@@@ %d abort, process", getpid())));
	sleep(60);
	default_SIGABRT(sig);
}
#endif /* PANDORA */
...
InitProcess(void)
{
	...
	if (IsUnderPostmaster && 
			...
			!IsPandoraPartitionLauncherProcess())
#else
	...
	default_SIGSEGV = pqsignal(SIGSEGV, pandora_segfault_handler);
	default_SIGABRT = pqsignal(SIGABRT, pandora_abort_handler);
#endif /* PANDORA */
}
