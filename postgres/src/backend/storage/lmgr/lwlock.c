...
static const char *const BuiltinTrancheNames[] = {
	...
	"LockManager",
#ifdef PANDORA
	/* LWTRANCHE_RELATION_META_MAPPING: */
	"PandoraRelationMetaMapping",
...
InitializeLWLocks(void)
{
	...
	lock = MainLWLockArray + PLEAF_MAPPING_LWLOCK_OFFSET;
	for (id = 0; id < NUM_PLEAF_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_PLEAF_MAPPING);

  /* Initialize ebi-sub mapping LWLocks in main array */
  lock = MainLWLockArray + EBI_SUB_MAPPING_LWLOCK_OFFSET;
  for (id = 0; id < NUM_EBI_SUB_PARTITIONS; id++, lock++)
    LWLockInitialize(&lock->lock, LWTRANCHE_EBI_SUB_MAPPING);
#endif /* DIVA */
...
#ifdef PANDORA
	/* Initialize relation meta LWLocks in main array */
	lock = MainLWLockArray + PANDORA_RELATION_META_MAPPING_LWLOCK_OFFSET;
	for (id = 0; id < NUM_PANDORA_RELATION_META_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_RELATION_META_MAPPING);
#endif /* PANDORA */
