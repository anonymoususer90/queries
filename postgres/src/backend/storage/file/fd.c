...
#ifdef PANDORA
int
FileRequest(struct io_uring* ring, File file, struct iovec *iovs, 
							off_t seekpos, int req_page_num, void *io_info)
{
	int			returnCode;
	Vfd		   *vfdP;
	...
	fcntl(F_SETFD, vfdP->fd, O_DIRECT);

	/* set read request */
	io_uring_prep_readv(sqe, vfdP->fd, iovs, req_page_num, seekpos);
...
#ifdef PANDORA
/*
 * PandoraSimpleOpen --- same as BasicOpenFilePerm except doesn't use VFD
 *
 * This is exported for use by places that really want a plain kernel FD,
 * but need to be proof against running out of FDs.  Once an FD has been
 * successfully returned, it is the caller's responsibility to ensure that
 * it will not be leaked on ereport()! This doesn't use VFD abstraction level,
 * but covers some elements(nfile, LRU) of it, so as not to spoil the FD
 * mangement logic inside it.
 * 
 * Because PANDORA partitions relation data into multiple files, there is a risk
 * of unnecessarily increasing the size of smgr and VfdCache if PostgreSQL's I/O
 * logic is used as is. To avoid this, backend that references partition files
 * doesn't cache the fd at the VFD abstraction level, but call this whenever
 * necessary to open it directly. (The same goes for close())
 */
int
PandoraSimpleOpen(const char *fileName, int fileFlags)
{
	int			fd;

	/* Close excess kernel FDs. */
	/* Call this to avoid spoiling the VFD's logic */
	ReleaseLruFiles();
	...
	fd = open(fileName, fileFlags & ~PG_O_DIRECT, pg_file_create_mode);
#else
	fd = open(fileName, fileFlags, pg_file_create_mode);
#endif
...
#endif
		/* Count the nfile to avoid spoiling VFD's logic */
		++nfile;
	...
	if (errno == EMFILE || errno == ENFILE)
	{
		ereport(LOG,
				(errcode(ERRCODE_INSUFFICIENT_RESOURCES),
				 errmsg("out of file descriptors: PANDORA")));
	}
...
/*
 * PandoraSimpleClose --- close a file opened via PandoraSimpleOpen().
 *
 * When the backend opens a file partitioned by PANDORA, it opens it via
 * PandoraSimpleOpen() without using the VFD abstraction level, so files opened
 * that way should also be safely closed via this function.
 * (See PandoraSimpleOpen() for details)
 */
int
PandoraSimpleClose(int fd)
{
	int result = close(fd);
	...
	if (result != 0)
	{
		elog(LOG, "could not close file (fd: %d): PANDORA", fd);
		return -1;
	}
