...
#ifdef PANDORA
/*
 * pandoraam_index_fetch_tuple
 */
static bool
pandoraam_index_fetch_tuple(struct IndexFetchTableData *scan,
						 ItemPointer tid,
						 Snapshot snapshot,
						 TupleTableSlot *slot,
						 bool *call_again, bool *all_dead)
{
	IndexFetchHeapData *hscan = (IndexFetchHeapData *) scan;
	BufferHeapTupleTableSlot *bslot = (BufferHeapTupleTableSlot *) slot;
	PandoraFingerprint fingerprint = slot->tts_pandora_fingerprint;
	PandoraRelationMeta *meta;
	bool got_partition_tuple;
	DualRefDescData	drefDesc;
	PandoraTuplePositionData tuplePosition;
	Relation relation;

	Assert(IsSiro(hscan->xs_base.rel) && IsPandora(hscan->xs_base.rel));
	...
	/* Get a tuple position information from fingerprint */
	PandoraFingerprintGetTuplePositionForIndexscan(meta, hscan->xs_base.rel, 
												   fingerprint, &tuplePosition);
	...
	if (!*call_again)
	{
		...
		hscan->xs_cbuf = PandoraReleaseAndReadBuffer(hscan->xs_cbuf,
											  hscan->xs_base.rel,
	...
	/* 
	 * Retrieve a tuple using the tuple position obtained through the 
	 * fingerprint. 
	 */
	got_partition_tuple = pandora_search_version(&tuplePosition,
												hscan->xs_base.rel,
												hscan->xs_cbuf,
												snapshot,
												&bslot->base.tupdata,
...
#ifdef PANDORA
/*
 * Search the visible version with fingerprint. (similar with index search)
 * Referenced heapam_index_fetch_tuple() & heapam_fetch_fow_version
 */
static bool
pandoraam_search_row_version(Relation relation,
							 ItemPointer tid,
							 Snapshot snapshot,
							 TupleTableSlot *slot)
{
	...
	PandoraFingerprint fingerprint = slot->tts_pandora_fingerprint;
	CmdType old_cmdtype = curr_cmdtype;
	PandoraRelationMeta *meta = PandoraGetRelationMeta(relation->rd_node.relNode);
	...
	tuplePosition = (PandoraTuplePosition) palloc0(sizeof(PandoraTuplePositionData));

	/* Find target location. */		
	PandoraFingerprintGetTuplePositionForUpdate(meta, relation, 
												fingerprint, tuplePosition);
					...
					int options, BulkInsertState bistate)
{
	...
	struct timespec stat_tpcc_breakdown_insert_heap_insert_start;
	struct timespec stat_tpcc_breakdown_insert_heap_insert_end;
#endif /* STAT_TPCC */
...
#ifdef PANDORA
	tuple->t_pandora_fingerprint = NULL;
#endif /* PANDORA */

#ifdef STAT_TPCC
	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_heap_insert_start);
#endif /* STAT_TPCC */
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_heap_insert_end);
...
#ifdef PANDORA
static TM_Result
pandoraam_tuple_update(Relation relation, ItemPointer otid, 
					PandoraTuplePosition tuplePosition, TupleTableSlot *slot,
					CommandId cid, Snapshot snapshot, Snapshot crosscheck,
					bool wait, TM_FailureData *tmfd,
					LockTupleMode *lockmode, bool *update_indexes)
{
	...
	result = heap_update_with_vc(relation, otid, tuplePosition, tuple, cid, 
				 ...
				 offnum = OffsetNumberNext(offnum))
			{
				...
				*/
				...
				else if (ret_id == -2)
				{
					...
					/* 
					 * The visible version is located in the EBI page, and we 
					 * will access it at EBI scan phase. 
					 */
					InsertUnobtainedVersionOffset(hscan, version_offset);
				}
...
static void
pandoraam_tuple_insert(Relation relation, TupleTableSlot *slot, CommandId cid,
					int options, BulkInsertState bistate)
{
	bool		shouldFree = true;
	HeapTuple	tuple = ExecFetchSlotHeapTuple(slot, true, &shouldFree);
	...
	slot->tts_tableOid = RelationGetRelid(relation);
	tuple->t_tableOid = slot->tts_tableOid;
	tuple->t_pandora_fingerprint = slot->tts_pandora_fingerprint;

	Assert(relation->rd_pandora_level_count == slot->lvl_cnt);

	/* Perform the insertion, and copy the resulting ItemPointer */
	pandora_insert(relation, tuple, cid, options, bistate);
	ItemPointerCopy(&tuple->t_self, &slot->tts_tid);
 ...
 * ------------------------------------------------------------------------
 */
static const TableAmRoutine pandoraam_methods = {
	.type = T_TableAmRoutine,

	.slot_callbacks = heapam_slot_callbacks,

	.scan_begin = pandora_beginscan,
	.scan_end = pandora_endscan,
	.scan_rescan = pandora_rescan,
	.scan_getnextslot = pandora_getnextslot,
	.scan_getnextslot_from_ebi = ebi_getnextslot,
	...
	.index_fetch_reset = heapam_index_fetch_reset,
	.index_fetch_end = heapam_index_fetch_end,
	.index_fetch_tuple = pandoraam_index_fetch_tuple,

	.tuple_insert = pandoraam_tuple_insert,
	.tuple_insert_speculative = heapam_tuple_insert_speculative,
	.tuple_complete_speculative = heapam_tuple_complete_speculative,
	.multi_insert = heap_multi_insert,
	.tuple_delete = heapam_tuple_delete,
	...
	/* Update tuple */
	.tuple_update_for_pandora_partition = pandoraam_tuple_update,

	.tuple_lock = heapam_tuple_lock,

	.tuple_fetch_row_version = heapam_fetch_row_version,
	.tuple_search_row_version = pandoraam_search_row_version,
...
const TableAmRoutine *
GetPandoraamTableAmRoutine(void)
{
	return &pandoraam_methods;
}

Datum
pandoraam_handler(PG_FUNCTION_ARGS)
{
	PG_RETURN_POINTER(&pandoraam_methods);
}
