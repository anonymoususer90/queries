...
_bt_doinsert(Relation rel, IndexTuple itup,
			 ...
			 Relation heapRel, int32 lvl_cnt,
			 PandoraFingerprint pandora_fingerprint)
#else
bool
_bt_doinsert(Relation rel, IndexTuple itup,
...
search:
	...
	if (checkUnique != UNIQUE_CHECK_EXISTING)
	{
		...
		_bt_insertonpg(rel, itup_key, insertstate.buf, InvalidBuffer, stack,
					   ...
					   insertstate.postingoff, false, pandora_fingerprint);
#else
		_bt_insertonpg(rel, itup_key, insertstate.buf, InvalidBuffer, stack,
					   itup, insertstate.itemsz, newitemoff,
...
_bt_check_unique(Relation rel, BTInsertState insertstate, Relation heapRel,
				 IndexUniqueCheck checkUnique, bool *is_unique,
				 PandoraFingerprint new_fingerprint,
				 uint32 *speculativeToken)
#else /* !PANDORA */
static TransactionId
_bt_check_unique(Relation rel, BTInsertState insertstate, Relation heapRel,
...
#ifdef PANDORA
					fingerprint = GetPandoraFingerprint(curitup);
#endif /* PANDORA */
				}
...
#endif /* PANDORA */
{
	...
	{
#ifdef PANDORA
		bool isPandora = (pandora_fingerprint != NULL);
#endif
		Buffer		rbuf;
		...
		rbuf = _bt_split(rel, itup_key, buf, cbuf, newitemoff, itemsz, itup,
						origitup, nposting, isPandora, pandora_fingerprint, 
																postingoff);
#else
		/* split the buffer into left and right halves */
		rbuf = _bt_split(rel, itup_key, buf, cbuf, newitemoff, itemsz, itup,
	...
	else
	{
		...
		{
			if (PageAddItemWithFingerprint(page, (Item) itup, itemsz, newitemoff,
									   false, false, itup->t_fingerprint_offset,
									   pandora_fingerprint) == InvalidOffsetNumber)
...
_bt_split(Relation rel, BTScanInsert itup_key, Buffer buf, Buffer cbuf,
		  ...
		  IndexTuple orignewitem, IndexTuple nposting, 
#ifdef PANDORA
		  bool isPandora,
		  PandoraFingerprint pandora_fingerprint,
#endif
		  uint16 postingoff)
{
	...
	for (i = P_FIRSTDATAKEY(oopaque); i <= maxoff; i = OffsetNumberNext(i))
	{
		...
		{
#ifdef PANDORA
			if (!_bt_pgaddtup(leftpage, itemsz, dataitem, afterleftoff, 
								isPandora, NULL, false))
#else
				  ...
				  bool isonly)
{
	...
	else
	{
		...
		_bt_insertonpg(rel, NULL, pbuf, buf, stack->bts_parent,
					   ...
					   stack->bts_offset + 1, 0, isonly, NULL);
#else
		_bt_insertonpg(rel, NULL, pbuf, buf, stack->bts_parent,
					   new_item, MAXALIGN(IndexTupleSize(new_item)),
			 ...
			 bool newfirstdataitem)
{
...
#ifdef PANDORA
	if (isPandora && (pandora_fingerprint == NULL))
	{
		pandora_fingerprint = (PandoraFingerprint)
	...
	if (isPandora)
	{
		if (unlikely(PageAddItemWithFingerprint(page, (Item) itup, itemsize, itup_off,
								false, false, itup->t_fingerprint_offset,
								pandora_fingerprint) == InvalidOffsetNumber))
			return false;
	}
