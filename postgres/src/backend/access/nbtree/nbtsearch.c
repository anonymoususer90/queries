...
#ifdef PANDORA
						, bool isPandora, int lastPartitionLevel, int mempart_rec_cnt
#endif
						);
...
#ifdef PANDORA
	if (scan->xs_heapfetch && IsPandora(scan->xs_heapfetch->rel))
		scan->xs_fingerprint = &(currItem->fingerprint);
#endif
...
_bt_readpage(IndexScanDesc scan, ScanDirection dir, OffsetNumber offnum)
{
	...
	int			mempart_rec_cnt = 0;

	if (scan->xs_heapfetch)
	{
		isPandora = IsPandora(scan->xs_heapfetch->rel);

		if (isPandora)
			mempart_rec_cnt = (scan->xs_heapfetch->rel->records_per_block * PandoraMempartPageCount);
	}
#endif
			...
			if (_bt_checkkeys(scan, itup, indnatts, dir, &continuescan))
			{
				...
				if (!BTreeTupleIsPosting(itup))
				{
					...
					_bt_saveitem(so, itemIndex, offnum, itup, 
										isPandora, scan->lastPartitionLevel,
										mempart_rec_cnt);
#else
					_bt_saveitem(so, itemIndex, offnum, itup);
#endif
			...
			)
{
	...
	{
#if 0
		PandoraFingerprint fingerprint = (PandoraFingerprint) ((char *) itup + itup->t_fingerprint_offset);

		if (fingerprint->rec_key.seqnum > (mempart_rec_cnt * fingerprint->rec_pos.first_lvl_partnum +
										   fingerprint->rec_pos.first_lvl_pos))
		{
			fprintf(stderr, "[INDEX SCAN] partid %d, seqnum %ld, first_lvl_partnum %u, first_lvl_pos %u, last_lvl_pos: %lu, in_partition_counts[0] %u, in_partition_counts[1] %u\n",
					fingerprint->rec_key.partid,
					fingerprint->rec_key.seqnum,
					fingerprint->rec_pos.first_lvl_partnum,
					fingerprint->rec_pos.first_lvl_pos,
					fingerprint->rec_pos.last_lvl_pos,
					fingerprint->rec_pos.in_partition_counts[0].cumulative_count,
					fingerprint->rec_pos.in_partition_counts[1].cumulative_count);
			Abort("seqnum > mempart_rec_cnt * first_lvl_partnum + first_lvl_pos");
		}
#endif

		memcpy(((char *) currItem) + offsetof(BTScanPosItem, fingerprint), 
				((char *) itup) + itup->t_fingerprint_offset, 
				SizeOfPandoraFingerprintData(lastPartitionLevel + 1));
	}
#endif 
