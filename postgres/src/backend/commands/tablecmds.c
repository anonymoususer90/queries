...
#include "hap/hap_catalog.h"
#include "hap/hap_hook.h"
 ...
 */
#ifdef HAP
HAP_HOOK5(ObjectAddress, DefineRelation,
 ...
 */
#ifdef HAP
HAP_HOOK9(void, createForeignKeyCheckTriggers,
 ...
 */
#ifdef HAP
HAP_HOOK4(void, ATExecSetRelOptions,
		  Relation, rel,
		  List *, defList,
		  AlterTableType, operation,
		  LOCKMODE, lockmode)
...
/*
 * HapCreateAndOpenRel
 *		Original DefineRelation() except partition related tasks.
 *
 * All operations of this function are identical to DefineRelation(). This
 * function is called after HAP-only tasks have already been processed in the
 * caller.
 *
 * Note that while DefineRelation() also performs partition related tasks,
 * this function has no operations related partitions. This is because HAP has
 * its own logic to partitions.
 */
static Relation
HapCreateAndOpenRel(CreateStmt *stmt, char relkind, Oid ownerId,
					ObjectAddress *typaddress, const char *queryString)
{
	char		relname[NAMEDATALEN];
	Oid			namespaceId;
	...
	/*
	 * Parse and validate reloptions, if any.
	 * Note that reloptions for the HAP are already extracted from caller.
	 */
	reloptions = transformRelOptions((Datum) 0, stmt->options, NULL, validnsps,
		true, false);
	...
	/* Unlike the original DefineRelation(), PARTITIONED also has AM */
	Assert(stmt->accessMethod != NULL);
...
/*
 * HapAddHiddenAttr
 *		Add hidden attribute to the schema.
 *
 * Since we don't know how many predicates the user will encode, we define the
 * hidden attribute as a variable-length byte array.
 *
 * Note that we set default value to the hidden attribute.
 * This makes ExecInsert() to consider the hidden attribute automatically.
 */
static void
HapAddHiddenAttr(CreateStmt *stmt)
{
	ColumnDef *hiddenAttr;
	A_Const *raw_default;
	char colname[64];

	sprintf(colname, "_hap_hidden_attribute");
	hiddenAttr = makeColumnDef(colname, BYTEAOID, -1, 0);

	/* Set default */
	raw_default = makeNode(A_Const);
	raw_default->val.sval.type = T_String;
	raw_default->val.sval.sval = palloc0(1);
	hiddenAttr->raw_default = (Node *)raw_default;

	/* Add it to the schema */
	stmt->tableElts = lappend(stmt->tableElts, hiddenAttr);
}

/*
 * Set the relishap flag to true for the given relation.
 */
static void
HapMarkPgClass(Relation rel)
{
	Relation pg_class;
	HeapTuple tuple;
	Form_pg_class rd_rel;
...
/*
 * Condition to hook the DefineRelation().
 */
HAP_DEFINE_HOOK_COND5(ObjectAddress, DefineRelation,
					  CreateStmt *, stmt, char, relkind, Oid, ownerId,
				  	  ObjectAddress *, typaddress, const char *, queryString)
{
	if (HapCheckAmUsingName(stmt->accessMethod))
		return true;
 ...
 */
HAP_DEFINE_HOOK_BODY5(ObjectAddress, DefineRelation,
					  CreateStmt *, stmt, char, relkind, Oid, ownerId,
				  	  ObjectAddress *, typaddress, const char *, queryString)
{
	ObjectAddress address;
	...
	if (!stmt->partbound)
		HapAddHiddenAttr(stmt);

	/* Do the original DefineRelation() things */
	rel = HapCreateAndOpenRel(stmt, relkind, ownerId, typaddress, queryString);

	/* Insert new pg_hap entry */
	HapInsertRelTuple(rel, 0, 0);

	/* Mark this table as HAP in pg_class */
	HapMarkPgClass(rel);
...
/*
 * HapCreateFKCheckTrigger
 *		Creates the insert or update "check" trigger that implements a given
 *		foreign key.
 *
 * Unlike the basic check triggers, HAP uses an INSERT BEFORE trigger and uses a
 * separate trigger function.
 */
static Oid
HapCreateFKCheckTrigger(Oid myRelOid, Oid refRelOid, Constraint *fkconstraint,
					 	Oid constraintOid, Oid indexOid, Oid parentTrigOid,
					 	bool on_insert)
{
	ObjectAddress trigAddress;
	...
	fk_trigger->replace = false;
	fk_trigger->isconstraint = true;
	fk_trigger->trigname = "HAP_RI_ConstraintTrigger_c";
	fk_trigger->relation = NULL;
	...
	if (on_insert)
	{
		...
		fk_trigger->events = TRIGGER_TYPE_INSERT;
		fk_trigger->timing = TRIGGER_TYPE_BEFORE;
	}
	else
	{
		fk_trigger->funcname = SystemFuncName("RI_FKey_check_upd");
...
/*
 * Condition to hook the createForeignKeyCheckTriggers().
 */
HAP_DEFINE_HOOK_COND9(void, createForeignKeyCheckTriggers,
		  			  Oid, myRelOid, Oid, refRelOid,
		  			  Constraint *, fkconstraint, Oid, constraintOid,
		  			  Oid, indexOid,
		  			  Oid, parentInsTrigger, Oid, parentUpdTrigger,
		  			  Oid *, insertTrigOid, Oid *, updateTrigOid)
{
	if (HapCheckAmUsingRelId(myRelOid) && HapCheckAmUsingRelId(refRelOid))
		return true;
 ...
 */
HAP_DEFINE_HOOK_BODY9(void, createForeignKeyCheckTriggers,
		  			  Oid, myRelOid, Oid, refRelOid,
		  			  Constraint *, fkconstraint, Oid, constraintOid,
		  			  Oid, indexOid,
		  			  Oid, parentInsTrigger, Oid, parentUpdTrigger,
		  			  Oid *, insertTrigOid, Oid *, updateTrigOid)
{
	/* Inherit hidden attribute descriptors from refRel */
	HapInheritHiddenAttrDesc(myRelOid, refRelOid);

	*insertTrigOid = HapCreateFKCheckTrigger(myRelOid, refRelOid, fkconstraint,
											 constraintOid, indexOid,
											 parentInsTrigger, true);

	*updateTrigOid = HapCreateFKCheckTrigger(myRelOid, refRelOid, fkconstraint,
											 constraintOid, indexOid,
											 parentUpdTrigger, false);
}

/*
 * __HAP_PANDORA_DefineInternalRel
 *		Create table that is defined by HAP/PANDORA internally.
 *
 * Tables needed by the internal operation of HAP/PANDORA are defined, not by
 * user requests, such as partition maps or RID tables.
 */
static Oid
__HAP_PANDORA_DefineInternalRel(Relation parentrel,
								char *relname, List *tableElts)
{
	Oid			namespaceId;
	Oid			relationId;
	Oid			tablespaceId;
	...
	ListCell   *listptr;
	AttrNumber	attnum;
	ObjectAddress parent_addr, child_addr;
	Oid			accessMethodId = InvalidOid;
	Oid			relid;
	Oid			ownerId;
	...
	/* Inherit parent table' informations */
	namespaceId = get_rel_namespace(RelationGetRelid(parentrel));
	tablespaceId = get_rel_tablespace(RelationGetRelid(parentrel));
	...
	accessMethodId = get_table_am_oid("hap", false);
	...
	/* Set dependency to DROP partition automatically */
	ObjectAddressSet(parent_addr, RelationRelationId,
					 RelationGetRelid(parentrel));
	ObjectAddressSet(child_addr, RelationRelationId, relid);
	recordDependencyOn(&child_addr, &parent_addr, DEPENDENCY_INTERNAL);

	return relid;
}

/*
 * HapDefinePartMapIndexes
 *		Define partition map's indexes.
 *
 * Partition map has two indexes, gin and hash.
 *
 * Gin is used to search partitions in planning where only a partial hidden
 * attribute values are determined.
 *
 * Hash is used to search partition in INSERT where all hidden attribute values
 * are determined.
 */
static void
HapDefinePartMapIndexes(HapPartCreateStmt *part_create_stmt,
						Relation parentrel,
						char *indexed_col_name)
{
	char gin_idxname[NAMEDATALEN], hash_idxname[NAMEDATALEN];
	ObjectAddress address;
	List *idxElts = NIL;
	IndexElem *idxElem;
	IndexStmt *stmt;
	...
	/* Make index on hidden partition key */
	idxElem = makeNode(IndexElem);
	idxElem->name = indexed_col_name;
	idxElts = lappend(idxElts, idxElem);

	/* Initialize index statemet */
	stmt = palloc0(sizeof(IndexStmt));
	rv = palloc0(sizeof(RangeVar));
	rv->relname = RelationGetRelationName(parentrel);
	rv->inh = true;
	rv->relpersistence = RELPERSISTENCE_PERMANENT;
	stmt->relation = rv;
	stmt->indexParams = idxElts;

	/* Define HASH index */
	sprintf(hash_idxname, "_hap_hidden_partition_map_index_hash_%s",
			RelationGetRelationName(parentrel));
	stmt->idxname = hash_idxname;
	stmt->accessMethod = "hash";
	address = DefineIndex(part_create_stmt->partmap_id, stmt,
						  InvalidOid, InvalidOid, InvalidOid,
						  false, true, true, false, false);

	/* Remember index's oid */
	part_create_stmt->hash_idx_id = address.objectId;

	/* Define GIN index */
	sprintf(gin_idxname, "_hap_hidden_partition_map_index_gin_%s",
			RelationGetRelationName(parentrel));
	stmt->idxname = gin_idxname;
	stmt->accessMethod = "gin";

	/*
	 * We use intarray opclass as a GIN index's opclass. So check that whether
	 * the intarray extension is installed or not.
	 */
	ginamid = get_index_am_oid("gin", false);
	if (!OpclassnameGetOpcid(ginamid, "gin__int_ops"))
		elog(ERROR, "HAP's partition map needs to install intarray extension");

	opclass = makeNode(String);
	opclass->sval = "gin__int_ops";
	idxElem->opclass = lappend(NIL, opclass);
	...
	/* Remember index's oid */
	part_create_stmt->gin_idx_id = address.objectId;
 ...
 *
 * Define partition mapping file that maps hidden attribute values to the hidden
 * partition oid.
 */
static void
HapDefinePartMap(HapPartCreateStmt *stmt, Relation parentrel)
{
	ColumnDef *hidden_partition_key, *hidden_partition_id;
	char relname[NAMEDATALEN], colname1[64], colname2[64];
	List *tableElts = NIL;

	/* Set name automatically */
	sprintf(relname, "_hap_hidden_partition_map_%s",
			RelationGetRelationName(parentrel));

	/* Make columns */
	sprintf(colname1, "_hap_hidden_partition_key");
	sprintf(colname2, "_hap_hidden_partition_id");

	hidden_partition_key = makeColumnDef(colname1, INT4ARRAYOID, -1, 0);
	hidden_partition_id = makeColumnDef(colname2, INT4OID, -1, 0);

	tableElts = lappend(tableElts, hidden_partition_key);
	tableElts = lappend(tableElts, hidden_partition_id);

	stmt->partmap_id = __HAP_PANDORA_DefineInternalRel(parentrel,
													   relname, tableElts);	

	/* Define indexes */
	HapDefinePartMapIndexes(stmt, parentrel, colname1);
}

/*
 * HapUpdateCatalogPartitionFlag
 *		Update pg_class to record partitioned flag
 *
 * Record this table is hidden attribute partitioned.
 */
static void
HapUpdateCatalogPartitionFlag(HapPartCreateStmt *stmt)
{
	Oid reloid = stmt->target_relid;
	Form_pg_class rd_rel;
	Relation pg_class;
	HeapTuple tuple;
...
/*
 * HapUpdateCatalogPartMapKeyIdxs
 *		Update pg_hap_hidden_attribute_desc to record partition key index.
 *
 * Record the partition key index in the descriptor corresponding to each
 * partition key.
 *
 * Through this, information about the partition key can be obtained at the smae
 * time as obtatining the descriptor from INSERT which requires minimum I/O.
 */
static void
HapUpdateCatalogPartMapKeyIdxs(HapPartCreateStmt *stmt)
{
	Form_pg_hap_hidden_attribute_desc hapdesc;
	Oid reloid = stmt->target_relid;
	Relation hapdescrel;
	HeapTuple tuple;
	ListCell *lc;

	hapdescrel = table_open(HapHiddenAttributeDescRelationId, RowExclusiveLock);

	/* Update the pg_hap_hidden_attribute_desc rows */
	foreach(lc, stmt->partkey_list)
	{
		HapPartKey *key = lfirst(lc);

		tuple = SearchSysCacheCopy2(HAPDESCRELID,
									ObjectIdGetDatum(reloid),
									Int16GetDatum(key->target_descid));
		if (!HeapTupleIsValid(tuple))
			elog(ERROR, "cache lookup failed for relation %u", reloid);
		hapdesc = (Form_pg_hap_hidden_attribute_desc) GETSTRUCT(tuple);

		/* Update the partkey index */
		hapdesc->happartkeyidx = foreach_current_index(lc);
		CatalogTupleUpdate(hapdescrel, &tuple->t_self, tuple);
	...
	/* Done */
	table_close(hapdescrel, RowExclusiveLock);

	CommandCounterIncrement();
}

/*
 * HapUpdateCatalogPartMap
 *		Update partmap related catalog entries.
 *
 * Update pg_class and pg_hap_hidden_attribute_desc to store partition map
 * related informations.
 */
static void
HapUpdateCatalogPartMap(HapPartCreateStmt *stmt)
{
	/* Update pg_hap_partition_map */
	HapInsertPartMapTuple(stmt->target_relid,
						  stmt->partmap_id,
						  stmt->hash_idx_id,
						  stmt->gin_idx_id);

	/* Update pg_class */
	HapUpdateCatalogPartitionFlag(stmt);

	/* Update pg_hap_hidden_attribute_desc */
	HapUpdateCatalogPartMapKeyIdxs(stmt);
}

/*
 * Condition to hook the ATExecSetRelOptions().
 */
HAP_DEFINE_HOOK_COND4(void, ATExecSetRelOptions,
		  			  Relation, rel,
		  			  List *, defList,
		  			  AlterTableType, operation,
		  			  LOCKMODE, lockmode)
{
	...
	if (!HapCheckAmUsingRel(rel))
		return false;

	/* Find hidden attribute related parameters */
	foreach(lc, defList)
	{
		DefElem *def = lfirst(lc);

		if (strncmp(def->defname, "hidden_partition_key", 20) == 0)
		{
			found = true;
			break;
		}
 ...
 */
HAP_DEFINE_HOOK_BODY4(void, ATExecSetRelOptions,
		 			  Relation, rel,
					  List *, defList,
					  AlterTableType, operation,
					  LOCKMODE, lockmode)
{
	DefElem *hidden_partition_key = NULL;
	DefElem *hidden_partition_num = NULL;
#ifdef PANDORA
	DefElem *pandora_spread_factor = NULL;
#endif /* PANDORA */
	uint64 mapped_partkey_total_count = 0;
	HapPartCreateStmt stmt;
	ListCell *lc;

	if (rel->rd_rel->relkind != RELKIND_RELATION)
		elog(ERROR,
			 "hidden partition can be created only for the basic relation");
	...
	foreach(lc, defList)
	{
		...
		if (strncmp(def->defname, "hidden_partition_key", 20) == 0)
			hidden_partition_key = def;
		else if (strncmp(def->defname, "hidden_partition_num", 20) == 0)
			hidden_partition_num = def;
#ifdef PANDORA
		else if (strncmp(def->defname, "pandora_spread_factor", 21) == 0)
			pandora_spread_factor = def;
#endif /* PANDORA */
	}

	/* Get the partition keys */
	HapBuildPartCreateStmt(&stmt, RelationGetRelid(rel),
						   hidden_partition_key, hidden_partition_num);

	/* Create partition map */
	HapDefinePartMap(&stmt, rel);

	/* Create partitions */
	for (int i = 0; i < stmt.part_count; i++)
	{
		Datum keys[HAP_MAX_PARTKEY_COUNT];
		uint64 mapped_partkey_per_part_count = 0;

		/* Insert partition keys to the partition map */
		for (;;)
		{
			int remained_partkey;

			HapGetNextPartKeyValues(&stmt, keys);

			HapInsertPartKeyToPartMap(&stmt, stmt.partmap_id, keys, i);

			mapped_partkey_total_count++;
			mapped_partkey_per_part_count++;

			/*
			 * Note that for the last partition, all remaining keys must be
			 * consumed.
			 */
			remained_partkey
				= i < stmt.part_count - 1 ?
				stmt.value_set_count_per_part - mapped_partkey_per_part_count :
				stmt.total_value_set_count - mapped_partkey_total_count;

			/* All part keys are consuemd, move to the next partition */
			if (!remained_partkey)
				break;
		}
	}
	...
	/* Update catalogs */
	HapUpdateCatalogPartMap(&stmt);

#ifdef PANDORA
	/* Connect it to the pandora module */
	PandoraRegisterRel(stmt.target_relid,
					   stmt.part_count, pandora_spread_factor);
#endif /* PANDORA */
}
#endif /* HAP */
