					 ...
					 TupleTableSlot *slot)
{
	...
	for (i = 0; i < trigdesc->numtriggers; i++)
	{
		...
		if (!TriggerEnabled(estate, relinfo, trigger, LocTriggerData.tg_event,
			...
			continue;
#ifdef HAP
		/* This trigger is called in HAP's logic */
		if (strncmp(trigger->tgname, "HAP_RI_ConstraintTrigger_c", 26) == 0)
			continue;
#endif /* HAP */
		  ...
		  transition_capture->tcs_update_new_table)))
	{
		...
		if (fdw_trigtuple == NULL && ItemPointerIsValid(tupleid))
			GetTupleForTrigger(estate,
							   ...
							   tupleid,
#ifdef PANDORA
							   newslot->tts_pandora_fingerprint,
#endif
							   LockTupleExclusive,
							   oldslot,
							   NULL,
...
#ifdef PANDORA
		if (IsPandora(tupsrc->ri_RelationDesc) &&
				oldslot->tts_tuple_position->needDecrRefCount)
		{
			PandoraRelationMeta *meta = 
				PandoraGetRelationMeta(tupsrc->ri_RelationDesc->rd_node.relNode);
			PandoraPartLevel partLvl = oldslot->tts_tuple_position->partitionLevel;
			PandoraPartNumber partNum = (partLvl == 0) ? 0 :
				oldslot->tts_tuple_position->partitionNumber;

			/* Decrement modification count. */
			DecrModificationCount(
				PandoraRelationMetaRefCounter(meta, partLvl, partNum), 
				oldslot->tts_tuple_position->modificationMethod);
		}
#endif /* PANDORA */
	}
}
...
#ifdef PANDORA
			if (isPandora)
				oldslot->tts_pandora_fingerprint = fingerprint;
#endif

			if (!table_tuple_search_row_version(relation, tid,
												estate->es_snapshot, oldslot))
				elog(ERROR, "failed to search tuple for trigger");
		}
		else
		{
	...
	switch (event->ate_flags & AFTER_TRIGGER_TUP_BITS)
	{
		...
		default:
			if (ItemPointerIsValid(&(event->ate_ctid1)))
			{
				...
				if (IsSiro(src_rel))
				{
					if (!table_tuple_search_row_version(src_rel,
														&(event->ate_ctid1),
														estate->es_snapshot,
														src_slot))
					elog(ERROR, "failed to search tuple1 for AFTER trigger");
				}
				else
				{
				...
				ItemPointerIsValid(&(event->ate_ctid2)))
			{
				...
				if (IsSiro(dst_rel))
				{
					if (!table_tuple_search_row_version(dst_rel,
														&(event->ate_ctid2),
														estate->es_snapshot,
														dst_slot))
					elog(ERROR, "failed to search tuple2 for AFTER trigger");
				}
				else
				{
...
#ifdef HAP
/*
 * HapInitHiddenAttrDescState
 *		Get both parent and child relations's hidden attribute descriptor lists
 *		and save them to the INSERT STATE.
 *
 * From relcache, copy the descriptor maps to the local INSERT STATE. This is
 * because relcache can be flushed so if we remember the pointer, we may access
 * to the wrong address.
 *
 * Return new parent's id in insert state's parent_relid_list.
 */
static int
HapInitHiddenAttrDescState(HapInsertState *state,
						   Oid parent_relid, Relation child_rel)
{
	List *cached_parent_desc_list, *parent_desc_list = NIL;
	List *cached_child_desc_list, *child_desc_list = NIL;
	List *encoding_value_list = NIL;
	int cached_idx = -1;
	int16 desccount;
	ListCell *lc;

	/* Find where the given parent's hidden attribute descriptors are cached */
	foreach(lc, child_rel->rd_hap_parent_relid_list)
	{
		...
		{
			cached_idx
				= list_cell_number(child_rel->rd_hap_parent_relid_list, lc);
			break;
		}
	}

	/*
	 * This case can happen when the parent table uses access method HAP but
	 * has no hidden attribute yet.
	 */
	if (cached_idx < 0)
		goto match;

	cached_parent_desc_list = list_nth(child_rel->rd_hap_parent_desc_map,
									   cached_idx);
	cached_child_desc_list = list_nth(child_rel->rd_hap_child_desc_map,
									  cached_idx);
	desccount = list_length(cached_parent_desc_list);
	
	/* Copy the cached informations to the local state */
	for (int descid = 0; descid < desccount; descid++)
	{
		HapHiddenAttrDesc *cached_parent_desc, *cached_child_desc;
		HapHiddenAttrDesc *parent_desc, *child_desc;

		cached_parent_desc
			= (HapHiddenAttrDesc *) list_nth(cached_parent_desc_list,
											 descid);
		cached_child_desc
			= (HapHiddenAttrDesc *) list_nth(cached_child_desc_list,
											 descid);

		parent_desc = palloc0(sizeof(HapHiddenAttrDesc));
		child_desc = palloc0(sizeof(HapHiddenAttrDesc));

		memcpy(parent_desc, cached_parent_desc, sizeof(HapHiddenAttrDesc));
		memcpy(child_desc, cached_child_desc, sizeof(HapHiddenAttrDesc));

		parent_desc_list = lappend(parent_desc_list, parent_desc);
		child_desc_list = lappend(child_desc_list, child_desc);

		/* Make placeholder for encoding values */
		encoding_value_list = lappend_int(encoding_value_list, 0);
	}

match:
	state->parent_relid_list = lappend_oid(state->parent_relid_list,
										   parent_relid);
	state->parent_desc_map = lappend(state->parent_desc_map,
									 parent_desc_list);
	state->child_desc_map = lappend(state->child_desc_map,
									child_desc_list);
	state->encoding_value_map = lappend(state->encoding_value_map,
										encoding_value_list);

	return list_length(state->parent_relid_list) - 1;
}

/*
 * HapDeconstructParentHiddenAttr
 *		Save parent's hidden attribute related informations to the HAP's insert
 *		state.
 *
 * The result of interpreting the given hidden attribute as the parent
 * relation's hidden attribute descriptors is stored in the INSERT STATE.
 *
 * If there are no hidden attribute descriptors of the parent relation,
 * initialize them and store them in the INSERT STATE.
 */
static void
HapDeconstructParentHiddenAttr(HapState *hapstate,
							   Oid parent_relid, Relation child_rel,
							   bytea *hidden_attribute)
{
	List *encoding_value_list, *parent_desc_list, *child_desc_list;
	...
	/* Get the corresponding index for the parent relation */
	foreach(lc, hap_insert_state->parent_relid_list)
	{
		Oid relid = lfirst_oid(lc);

		if (relid == parent_relid)
		{
			id = list_cell_number(hap_insert_state->parent_relid_list, lc);	
			break;
		}
	}

	/* Initialize descriptors if this is the first time for the relation */
	if (id == -1)
		id = HapInitHiddenAttrDescState(hap_insert_state,
										parent_relid, child_rel);
	Assert(id >= 0);

	parent_desc_list = list_nth(hap_insert_state->parent_desc_map, id);
	encoding_value_list = list_nth(hap_insert_state->encoding_value_map, id);
	child_desc_list = list_nth(hap_insert_state->child_desc_map, id);

	/* Interpret the hidden attribute and store encoding values */
	forthree(lc, parent_desc_list,
			 lc2, encoding_value_list,
			 lc3, child_desc_list)
	{
		HapHiddenAttrDesc *parent_desc = lfirst(lc);
		HapHiddenAttrDesc *child_desc = lfirst(lc3);
		int16 encoding_value;

		encoding_value = HapExtractEncodingValueFromBytea(parent_desc,
														  hidden_attribute);

		/* Store encoding value */
		lfirst_int(lc2) = encoding_value;

		/*
		 * If this child descriptor is used for partition key, 
		 * write the partition key value to the corresponding position.
		 */
		if (child_desc->partkeyidx != -1)
		{
			hap_insert_state->partkeys[child_desc->partkeyidx]
				= (encoding_value |
					(child_desc->descid) << HAP_PARTMAP_DESC_SHIFT);
			hap_insert_state->partkeynum++;
		}
	}
 ...
 *		Check foreign keys and store hidden attributes in hap state.
 *
 * This funcion only calls trigger function named
 * "HAP_RI_FKey_check_before_ins()".
 */
void
HapExecForeignKeyCheckTriggers(EState *estate, ResultRelInfo *relinfo,
							   TupleTableSlot *slot)
{
	TriggerDesc *trigdesc = relinfo->ri_TrigDesc;
	TriggerData LocTriggerData = {0};
	...
	struct timespec stat_tpcc_breakdown_insert_deconstruct_hidden_start;
	struct timespec stat_tpcc_breakdown_insert_deconstruct_hidden_end;
#endif /* STAT_TPCC */
	...
	/*
	 * @trigdesc has not only HAP's triggers but also unrelated triggers. Look
	 * at the name of the trigger and execute only the forign key check trigger
	 * of HAP.
	 */
	for (int i = 0; i < trigdesc->numtriggers; i++)
	{
		...
		/* Call HAP_RI_FKey_check_before_ins() */
		hidden_attribute = (Datum) ExecCallTriggerFunc(&LocTriggerData, i,
													   relinfo->ri_TrigFunctions,
													   relinfo->ri_TrigInstrument,
													   GetPerTupleMemoryContext(estate));

		/* Store the hidden attribute and parent table oid */
		Assert(estate->es_hapstate != NULL);
#ifdef STAT_TPCC
		if (stat_tpcc_breakdown_insert_exec_insert_flag)
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_insert_deconstruct_hidden_start);
#endif /* STAT_TPCC */
		HapDeconstructParentHiddenAttr(estate->es_hapstate,
		...
		{
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_insert_deconstruct_hidden_end);
