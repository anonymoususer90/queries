			   ...
			   int elevel)
{
...
#ifdef PANDORA
 	{
 		if (onerel->rd_pandora_partitioning)
 		{
 			/* !!! TODO: Only valid for ANALYZE in init !!! */
 			numrows = PandoraGetNumRows(onerel);
 			totalrows = numrows;
 			totaldeadrows = 0;
 		}
 		else
	 ...
	 */
#ifdef PANDORA
	/* TODO !!! It may need to fix !!! */
	if (!onerel->rd_pandora_partitioning && numrows > 0)
#else /* !PANDORA */
	if (numrows > 0)
#endif /* PANDORA */
	{
...
#ifdef PANDORA
	scan = table_beginscan_analyze(onerel, NULL);
#else /* !PANDORA */
