...
#ifdef DIVA
#define EbiBufferGetDatum(ebi_buffer) Int32GetDatum(ebi_buffer)
#define DatumGetEbiBuffer(datum) ((int32) DatumGetInt32(datum))
#endif /* DIVA */
...
#ifdef DIVA
	ResourceArray ebibufferarr;	/* owned ebi buffers */
#ifdef PANDORA
	ResourceArray readbufdescarr;	/* owned read buffers */
	ResourceArray iouringarr;	/* owned io uring structures */
#endif /* PANDORA */
#endif /* DIVA */
...
#ifdef HAP_REDUCTION
	ResourceArray substitutioncachearr;		/* substitotion cache contexts */
#endif
...
#ifdef DIVA
static void PrintEbiBufferLeakWarning(int32_t ebiBuffer);
#ifdef PANDORA
static void PrintReadBufDescLeakWarning(ReadBufDesc readBufDesc);
static void PrintIoUringLeakWarning(struct io_uring *ring);
#endif /* PANDORA */
#endif /* DIVA */
...
#ifdef HAP_REDUCTION
	ResourceArrayInit(&(owner->substitutioncachearr), PointerGetDatum(NULL));
#endif

#ifdef DIVA
	ResourceArrayInit(&(owner->ebibufferarr), EbiBufferGetDatum(-1));
#ifdef PANDORA
	ResourceArrayInit(&(owner->readbufdescarr), PointerGetDatum(NULL));
	ResourceArrayInit(&(owner->iouringarr), PointerGetDatum(NULL));
#endif /* PANDORA */
#endif /* DIVA */
...
#ifdef DIVA
		/* Ditto for ebi buffer */
		while (ResourceArrayGetAny(&(owner->ebibufferarr), &foundres))
		{
			int32 res = (int32) DatumGetEbiBuffer(foundres);

			if (isCommit)
				PrintEbiBufferLeakWarning(res);
			UnpinEbiSubBuffer(res);
		}
#ifdef PANDORA
		/* Ditto for read buffer description */
		while (ResourceArrayGetAny(&(owner->readbufdescarr), &foundres))
		{
			ReadBufDesc res = (ReadBufDesc) DatumGetPointer(foundres);

			if (isCommit)
				PrintReadBufDescLeakWarning(res);
			PushReadBufDesc(res);
		}

		/* Ditto for io uring structure */
		while (ResourceArrayGetAny(&(owner->iouringarr), &foundres))
		{
			struct io_uring *res = (struct io_uring *) DatumGetPointer(foundres);

			if (isCommit)
				PrintIoUringLeakWarning(res);
			
			/* exit io uring and forget it from resource array */
			io_uring_queue_exit(res);
			ResourceOwnerForgetIoUring(CurrentResourceOwner, res);
		}
#endif /* PANDORA */
#endif /* DIVA */
#ifdef HAP_REDUCTION
		/* Ditto for substitution caches */
		while (ResourceArrayGetAny(&(owner->substitutioncachearr), &foundres))
		{
			HapSubstitutionCache *res = (HapSubstitutionCache *) 
													DatumGetPointer(foundres);

			/* We free substitution cache here. */
			FreeSubstitutionCache(res);
		}
#endif
	}
...
#ifdef HAP_REDUCTION
	Assert(owner->substitutioncachearr.nitems == 0);
#endif
	Assert(owner->nlocks == 0 || owner->nlocks == MAX_RESOWNER_LOCKS + 1);
#ifdef DIVA
	Assert(owner->ebibufferarr.nitems == 0);
#ifdef PANDORA 
	Assert(owner->readbufdescarr.nitems == 0);
	Assert(owner->iouringarr.nitems == 0);
#endif /* PANDORA */
#endif /* DIVA */
...
#ifdef DIVA
	ResourceArrayFree(&(owner->ebibufferarr));
#ifdef PANDORA
	ResourceArrayFree(&(owner->readbufdescarr));
	ResourceArrayFree(&(owner->iouringarr));
#endif
#endif
#ifdef HAP_REDUCTION
	ResourceArrayFree(&(owner->substitutioncachearr));
#endif
...
/*
 * Make sure there is room for at least one more entry in a ResourceOwner's
 * substitution cache array.
 *
 * This is separate from actually inserting an entry because if we run out
 * of memory, it's critical to do so *before* acquiring the resource.
 */
void
ResourceOwnerEnlargeReadBufferDescs(ResourceOwner owner)
{
	/* We used to allow pinning buffers without a resowner, but no more */
	Assert(owner != NULL);
	ResourceArrayEnlarge(&(owner->readbufdescarr));
}

/*
 * Remember that a read buffer descriptor is owned by a ResourceOwner
 *
 * Caller must have previously done ResourceOwnerEnlargeReadBufferDescs()
 */
void
ResourceOwnerRememberReadBufferDesc(ResourceOwner owner, ReadBufDesc readBufDesc)
{
	ResourceArrayAdd(&(owner->readbufdescarr), PointerGetDatum(readBufDesc));
}

/*
 * Forget that a read buffer descriptor is owned by a ResourceOwner
 */
void
ResourceOwnerForgetReadBufferDesc(ResourceOwner owner, ReadBufDesc readBufDesc)
{
	if (!ResourceArrayRemove(&(owner->readbufdescarr), PointerGetDatum(readBufDesc)))
		elog(ERROR, "read buffer desc %d is not owned by resource owner %s",
			 readBufDesc->buf_id, owner->name);
}
#endif

#ifdef HAP_REDUCTION
void
ResourceOwnerEnlargeSubstitutionCache(ResourceOwner owner)
{
	/* We used to allow pinning buffers without a resowner, but no more */
	Assert(owner != NULL);
	ResourceArrayEnlarge(&(owner->substitutioncachearr));
}

/*
 * Remember that a substitution cache is owned by a ResourceOwner
 *
 * Caller must have previously done ResourceOwnerEnlargeSubstitutionCache()
 */
void
ResourceOwnerRememberSubstitutionCache(ResourceOwner owner, 
													HapSubstitutionCache *cache)
{
	ResourceArrayAdd(&(owner->substitutioncachearr), PointerGetDatum(cache));
}

/*
 * Forget that a substitution cache is owned by a ResourceOwner
 */
void
ResourceOwnerForgetSubstitutionCache(ResourceOwner owner, 
													HapSubstitutionCache *cache)
{
	if (!ResourceArrayRemove(&(owner->substitutioncachearr), PointerGetDatum(cache)))
		elog(ERROR, "substitution cache %d is not owned by resource owner %s",
			 cache->cache_idx, owner->name);
}
#endif /* HAP_REDUCTION */
...
/*
 * Make sure there is room for at least one more entry in a ResourceOwner's
 * ebi buffer descriptor.
 *
 * This is separate from actually inserting an entry because if we run out
 * of memory, it's critical to do so *before* acquiring the resource.
 */
void
ResourceOwnerEnlargeEbiBuffers(ResourceOwner owner)
{
	/* We used to allow pinning buffers without a resowner, but no more */
	Assert(owner != NULL);
	ResourceArrayEnlarge(&(owner->ebibufferarr));
}

/*
 * Remember that a ebi buffer is owned by a ResourceOwner
 *
 * Caller must have previously done ResourceOwnerEnlargeEbiBuffers()
 */
void
ResourceOwnerRememberEbiBuffer(ResourceOwner owner, int32_t ebiBuffer)
{
	ResourceArrayAdd(&(owner->ebibufferarr), EbiBufferGetDatum(ebiBuffer));
}

/*
 * Forget that a ebi buffer descriptor is owned by a ResourceOwner
 */
void
ResourceOwnerForgetEbiBuffer(ResourceOwner owner, int32_t ebiBuffer)
{
	if (!ResourceArrayRemove(&(owner->ebibufferarr), EbiBufferGetDatum(ebiBuffer)))
		elog(ERROR, "ebi buffer desc %d is not owned by resource owner %s",
			 ebiBuffer, owner->name);
}
#endif /* DIVA */
 ...
 */
static void
PrintEbiBufferLeakWarning(int32_t ebiBuffer)
{
	elog(WARNING, "ebi buffer leak: %d still referenced",
		 ebiBuffer);
}
 ...
 */
static void
PrintReadBufDescLeakWarning(ReadBufDesc readBufDesc)
{
	elog(WARNING, "read buffer desc leak: %d still referenced",
		 readBufDesc->buf_id);
}

/*
 * Make sure there is room for at least one more entry in a ResourceOwner's
 * io uring structure array.
 *
 * This is separate from actually inserting an entry because if we run out
 * of memory, it's critical to do so *before* acquiring the resource.
 */
void
ResourceOwnerEnlargeIoUrings(ResourceOwner owner)
{
	/* We used to allow pinning buffers without a resowner, but no more */
	Assert(owner != NULL);
	ResourceArrayEnlarge(&(owner->iouringarr));
}

/*
 * Remember that a io uring structuer is owned by a ResourceOwner
 *
 * Caller must have previously done ResourceOwnerEnlargeIoUring()
 */
void
ResourceOwnerRememberIoUring(ResourceOwner owner, struct io_uring *ring)
{
	ResourceArrayAdd(&(owner->iouringarr), PointerGetDatum(ring));
}

/*
 * Forget that a io uring structure is owned by a ResourceOwner
 */
void
ResourceOwnerForgetIoUring(ResourceOwner owner, struct io_uring *ring)
{
	if (!ResourceArrayRemove(&(owner->iouringarr), PointerGetDatum(ring)))
		elog(ERROR, "ring is not owned by resource owner %s", owner->name);
}
 ...
 */
static void
PrintIoUringLeakWarning(struct io_uring *ring)
{
	elog(WARNING, "io uring leak: something still referenced");
}
#endif /* PANDORA */
