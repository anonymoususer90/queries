...
#ifdef HAP
/*
 * HAP_RI_CopyHiidenAttribute_from_SPI
 * 		Copy the hidden attribute to caller's variable.
 *
 * SPI tuple belongs to SPI's context. So we should copy it to caller's context.
 * Otherwise the tuple will be freed when the caller wants to process it.
 */
static void
HAP_RI_copy_hidden_attribute(bytea *hidden_attribute)
{
	TupleDesc	tupleDesc = SPI_tuptable->tupdesc;
	uint64		numvals = SPI_tuptable->numvals;
	HeapTuple	tuple = SPI_tuptable->vals[numvals - 1];
	int natts = tupleDesc->natts;
	bytea *tmp_hidden_attribute;
	Datum		values[40];
	bool		isnull[40];
	int bytesize;

	/* Get the scanned tuple (foreign key tuple of parent table) */
	Assert(numvals == 1);
	heap_deform_tuple(tuple, tupleDesc, values, isnull);

	/* Get the hidden attribute from the SPI tuple */
	Assert(natts == 1);
	tmp_hidden_attribute = (bytea *) DatumGetPointer(values[0]);
	bytesize = VARSIZE_ANY(tmp_hidden_attribute);

	/* Store the hidden attribute to our context */
	memcpy(hidden_attribute, tmp_hidden_attribute, bytesize);
}

/*
 * HAP_RI_FKey_check_get_hidden_attribute -
 *
 * Check foreign key existence (combined for INSERT and UPDATE).
 *
 * Unlike the existing foreign key check, it selects the hidden attribute and
 * copy it to the given @hidden_attribute.
 */
static void
HAP_RI_FKey_check_get_hidden_attribute(TriggerData *trigdata,
	bytea *hidden_attribute)
{
	...
	if ((qplan = ri_FetchPreparedPlan(&qkey)) == NULL)
	{
		...
		/* ----------
		 * The query string built is
		 *	SELECT hidden_attribute FROM [ONLY] <pktable> x WHERE pkatt1 = $1 [AND ...]
		 *		   FOR KEY SHARE OF x
		 * The type id's for the $ parameters are those of the
		 * corresponding FK attributes.
		 * ----------
		...
		pk_only = pk_rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE ?
			"" : "ONLY ";
		quoteRelationName(pkrelname, pk_rel);
		appendStringInfo(&querybuf, "SELECT _hap_hidden_attribute FROM %s%s x",
						 pk_only, pkrelname);
		querysep = "WHERE";
		for (int i = 0; i < riinfo->nkeys; i++)
		{
	...
	/*
	 * Pass the required attribute to the caller.
	 * It must be called before SPI_finish() since memory context.
	 */
	HAP_RI_copy_hidden_attribute(hidden_attribute);
 ...
 *		Check that RI trigger function was called in expected context.
 *
 * HAP expects trigger to be called as INSERT BEFORE ROW.
 */
static void
HAP_RI_CheckTrigger(FunctionCallInfo fcinfo, const char *funcname)
{
	TriggerData *trigdata = (TriggerData *) fcinfo->context;
	...
	if (!TRIGGER_FIRED_BEFORE(trigdata->tg_event) ||
		...
		ereport(ERROR,
				(errcode(ERRCODE_E_R_I_E_TRIGGER_PROTOCOL_VIOLATED),
				errmsg("function \"%s\" must be fired BEFORE ROW", funcname)));
 ...
 *		Check foreign key existence at insert on a table that uses HAP.
 *
 * Check foreign key and get the hidden attribute. Return the address of bytea
 * which is hidden attribute.
 *
 * Note that we call palloc for hidden attribute and current context is EState's
 * per-tuple memory context that will be reset by ExecModifyTable().
 */
Datum
HAP_RI_FKey_check_before_ins(PG_FUNCTION_ARGS)
{
	bytea *hidden_attribute;

	HAP_RI_CheckTrigger(fcinfo, "HAP_RI_FKey_check_before_ins");

	/* Allocate memory for hidden attribute */
	hidden_attribute = palloc0(64);

	HAP_RI_FKey_check_get_hidden_attribute((TriggerData *) fcinfo->context,
		hidden_attribute);

	return PointerGetDatum(hidden_attribute);
}
#else /* !HAP */
/*
 * Dummy function.
