...
Datum
switch_d_trigger(PG_FUNCTION_ARGS)
{
	d_trigger = !d_trigger;
...
Datum
init_timermanager_func(PG_FUNCTION_ARGS)
{
	if (GetCurrentTransactionId() % mod_value == 0)
		init_timermanager();
...
Datum
stamp_start_func(PG_FUNCTION_ARGS)
{
	if (GetCurrentTransactionId() % mod_value == 0)
		stamp_start();
...
Datum
stamp_end_func(PG_FUNCTION_ARGS)
{
	if (GetCurrentTransactionId() % mod_value == 0)
		stamp_end();
...
Datum
print_timermanager_func(PG_FUNCTION_ARGS)
{
	if (GetCurrentTransactionId() % mod_value == 0)
	{
		char trx_name[16];
		VarChar *src = PG_GETARG_VARCHAR_PP(0);
		int32 len = VARSIZE_ANY_EXHDR(src);
		char *instr = VARDATA_ANY(src);
		strncpy(trx_name, instr, len);
		trx_name[len] = '\0';
		print_timermanager(trx_name);
	}
...
ebi_tree_print(PG_FUNCTION_ARGS)
{
  /* Do not anything, we can not use this in vanilla version */
...
#ifdef PANDORA
Datum
print_relation_meta(PG_FUNCTION_ARGS)
{
