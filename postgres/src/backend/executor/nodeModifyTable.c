...
#include "hap/hap_state.h"
#include "hap/hap_trigger.h"
#endif /* HAP */
...
static TupleTableSlot *mergeGetUpdateNewTuple(ResultRelInfo *relinfo,
											  ...
											  MergeActionState *relaction);
#ifdef HAP
static TupleTableSlot *HapGetInsertNewTuple(EState *estate,
											ResultRelInfo *relinfo,
											TupleTableSlot *planSlot);
static TupleTableSlot *HapExecInsert(ModifyTableContext *context,
									 ResultRelInfo *resultRelInfo,
									 TupleTableSlot *slot,
									 bool canSetTag,
		   ...
		   ResultRelInfo **insert_destrel)
{
	...
	MemoryContext oldContext;
#ifdef EXECSTAMP
	fprintf(stderr, "ExecStamp: ExecInsert, rel: %u, pid: %d\n",
			resultRelInfo->ri_RelationDesc->rd_id, MyProcPid);
#endif /* EXECSTAMP */
...
#ifdef DIVA
			elog(ERROR, "DIVA doesn't support INSERT ON CONFLICT yet");
#endif /* DIVA */
		   ...
		   TupleTableSlot **epqreturnslot)
{
	...
	TM_Result	result;
#ifdef EXECSTAMP
	fprintf(stderr, "ExecStamp: ExecDelete, rel: %u, pid: %d\n",
			resultRelInfo->ri_RelationDesc->rd_id, MyProcPid);
#endif /* EXECSTAMP */
...
ldelete:;
	...
	if (processReturning && resultRelInfo->ri_projectReturning)
	{
		...
		else
		{
			...
			{
#ifdef DIVA
				if (IsSiro(resultRelationDesc))
				{
					if (!table_tuple_search_row_version(resultRelationDesc, tupleid,
														estate->es_snapshot, slot))
						elog(ERROR, "failed to search deleted tuple for DELETE RETURNING");
				}
				else
				{
...
#ifdef PANDORA
	if (IsPandora(resultRelationDesc))
	{
		/*
		 * replace the tuple in the partition.
		 */
		result = table_tuple_update_for_pandora_partition(resultRelationDesc, 
									tupleid, 
		   ...
		   bool canSetTag)
{
	...
	struct timespec stat_tpcc_breakdown_update_ExecUpdateAct_start;
	struct timespec stat_tpcc_breakdown_update_ExecUpdateAct_end;
#endif /* STAT_TPCC */
#ifdef EXECSTAMP
	fprintf(stderr, "ExecStamp: ExecUpdate, rel: %u, pid: %d\n",
			resultRelInfo->ri_RelationDesc->rd_id, MyProcPid);
#endif /* EXECSTAMP */
...
redo_act:
#ifdef STAT_TPCC
		if (stat_tpcc_breakdown_update_exec_update_flag)
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_update_ExecUpdateAct_start);
#endif /* STAT_TPCC */
		...
		{
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_update_ExecUpdateAct_end);
...
#ifdef DIVA
					elog(ERROR, "DIVA only supports \"repeatable_read\" isolation level");
#endif /* DIVA */
...
ExecModifyTable(PlanState *pstate)
{
	...
	/* INSERT */
	struct timespec stat_tpcc_insert_start;
	struct timespec stat_tpcc_insert_end;

	/* UPDATE */
	struct timespec stat_tpcc_update_start;
	struct timespec stat_tpcc_update_end;
#endif /* STAT_TPCC */
	...
	{
		siro = IsSiro(resultRelInfo->ri_RelationDesc);
	}
#endif /* DIVA */

#ifdef PANDORA
	isPandora = IsPandora(resultRelInfo->ri_RelationDesc);
#endif /* PANDORA */
	...
	for (;;)
	{
		...
		switch (operation)
		{
			case CMD_INSERT:
#ifdef STAT_TPCC
				clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_insert_start);

				/* BREAKDOWN INSERT */
				stat_tpcc_breakdown_insert_exec_insert_flag = true;
#endif /* STAT_TPCC */
...
#ifdef HAP
				if (HapCheckAmUsingRel(node->rootResultRelInfo->ri_RelationDesc))
				{
					slot = HapGetInsertNewTuple(estate, resultRelInfo,
												context.planSlot);
					slot = HapExecInsert(&context, resultRelInfo, slot,
...
#ifdef STAT_TPCC
				clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_insert_end);

				stat_tpcc_elapsed_time
					= stat_tpcc_get_time_diff(stat_tpcc_insert_start,
											  stat_tpcc_insert_end);

				stat_tpcc_insert_cnt++;

				stat_tpcc_insert_total_time += stat_tpcc_elapsed_time;

				stat_tpcc_insert_max_time
					= stat_tpcc_insert_max_time < stat_tpcc_elapsed_time ?
						stat_tpcc_elapsed_time : stat_tpcc_insert_max_time;

				stat_tpcc_insert_min_time
					= stat_tpcc_insert_min_time > stat_tpcc_elapsed_time ?
						stat_tpcc_elapsed_time : stat_tpcc_insert_min_time;

				/* BREAKDOWN INSERT */
				stat_tpcc_breakdown_insert_total += stat_tpcc_elapsed_time;
			...
			case CMD_UPDATE:
#ifdef STAT_TPCC
				clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_update_start);

				/* BREAKDOWN UPDATE */
				stat_tpcc_breakdown_update_exec_update_flag = true;
#endif /* STAT_TPCC */
				...
				else
				{
					...
					if (siro)
					{
						...
						if (isPandora)
						{
							oldSlot->tts_pandora_fingerprint =
								slot->tts_pandora_fingerprint;
						}
#endif /* PANDORA */

						if (!table_tuple_search_row_version(relation, tupleid,
															estate->es_snapshot,
															oldSlot))
							elog(ERROR, "failed to search tuple being updated");
					}
					else
					{
...
#ifdef PANDORA
				if (oldSlot)
					slot->tts_tuple_position = oldSlot->tts_tuple_position;

				if (context.planSlot)
					slot->tts_pandora_fingerprint = 
						context.planSlot->tts_pandora_fingerprint;
#endif /* PANDORA */
				...
				slot = ExecUpdate(&context, resultRelInfo, tupleid, oldtuple,
								  slot, node->canSetTag);
#ifdef STAT_TPCC
				clock_gettime(CLOCK_MONOTONIC, &stat_tpcc_update_end);

				stat_tpcc_elapsed_time
					= stat_tpcc_get_time_diff(stat_tpcc_update_start,
											  stat_tpcc_update_end);

				stat_tpcc_update_cnt++;

				stat_tpcc_update_total_time += stat_tpcc_elapsed_time;

				stat_tpcc_update_max_time
					= stat_tpcc_update_max_time < stat_tpcc_elapsed_time ?
						stat_tpcc_elapsed_time : stat_tpcc_update_max_time;

				stat_tpcc_update_min_time
					= stat_tpcc_update_min_time > stat_tpcc_elapsed_time ?
						stat_tpcc_elapsed_time : stat_tpcc_update_min_time;

				/* BREAKDOWN UPDATE */
				stat_tpcc_breakdown_update_total += stat_tpcc_elapsed_time;
...
#ifdef HAP
/*
 * HapInheritHiddenAttr
 *		Copy the encoding value of the parent hidden attribute.
 *
 * Get the encoding values of the parent hidden attribute and copy it to the
 * child hidden attribute.
 */
static inline void
HapInheritHiddenAttr(bytea *off,
					 List *child_desc_list, List *encoding_value_list)
{
	ListCell *lc, *lc2;

	/* Apply it */
	forboth(lc, child_desc_list, lc2, encoding_value_list)
		HapApplyEncodingValueToBytea(lfirst(lc), lfirst_int(lc2), (bytea *) off);
}

/*
 * HapBuildHiddenAttr
 *		Build a hidden attribute.
 *
 * Build a hidden attribute for the insert tuple based on the hidden attributes
 * obtained from the foreign key check.
 */
static void
HapBuildHiddenAttr(HapState *hapstate, TupleTableSlot *slot)
{
	HapInsertState *hap_insert_state = hapstate->hap_insert_state;
	HeapTupleTableSlot *hslot = (HeapTupleTableSlot *) slot;
	HeapTuple tuple = hslot->tuple;
	int tuple_len, nbytes;
	ListCell *lc, *lc2;
	char *off;

	/*
	 * To get the address of the hidden attribute, we have to calculate the byte
	 * size of the hidden attribute.
	 */
	Assert(hap_insert_state->nbyte >= 0);
	nbytes = hap_insert_state->nbyte;

	/* bytea has a header. We have to consider it */
	if (nbytes <= VARATT_SHORT_MAX)
		nbytes += VARHDRSZ_SHORT;
	else
		nbytes += VARHDRSZ;

	/* Using the size of the hidden attribute, calculate the address */
	tuple_len = tuple->t_len;
	off = (char *)(tuple->t_data) + (tuple_len - nbytes);

	/* Inherit the hidden attributes of the parents to the given slot */
	forboth(lc, hap_insert_state->child_desc_map,
			lc2, hap_insert_state->encoding_value_map)
		HapInheritHiddenAttr((bytea *)off, lfirst(lc), lfirst(lc2));
}
 ...
 *		ExecInsert() for HAP.
 *
 * HAP has some differences from heap access method.
 *
 * 1. HAP checks foreign key before insert.
 * 2. HAP routes partition after foreign key check.
 *
 * To apply these things, we made our own function instead ExecInsert().
 */
static TupleTableSlot *
HapExecInsert(ModifyTableContext *context,
		 	  ResultRelInfo *resultRelInfo,
		 	  TupleTableSlot *slot,
			  bool canSetTag,
			  TupleTableSlot **inserted_tuple,
			  ResultRelInfo **insert_destrel)
{
	...
	struct timespec stat_tpcc_breakdown_insert_HapGetPartIdHash_start;
	struct timespec stat_tpcc_breakdown_insert_HapGetPartIdHash_end;

	struct timespec stat_tpcc_breakdown_insert_fkey_check_start;
	struct timespec stat_tpcc_breakdown_insert_fkey_check_end;

	struct timespec stat_tpcc_breakdown_insert_build_hidden_start;
	struct timespec stat_tpcc_breakdown_insert_build_hidden_end;
#endif /* STAT_TPCC */

#ifdef EXECSTAMP
	fprintf(stderr, "ExecStamp: HapExecInsert, rel: %u, pid: %d\n",
			resultRelInfo->ri_RelationDesc->rd_id, MyProcPid);
#endif /* EXECSTAMP */
	 ...
	 * HAP's own routine.
	 *
	 * If the target table uses HAP, it has hidden attribute. So foreign key check
 	 * trigger must be performed before the insert (the original routine performs
	 * foreign key check trigger after the insert).
	 *
	 * Foreign key check trigger is triggered for the relations which are only
	 * exists in pg_hap entry. It means that these relations are not a partition,
	 * just regular table or PARTITIONED table. So the trigger must be performed
	 * before the partition routing.
	 *
	 * Foreign key check trigger uses FOR SHARE clause which calls ExecLockRows().
	 * So it is not a problem performing foreign key check trigger before the
	 * insert in aspect of integrity.
	 */
	if (resultRelInfo->ri_TrigDesc &&
		resultRelInfo->ri_TrigDesc->trig_insert_before_row) {
		hap_insert_state->partkeynum = 0;
#ifdef STAT_TPCC
		if (stat_tpcc_breakdown_insert_exec_insert_flag)
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_insert_fkey_check_start);
#endif /* STAT_TPCC */
		HapExecForeignKeyCheckTriggers(estate, resultRelInfo, slot);
#ifdef STAT_TPCC
		if (stat_tpcc_breakdown_insert_exec_insert_flag)
		{
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_insert_fkey_check_end);
		...
		if (stat_tpcc_breakdown_insert_exec_insert_flag)
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_insert_build_hidden_start);
#endif /* STAT_TPCC */
		HapBuildHiddenAttr(hapstate, slot);
#ifdef STAT_TPCC
		if (stat_tpcc_breakdown_insert_exec_insert_flag)
		{
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_insert_build_hidden_end);
	...
	vlock:
		...
		else
		{
#ifdef PANDORA
			/* Get level count */
			lvl_cnt = resultRelationDesc->rd_pandora_level_count;

			/* Alloc new record info */
			pandora_fingerprint = palloc0(SizeOfPandoraFingerprintData(lvl_cnt));

#ifdef STAT_TPCC
			if (stat_tpcc_breakdown_insert_exec_insert_flag)
				clock_gettime(CLOCK_MONOTONIC,
							  &stat_tpcc_breakdown_insert_HapGetPartIdHash_start);
#endif /* STAT_TPCC */

			/* Using the hidden attribute, find the real partition */
			partid = HapGetPartIdHash(resultRelationDesc,
								 hapstate->hap_insert_state->partkeys,
								 hapstate->hap_insert_state->partkeynum);

#ifdef STAT_TPCC
			if (stat_tpcc_breakdown_insert_exec_insert_flag)
			{
				clock_gettime(CLOCK_MONOTONIC,
							  &stat_tpcc_breakdown_insert_HapGetPartIdHash_end);
			...
			pandora_fingerprint->rec_key.partid = partid;
			slot->tts_pandora_fingerprint = pandora_fingerprint;
			slot->lvl_cnt = lvl_cnt;
#endif /* PANDORA */
...
#ifdef PANDORA
			slot->tts_pandora_fingerprint = NULL;
			pfree(pandora_fingerprint);
#endif /* PANDORA */
		}
	}
...
/*
 * HapGetInsertNewTuple
 *		Reallocate a hidden attribute of the slot.
 *
 * The given planSlot has a hidden attribute, but it is a default value which is
 * 0 byte with only the header.
 *
 * Reallocate the hidden attribute to get the right size. Then call the
 * ExecGetInsertNewTuple() which is expected for original INSERT routine.
 */
static TupleTableSlot *
HapGetInsertNewTuple(EState *estate, ResultRelInfo *relinfo,
					 TupleTableSlot *planSlot)
{
	HapInsertState *hap_insert_state = estate->es_hapstate->hap_insert_state;
	MemoryContext oldContext;
	Datum hidden_attribute;
	Oid relid;

	oldContext = MemoryContextSwitchTo(GetPerTupleMemoryContext(estate));

	/* Get a size information to reallocate the hidden attribute */
	relid = RelationGetRelid(relinfo->ri_RelationDesc);
	if (hap_insert_state->nbyte == -1)
		hap_insert_state->nbyte = HapRelidGetHiddenAttrBytesize(relid);

	/*
	 * Reallocate the hidden attribute. The hidden attribute is located in the
	 * last value.
	 *
	 * Note that we have to allocate additional 4bytes for the header of the
	 * byte array.
	 */
	hidden_attribute = (Datum)palloc0(hap_insert_state->nbyte + VARHDRSZ);
	planSlot->tts_values[planSlot->tts_nvalid - 1] = hidden_attribute;
	SET_VARSIZE(hidden_attribute, hap_insert_state->nbyte + VARHDRSZ);

	MemoryContextSwitchTo(oldContext);

	return ExecGetInsertNewTuple(relinfo, planSlot);
}
#endif /* HAP */
