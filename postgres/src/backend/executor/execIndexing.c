					  ...
					  List *arbiterIndexes)
{
...
#ifdef PANDORA
	Datum		values[INDEX_MAX_KEYS + 2]; /* record key, lvl_cnt */
#else
	...
	struct timespec stat_tpcc_breakdown_insert_ExecInsertIndexTuples_start;
	struct timespec stat_tpcc_breakdown_insert_ExecInsertIndexTuples_end;

	struct timespec stat_tpcc_breakdown_insert_index_insert_start;
	struct timespec stat_tpcc_breakdown_insert_index_insert_end;

	if (stat_tpcc_breakdown_insert_exec_insert_flag)
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_ExecInsertIndexTuples_start);
#endif /* STAT_TPCC */
	...
	for (i = 0; i < numIndices; i++)
	{
...
#ifdef PANDORA
		/* Store the pandora info and lvl_cnt of new record */
		values[INDEX_MAX_KEYS] = (Datum)(slot->tts_pandora_fingerprint);
		values[INDEX_MAX_KEYS + 1] = (Datum)(slot->lvl_cnt);
#endif /* PANDORA */
		...
		if (stat_tpcc_breakdown_insert_exec_insert_flag)
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_insert_index_insert_start);
#endif /* STAT_TPCC */
		...
		{
			clock_gettime(CLOCK_MONOTONIC,
						  &stat_tpcc_breakdown_insert_index_insert_end);
	...
	{
		clock_gettime(CLOCK_MONOTONIC,
					  &stat_tpcc_breakdown_insert_ExecInsertIndexTuples_end);
