	...
	if (scandesc == NULL)
	{
		...
		if (node->ioss_NumRuntimeKeys == 0 || node->ioss_RuntimeKeysReady)
			index_rescan(scandesc,
						 ...
						 node->ioss_NumOrderByKeys);
#ifdef EXECSTAMP
		fprintf(stderr, "ExecStamp: IndexOnlyScan, rel: %u, pid: %d\n",
...
#ifdef PANDORA
			if (IsPandora(scandesc->heapRelation))
			{
				node->ioss_TableSlot->tts_pandora_fingerprint =
					scandesc->xs_fingerprint;
			}
#endif /* PANDORA */
