 ...
 */
#ifdef HAP
HAP_HOOK4(SeqScan *, create_seqscan_plan,
...
#ifdef HAP
	scan_plan->scan.plan.hap_partid_list = NULL;
#endif
...
#ifdef HAP
/*
 * Condition to hook the create_seqscan_plan().
 */
HAP_DEFINE_HOOK_COND4(SeqScan *, create_seqscan_plan,
					  PlannerInfo *, root, Path *, best_path,
					  List *, tlst, List *, scan_clauses)
{
	RelOptInfo *rel = best_path->parent;

	if (rel->hap_propagated_exprs != NIL || 
			rel->hap_propagated_joininfo_exprs != NIL)
		return true;
 ...
 */
HAP_DEFINE_HOOK_BODY4(SeqScan *, create_seqscan_plan,
					  PlannerInfo *, root, Path *, best_path,
					  List *, tlist, List *, scan_clauses)
{
	SeqScan    *scan_plan;
	...
	List *hap_partid_list_from_origin = NIL;
	List *hap_partid_list_from_joininfo = NIL;

	/* Get the hidden partition oids */
	if (rel->hap_propagated_exprs)
		hap_partid_list_from_origin = 
			HapGetPartIdListGin(rte->relid, rel->hap_propagated_exprs);

	if (rel->hap_propagated_joininfo_exprs)
		hap_partid_list_from_joininfo = 
			HapGetPartIdListGin(rte->relid, rel->hap_propagated_joininfo_exprs);

	/* Remove duplication */
	if (hap_partid_list_from_origin != NIL && 
			hap_partid_list_from_joininfo != NIL)
	{
		hap_partid_list = 
			list_union_int(hap_partid_list_from_origin, hap_partid_list_from_joininfo); 
		list_free(hap_partid_list_from_origin);
		list_free(hap_partid_list_from_joininfo);
	}
	else if (hap_partid_list_from_origin != NIL)
	{
		hap_partid_list = hap_partid_list_from_origin;
	}
	else if (hap_partid_list_from_joininfo != NIL)
	{
		hap_partid_list = hap_partid_list_from_joininfo;
	}
	...
	scan_plan->scan.plan.hap_partid_list = hap_partid_list;
