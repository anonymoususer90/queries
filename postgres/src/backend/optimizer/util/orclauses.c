 ...
 */
#ifdef HAP
HAP_HOOK1(void, extract_restriction_or_clauses,
		  PlannerInfo *, root)
#else /* !HAP */
void
...
#ifdef HAP
/*
 * Condition to hook the extract_restriction_or_clauses().
 */
HAP_DEFINE_HOOK_COND1(void, extract_restriction_or_clauses,
					  PlannerInfo *, root)
{
	return enable_hap_planner;
}

/*
 * Hook on extract_restriction_or_clauses().
 */
HAP_DEFINE_HOOK_BODY1(void, extract_restriction_or_clauses,
					  PlannerInfo *, root)
{
	Index		rti;
		...
		foreach(lc, rel->joininfo)
		{
				...
				rinfo->norm_selec <= 1)
			{
				...
				if (orclause)
				{
					...
					/*
					 * If the target attribute in the extracted OR clause can be
					 * encoded as a hidden attribute, check whether the original
					 * OR clause has to be propagated instead propagating new OR
					 * clause.
					 */
					if (HapRelidIsEncoded(rte->relid))
						hap_consider_propagating_joininfo(root, rel,
														  rinfo->clause);
				}
			}
		}
